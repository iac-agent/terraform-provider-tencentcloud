...
func TencentKmsBasicInfo() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		"alias": {
			...
			Required:    true,
			Description: "名称 CMK. The 名称 can only contain English letters，numbers，underscore and hyphen '-'. The first character must be a letter or number。",
		},
		"description": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "描述 CMK. The maximum is 1024 bytes。",
		},
		"is_enabled": {
			Type:          schema.TypeBool,
			Optional:      true,
			ConflictsWith: []string{"is_archived"},
			Description:   "指定是否enable 键 默认值为 `false`. This field is conflict with `is_archived`，valid when key_state is `已启用`，`已禁用`，`Archived`。",
		},
		"is_archived": {
			Type:          schema.TypeBool,
			Optional:      true,
			ConflictsWith: []string{"is_enabled"},
			Description:   "指定是否archive 键 默认值为 `false`. This field is conflict with `is_enabled`，valid when key_state is `已启用`，`已禁用`，`Archived`。",
		},
		"pending_delete_window_in_days": {
			...
			Description:  "Duration in days after which the 键 is deleted after destruction of the resource，must be between 7 and 30 days. 默认为 7 days。",
		},
		"tags": {
			Type:        schema.TypeMap,
			Optional:    true,
			Description: "标签 of CMK。",
		},
		"key_state": {
			Type:        schema.TypeString,
			Computed:    true,
...
func ResourceTencentCloudKmsKey() *schema.Resource {
	specialInfo := map[string]*schema.Schema{
		"key_usage": {
			...
			Description: "Usage of CMK. Available values include `ENCRYPT_DECRYPT`，`ASYMMETRIC_DECRYPT_RSA_2048`，`ASYMMETRIC_DECRYPT_SM2`，`ASYMMETRIC_SIGN_VERIFY_SM2`，`ASYMMETRIC_SIGN_VERIFY_RSA_2048`，`ASYMMETRIC_SIGN_VERIFY_ECC`. 默认值为 `ENCRYPT_DECRYPT`。",
		},
		"key_rotation_enabled": {
			Type:        schema.TypeBool,
			Optional:    true,
			Default:     false,
			Description: "指定是否enable 键 rotation，valid when key_usage is `ENCRYPT_DECRYPT`. 默认值为 `false`。",
		},
		"hsm_cluster_id": {
			Type:        schema.TypeString,
			Optional:    true,
