	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"real_name": {
				Required:    true,
				Type:        schema.TypeString,
				Description: "Real 名称，maximum length 20 characters，cannot contain blank characters。",
			},
			"phone": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "Enter it in the 格式 of country area 代码|mobile phone number. For example: +86|***********，+852|xxxxxxxx. Please provide at least one of `phone` or `email`。",
			},
			"email": {
				Optional:    true,
				Type:        schema.TypeString,
			...
			"validate_from": {
				...
				Computed:    true,
				Type:        schema.TypeString,
				Description: "用户 effective time，such as: 2021-09-22T00:00:00+00:00If the effective and expiry time are not filled in，the 用户 will be valid for a long time。",
			},
			"validate_to": {
				Optional:    true,
				Computed:    true,
				Type:        schema.TypeString,
				Description: "用户 过期时间，such as: 2021-09-23T00:00:00+00:00If the effective and expiry time are not filled in，the 用户 will be valid for a long time。",
			},
			"group_id_set": {
				Optional:    true,
				Computed:    true,
				...
				Description: "The set of 用户 group IDs to which it belongs。",
			},
			"auth_type": {
				Optional:    true,
				Type:        schema.TypeInt,
				Description: "Authentication method，0 - local，1 - LDAP，2 - OAuth. 如果未传入，the 默认为 0。",
			},
			"validate_time": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "Access time 周期 限制，a string composed of 0 and 1，length 168 (7 * 24)，representing the time 周期 the 用户 is allowed to access in a week. The Nth character in the string represents the Nth hour of the week，0 - means access is not allowed，1 - means access is allowed。",
			},
			"department_id": {
				Optional:    true,
				Type:        schema.TypeString,
