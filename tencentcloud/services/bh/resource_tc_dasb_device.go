	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"os_name": {
				...
				Description:  "Operating 系统 名称，仅 Linux，Windows 或 MySQL。",
			},
			"ip": {
				Type:        schema.TypeString,
				Required:    true,
			...
			"name": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "Hostname，可以 是 空。",
			},
			"department_id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "department ID 到 其中 device belongs。",
			},
			"ip_port_set": {
				Type:        schema.TypeSet,
				...
				Optional:    true,
				Computed:    true,
				Description: "Asset multi-节点: 字段 ip 和 端口",
			},
		},
	}
}
