				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"mode": {
							...
							Optional:    true,
							Computed:    true,
							Description: "Screenshot 模式，取值范围：{Interval，Average，KeyFrame}- Interval means interval 模式 Average means average 模式- KeyFrame represents the 键 frame 模式- Interval 模式: Start，TimeInterval，The Count parameter takes effect. When Count is set and TimeInterval is not set，表示to capture all frames，a total of Count pictures- Average 模式: Start，the Count parameter takes effect. express。",
						},
						"start": {
							Type:        schema.TypeString,
							Optional:    true,
						...
						"time_interval": {
							...
							Optional:    true,
							Computed:    true,
							Description: "Screenshot 时间间隔，(0 3600]，（秒）， Support float 格式，the execution accuracy is accurate to milliseconds。",
						},
						"count": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "数量 screenshots，range (0 10000]。",
						},
						"width": {
							Type:        schema.TypeString,
							Optional:    true,
							Computed:    true,
							Description: "wide，取值范围：[128，4096]，单位：px，If only Width is set，Height is calculated according to the original ratio of the video。",
						},
						"height": {
							Type:        schema.TypeString,
							Optional:    true,
							Computed:    true,
							Description: "high，取值范围：[128，4096]，单位：px，If only Height is set，Width is calculated according to the original ratio of the video。",
						},
						"ci_param": {
							Type:        schema.TypeString,
							Optional:    true,
							Computed:    true,
							Description: "Screenshot image processing parameters，for example: imageMogr2/格式/png。",
						},
						"is_check_count": {
							Type:        schema.TypeString,
							Optional:    true,
							Computed:    true,
							Description: "是否check the 数量 screenshots forcibly，when using custom interval 模式 to take screenshots，the video time is not long enough to capture Count screenshots，you can switch to average screenshot 模式 to capture Count screenshots。",
						},
						"is_check_black": {
							Type:        schema.TypeString,
							Optional:    true,
						...
						"black_level": {
							...
							Optional:    true,
							Computed:    true,
							Description: "Screenshot black screen detection parameters，Valid when IsCheckBlack=true，值 reference range [30，100]，indicating the proportion of black pixels，the smaller the 值，the smaller the proportion of black pixels，Start&gt;0，the parameter setting is invalid，no filter black screen，Start =0 parameter is valid，the 开始时间 of the frame capture is the first frame non-black screen start。",
						},
						"pixel_black_threshold": {
							Type:        schema.TypeString,
							Optional:    true,
							Computed:    true,
							Description: "Screenshot black screen detection parameters，Valid when IsCheckBlack=true，The threshold for judging whether a pixel is a black point，取值范围：[0，255]。",
						},
						"snapshot_out_mode": {
							Type:        schema.TypeString,
							Optional:    true,
							Computed:    true,
							Description: "Screenshot output 模式 parameters，取值范围：{OnlySnapshot，OnlySprite，SnapshotAndSprite}，OnlySnapshot means output only screenshot 模式 OnlySprite means only output sprite 模式 SnapshotAndSprite means output screenshot and sprite 模式",
						},
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"cell_width": {
										...
										Optional:    true,
										Computed:    true,
										Description: "Single image width 取值范围：[8，4096]，单位：px。",
									},
									"cell_height": {
										Type:        schema.TypeString,
										Optional:    true,
										Computed:    true,
										Description: "Single image height 取值范围：[8，4096]，单位：px。",
									},
									"padding": {
										Type:        schema.TypeString,
										Optional:    true,
										Computed:    true,
										Description: "screenshot padding size，取值范围：[8，4096]，单位：px。",
									},
									"margin": {
										Type:        schema.TypeString,
										Optional:    true,
										Computed:    true,
										Description: "screenshot margin size，取值范围：[8，4096]，单位：px。",
									},
									"color": {
										Type:        schema.TypeString,
										Required:    true,
