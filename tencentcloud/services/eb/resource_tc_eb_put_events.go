				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"source": {
							...
							Required:    true,
							Description: "Event 来源 information，new product reporting must comply with EB specifications。",
						},
						"data": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Event data，the 内容 is controlled by the system that created the event，the current datacontenttype only supports application/json;charset=utf-8，so this field is a json string。",
						},
						"type": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Event 类型，customizable，可选 The cloud service writes COS:Created:PostObject by default，use: to separate the 类型 field。",
						},
						"subject": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Detailed 描述 event 来源，customizable，可选 The cloud service 默认为 the standard qcs resource representation syntax: qcs::dts:ap-guangzhou:appid/uin:xxx。",
						},
						"time": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "The 时间戳 in milliseconds when the event occurred,time.Now().UnixNano()/1e6。",
						},
					},
				},
			},
