	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"template_name": {
				...
				Required:    true,
				Description: "模板名称，仅 support 0-9 和 -z。",
			},

			"video_bitrate": {
				Type:        schema.TypeInt,
				Required:    true,
				Description: "视频 bitrate，0 对于 源站，范围 0kbps - 8000kbps。",
			},

			"acodec": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "默认值 aac，不 support now。",
			},

			"audio_bitrate": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "默认值 0，范围 0 - 500。",
			},

			"vcodec": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "视频 codec，默认值 源站，support h264/h265/源站。",
			},
			...
			"need_video": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "keep 视频 或 不，默认值 1 对于 yes，0 对于 无。",
			},

			"width": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "template 宽度，默认值 0，范围 0 - 3000，必须 是 pow 的 2。",
			},

			"need_audio": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "keep 音频 或 不，默认值 1 对于 yes，0 对于 无。",
			},

			"height": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "template 高度，默认值 0，范围 0 - 3000，必须 是 pow 的 2，needed while AiTransCode = 1。",
			},

			"fps": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "视频 fps，默认值 0，范围 0 - 60。",
			},

			"gop": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "gop 的 视频，second，默认值 源站 的 视频，范围 2 - 6。",
			},

			"rotate": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "roate degree，默认值 0，support 0/90/180/270。",
			},

			"profile": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "quality 的 视频，默认值 baseline，support baseline/main/high。",
			},

			"bitrate_to_orig": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "base 在 源站 bitrate 如果 源站 bitrate 是 lower 比 setting bitrate. 默认值 0，1 对于 yes，0 对于 无。",
			},

			"height_to_orig": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "base 在 源站 高度 如果 源站 高度 是 lower 比 setting 高度. 默认值 0，1 对于 yes，0 对于 无。",
			},

			"fps_to_orig": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "base 在 源站 fps 如果 源站 fps 是 lower 比 setting fps. 默认值 0，1 对于 yes，0 对于 无。",
			},

			"ai_trans_code": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "启用 high speed 模式，默认值 0，1 对于 启用，0 对于 无。",
			},
			...
			"short_edge_as_height": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "let short edge 作为 高度。",
			},
...
func resourceTencentCloudCssLiveTranscodeTemplateCreate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_css_live_transcode_template.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseCssClient().CreateLiveTranscodeTemplate(request)
		if e != nil {
...
func resourceTencentCloudCssLiveTranscodeTemplateRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_css_live_transcode_template.read")()
	defer tccommon.InconsistentCheck(d, meta)()
...
func resourceTencentCloudCssLiveTranscodeTemplateUpdate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_css_live_transcode_template.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseCssClient().ModifyLiveTranscodeTemplate(request)
		if e != nil {
			return tccommon.RetryError(e)
		} else {
			log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n",
...
func resourceTencentCloudCssLiveTranscodeTemplateDelete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_css_live_transcode_template.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
