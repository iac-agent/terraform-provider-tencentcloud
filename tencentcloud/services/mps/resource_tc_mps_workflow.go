	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"workflow_name": {
				...
				Description: "Workflow 名称，up to 128 characters. The 名称 is unique for the same 用户",
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"type": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "类型 trigger，currently only supports CosFileUpload。",
						},
						"cos_file_upload_trigger": {
							Type:        schema.TypeList,
							MaxItems:    1,
							Optional:    true,
							Description: "Mandatory and valid when 类型 is CosFileUpload，the rule is triggered for COS.注意：此字段可能返回 null，表示无法获取有效值。",
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"bucket": {
										Type:        schema.TypeString,
										Required:    true,
										Description: "The 名称 COS 存储桶 bound to the workflow。",
									},
									"region": {
										Type:        schema.TypeString,
										Required:    true,
										Description: "The park to which the COS 存储桶 bound to the workflow belongs。",
									},
									"dir": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "The input 路径 directory of the workflow binding must be an absolute 路径，that is，start and end with `/`。",
									},
									"formats": {
										Type: schema.TypeSet,
										Elem: &schema.Schema{
										...
										Optional:    true,
										Computed:    true,
										Description: "A 列表 file formats that are allowed to be triggered by the workflow，如果未填写 in，it means that files of all formats can trigger the workflow。",
									},
								},
							},
						},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"type": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "类型 media processing output object storage location，now only supports COS。",
						},
						"cos_output_storage": {
							Type:        schema.TypeList,
							MaxItems:    1,
							Optional:    true,
							Description: "Valid when 类型 is COS，this item 为必填项，indicating the media processing COS output location.注意：此字段可能返回 null，表示无法获取有效值。",
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"bucket": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "The target 存储桶 名称 file output generated by media processing，如果未填写，it means the upper layer。",
									},
									"region": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "The park of the target 存储桶 for the output of the file generated by media processing. 如果未填写，it means inheriting from the upper layer。",
									},
								},
							},
						},
			...
			"output_dir": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "The target directory of the 输出文件 generated by media processing，如果未填写，it means that it is consistent with the directory where the trigger file is located。",
			},
			...
			"media_process_task": {
				...
				Type:        schema.TypeList,
				MaxItems:    1,
				Description: "Media Processing 类型 Task Parameters。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"transcode_task_set": {
							Type:        schema.TypeList,
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									"raw_parameter": {
										...
										Optional:    true,
										Computed:    true,
										Description: "Video transcoding custom parameters，valid when Definition is filled with 0.This parameter is used in highly customized scenarios. It is recommended that you use Definition to 指定transcoding parameters first.注意：此字段可能返回 null，表示无法获取有效值。",
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"container": {
													Type:        schema.TypeString,
										...
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												...
												"remove_video": {
													Type:        schema.TypeInt,
													Optional:    true,
													Description: "是否remove video data，值:0: reserved.1: remove。",
												},
												"remove_audio": {
													Type:        schema.TypeInt,
													Optional:    true,
													Description: "是否remove audio data，值:0: reserved.1: remove。",
												},
												"video_template": {
													Type:        schema.TypeList,
													MaxItems:    1,
													...
													Elem: &schema.Resource{
														Schema: map[string]*schema.Schema{
															...
															"fps": {
																Type:        schema.TypeInt,
																Optional:    true,
																Description: "Video frame rate，取值范围：[0，100]，unit: Hz.When the 值 is 0，it means that the frame rate is consistent with the original video。",
															},
															"bitrate": {
																Type:        schema.TypeInt,
																Optional:    true,
															...
															"resolution_adaptive": {
																Type:        schema.TypeString,
																Optional:    true,
																Description: "Adaptive resolution，可选 values:```open: open，at this time，Width represents the long side of the video，Height represents the short side of the video.close: close，at this time，Width represents the width of the video，and Height represents the height of the video.Note: In adaptive 模式，Width cannot be smaller than Height。",
															},
															"width": {
																Type:        schema.TypeInt,
																Optional:    true,
																Description: "The maximum 值 of video stream width (or long side)，取值范围：0 and [128，4096]，unit: px.When Width and Height are both 0，the resolution is the same.When Width is 0 and Height is not 0，Width is scaled proportionally.When Width is not 0 and Height is 0，Height is scaled proportionally.When both Width and Height are not 0，the resolution is specified by the 用户",
															},
															"height": {
																Type:        schema.TypeInt,
																Optional:    true,
																Description: "The maximum 值 of video stream height (or short side)，取值范围：0 and [128，4096]，unit: px。",
															},
															"gop": {
																Type:        schema.TypeInt,
																Optional:    true,
															...
															"vcrf": {
																Type:        schema.TypeInt,
																Optional:    true,
																Description: "Video constant bit rate control factor，the 值 range is [1，51]，Fill in 0 to disable this parameter.If there is no special requirement，it is not recommended to 指定this parameter。",
															},
															"content_adapt_stream": {
																Type:        schema.TypeInt,
																Optional:    true,
																Description: "内容 Adaptive Encoding. 可选 值:0: not open.1: open.默认值：0.When this parameter is turned on，multiple 代码 streams with different resolutions and different bit rates will be adaptively generated. The width and height of the VideoTemplate are the maximum resolutions among the multiple 代码 streams，and the bit rates in the VideoTemplate are multiple 代码 rates. The highest bit rate in the stream，the vcrf in VideoTemplate is the highest quality among multiple bit streams. When the resolution，bit rate and vcrf are not set，the highest resolution generated by the ContentAdaptStream parameter is the resolution of the video 来源，and the video quality is close to vmaf95. To enable this parameter or learn about billing details，please contact your Tencent Cloud Business。",
															},
														},
													},
												},
													...
													Elem: &schema.Resource{
														Schema: map[string]*schema.Schema{
															...
															"audio_channel": {
																Type:        schema.TypeInt,
																Optional:    true,
																Description: "Audio channel 模式，可选 values:`1: single channel.2: Dual channel.6: Stereo.When the package 格式 of the media is an audio 格式 (flac，ogg，mp3，m4a)，the 数量 channels is not allowed to be set to stereo。",
															},
															"stream_selects": {
																Type: schema.TypeSet,
																Elem: &schema.Schema{
																...
																Optional:    true,
																Description: "指定audio track to preserve for the output. The 默认为 to keep all sources。",
															},
														},
													},
												},
													...
													Elem: &schema.Resource{
														Schema: map[string]*schema.Schema{
															...
															"max_video_bitrate": {
																Type:        schema.TypeInt,
																Optional:    true,
																Description: "The upper 限制 of the video bit rate，No filling means no modification。",
															},
														},
													},
												},
													...
													Elem: &schema.Resource{
														Schema: map[string]*schema.Schema{
															"path": {
																Type:        schema.TypeString,
																Optional:    true,
																Description: "The 地址 of the subtitle file to be compressed into the video。",
															},
															"stream_index": {
																Type:        schema.TypeInt,
																Optional:    true,
																Description: "指定subtitle track to be compressed into the video. If there is a specified 路径，the 路径 has a higher 优先级 路径 and StreamIndex 指定at least one。",
															},
															"font_type": {
																Type:        schema.TypeString,
																Optional:    true,
															...
															"font_size": {
																Type:        schema.TypeString,
																Optional:    true,
																Description: "Font size，格式: Npx，N is a 值，如果未指定，the subtitle file shall prevail。",
															},
															"font_color": {
																Type:        schema.TypeString,
																Optional:    true,
									...
									"watermark_set": {
										Type:        schema.TypeList,
										Optional:    true,
										Description: "Watermark list，support multiple pictures or text watermarks，up to 10.注意：此字段可能返回 null，表示无法获取有效值。",
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"definition": {
													Type:        schema.TypeInt,
												...
												"raw_parameter": {
													...
													Optional:    true,
													Computed:    true,
													Description: "Watermark custom parameters，valid when Definition is filled with 0.This parameter is used in highly customized scenarios，it is recommended that you use Definition to 指定watermark parameters first.Watermark custom parameters do not support screenshot watermarking。",
													Elem: &schema.Resource{
														Schema: map[string]*schema.Schema{
															"type": {
																Type:        schema.TypeString,
																Required:    true,
																Description: "Watermark 类型，可选 值:image: image watermark。",
															},
															"coordinate_origin": {
																Type:        schema.TypeString,
																Optional:    true,
																Description: "Origin position，currently only supports:TopLeft: 表示that the origin of the coordinates is at the upper left corner of the video image，and the origin of the watermark is the upper left corner of the picture or text.默认值：TopLeft。",
															},
															"x_pos": {
																Type:        schema.TypeString,
																Optional:    true,
															...
															"image_template": {
																...
																MaxItems:    1,
																Optional:    true,
																Description: "Image watermark template，when 类型 is image，this field 为必填项. When 类型 is text，this field is invalid。",
																Elem: &schema.Resource{
																	Schema: map[string]*schema.Schema{
																		"image_content": {
																			Type:        schema.TypeList,
																			MaxItems:    1,
																			Required:    true,
																			Description: "The input 内容 of the watermark image. Support jpeg，png image 格式",
																			Elem: &schema.Resource{
																				Schema: map[string]*schema.Schema{
																					"type": {
																						Type:        schema.TypeString,
																						Required:    true,
																						Description: "Enter 类型 来源 object，which supports COS and URL",
																					},
																					"cos_input_info": {
																						Type:        schema.TypeList,
																						MaxItems:    1,
																						Optional:    true,
																						Description: "Valid when 类型 is COS，this item 为必填项，indicating media processing COS object information。",
																						Elem: &schema.Resource{
																							Schema: map[string]*schema.Schema{
																								"bucket": {
																									Type:        schema.TypeString,
																									Required:    true,
																									Description: "The 名称 COS 存储桶 where the media processing object file is located。",
																								},
																								"region": {
																									Type:        schema.TypeString,
																									Required:    true,
																									Description: "The park to which the COS 存储桶 where the media processing target file resides belongs。",
																								},
																								"object": {
																									Type:        schema.TypeString,
																									Required:    true,
																									Description: "Input 路径 for media processing object files。",
																								},
																							},
																						},
																					},
																					"url_input_info": {
																						...
																						MaxItems:    1,
																						Optional:    true,
																						Description: "Valid when 类型 is URL，this item 为必填项，indicating media processing URL object information.注意：此字段可能返回 null，表示无法获取有效值。",
																						Elem: &schema.Resource{
																							Schema: map[string]*schema.Schema{
																								"url": {
																									Type:        schema.TypeString,
																		...
																		"width": {
																			Type:        schema.TypeString,
																			Optional:    true,
																			Description: "The width of the watermark. Support %，px two formats:When the string ends with %，it means that the watermark Width is a percentage of the video width，such as 10% means that the Width is 10% of the video width.When the string ends with px，it means that the watermark Width unit is pixels，such as 100px means that the Width is 100 pixels.默认值：10%。",
																		},
																		"height": {
																			Type:        schema.TypeString,
																			Optional:    true,
																			Description: "The height of the watermark. Support %，px two formats:When the string ends with %，it means that the watermark Height is the percentage size of the video height，such as 10% means that the Height is 10% of the video height.When the string ends with px，it means that the watermark Height unit is pixel，such as 100px means that the Height is 100 pixels.默认值：0px，indicating that Height is scaled according to the aspect ratio of the original watermark image。",
																		},
																		"repeat_type": {
																			Type:        schema.TypeString,
																			Optional:    true,
												...
												"text_content": {
													Type:        schema.TypeString,
													Optional:    true,
													Description: "Text 内容，the length does not exceed 100 characters. Fill in only when the watermark 类型 is text watermark.Text watermark does not support screenshot watermarking。",
												},
												"svg_content": {
													Type:        schema.TypeString,
													Optional:    true,
													Description: "SVG 内容 The length cannot exceed 2000000 characters. Fill in only if the watermark 类型 is SVG watermark.SVG watermark does not support screenshot watermarking。",
												},
												"start_time_offset": {
													Type:        schema.TypeFloat,
													Optional:    true,
													Description: "The 开始时间 偏移量 of the watermark，unit: second. Do not fill in or fill in 0，which means that the watermark will start to appear when the screen appears.Do not fill in or fill in 0，which means the watermark will appear from the beginning of the screen.When the 值 is greater than 0 (assumed to be n)，it means that the watermark appears from the nth second of the screen.When the 值 is less than 0 (assumed to be -n)，it means that the watermark starts to appear n seconds before the end of the screen。",
												},
												"end_time_offset": {
													Type:        schema.TypeFloat,
													Optional:    true,
													Description: "结束时间 偏移量 of watermark，unit: second.Do not fill in or fill in 0，indicating that the watermark lasts until the end of the screen.When the 值 is greater than 0 (assumed to be n)，it means that the watermark lasts until the nth second and disappears.When the 值 is less than 0 (assumed to be -n)，it means that the watermark lasts until it disappears n seconds before the end of the screen。",
												},
											},
										},
									},
										...
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"coordinate_origin": {
													Type:        schema.TypeString,
													Optional:    true,
													Description: "Origin position，currently only supports:TopLeft: 表示that the coordinate origin is located in the upper left corner of the video image，and the origin of the mosaic is the upper left corner of the picture or text默认值：TopLeft。",
												},
												"x_pos": {
													Type:        schema.TypeString,
													Optional:    true,
												...
												"width": {
													Type:        schema.TypeString,
													Optional:    true,
													Description: "The width of the mosaic. Support %，px two formats:When the string ends with %，it means that the mosaic Width is the percentage size of the video width，such as 10% means that the Width is 10% of the video width.The string ends with px，indicating that the mosaic Width unit is pixels，such as 100px 表示that the Width is 100 pixels.默认值：10%。",
												},
												"height": {
													Type:        schema.TypeString,
													Optional:    true,
													Description: "The height of the mosaic. Support %，px two formats.When the string ends with %，it means that the mosaic Height is the percentage size of the video height，such as 10% means that the Height is 10% of the video height.When the string ends with px，it means that the mosaic Height unit is pixel，such as 100px means that the Height is 100 pixels.默认值：10%。",
												},
												"start_time_offset": {
													Type:        schema.TypeFloat,
													Optional:    true,
													Description: "The 开始时间 偏移量 of the mosaic，unit: second. Do not fill or fill in 0，which means that the mosaic will start to appear when the screen appears.Fill in or fill in 0，which means that the mosaic will appear from the beginning of the screen.When the 值 is greater than 0 (assumed to be n)，it means that the mosaic appears from the nth second of the screen.When the 值 is less than 0 (assumed to be -n)，it means that the mosaic starts to appear n seconds before the end of the screen。",
												},
												"end_time_offset": {
													Type:        schema.TypeFloat,
													Optional:    true,
													Description: "The 结束时间 偏移量 of the mosaic，unit: second.Fill in or fill in 0，indicating that the mosaic continues until the end of the screen.When the 值 is greater than 0 (assumed to be n)，it means that the mosaic lasts until the nth second and disappears.When the 值 is less than 0 (assumed to be -n)，it means that the mosaic lasts until it disappears n seconds before the end of the screen。",
												},
											},
										},
									},
									"start_time_offset": {
										Type:        schema.TypeFloat,
										Optional:    true,
										Description: "The 开始时间 偏移量 of the transcoded video，unit: second.Do not fill in or fill in 0，indicating that the transcoded video starts from the beginning of the original video.When the 值 is greater than 0 (assumed to be n)，it means that the transcoded video starts from the nth second position of the original video.When the 值 is less than 0 (assumed to be -n)，it means that the transcoded video starts from the position n seconds before the end of the original video。",
									},
									"end_time_offset": {
										Type:        schema.TypeFloat,
										Optional:    true,
										Description: "结束时间 偏移量 of video after transcoding，unit: second.Do not fill in or fill in 0，indicating that the transcoded video continues until the end of the original video.When the 值 is greater than 0 (assumed to be n)，it means that the transcoded video lasts until the nth second of the original video and terminates.When the 值 is less than 0 (assumed to be -n)，it means that the transcoded video lasts until n seconds before the end of the original video。",
									},
									"output_storage": {
										Type:        schema.TypeList,
										MaxItems:    1,
										Optional:    true,
										Description: "The target storage of the transcoded file，如果未填写，it will inherit the OutputStorage 值 of the upper layer.注意：此字段可能返回 null，表示无法获取有效值。",
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"type": {
													Type:        schema.TypeString,
									...
									"output_object_path": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "The 输出路径 of the main file after transcoding can be a relative 路径 or an absolute 路径 如果未填写，the 默认为 a relative 路径: {inputName}_transcode_{definition}.{格式}。",
									},
									"segment_object_name": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "The 输出路径 of the transcoded fragment file (the 路径 of ts when transcoding HLS)，can only be a relative 路径 如果未填写，the default is: `{inputName}_transcode_{definition}_{number}.{格式}。",
									},
									"object_number_format": {
										Type:        schema.TypeList,
										MaxItems:    1,
										Optional:    true,
										Description: "Rules for the `{number}` variable in the 输出路径 after transcoding.注意：此字段可能返回 null，表示无法获取有效值。",
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"initial_value": {
													Type:        schema.TypeInt,
													Optional:    true,
													Description: "The starting 值 of `{number}` variable，the 默认为 0。",
												},
												"increment": {
													Type:        schema.TypeInt,
													Optional:    true,
													Description: "The growth step of the `{number}` variable，the 默认为 1。",
												},
												"min_length": {
													Type:        schema.TypeInt,
													Optional:    true,
													Description: "The 最小长度the `{number}` variable，if insufficient，placeholders will be filled. 默认为 1。",
												},
												"place_holder": {
													Type:        schema.TypeString,
													Optional:    true,
													Description: "When the length of the `{number}` variable is insufficient，a placeholder is added. 默认为 0。",
												},
											},
										},
									},
									"head_tail_parameter": {
										...
										MaxItems:    1,
										Optional:    true,
										Description: "Opening and ending parameters.注意：此字段可能返回 null，表示无法获取有效值。",
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"head_set": {
													Type:        schema.TypeList,
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									"start_time_offset": {
										Type:        schema.TypeFloat,
										Required:    true,
										Description: "The 开始时间 of the animation in the video，（秒）。",
									},
									"end_time_offset": {
										Type:        schema.TypeFloat,
										Required:    true,
										Description: "The 结束时间 of the animation in the video，（秒）。",
									},
									"output_storage": {
										Type:        schema.TypeList,
										MaxItems:    1,
									...
									"output_object_path": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "The 输出路径 of the file after rotating the image，which can be a relative 路径 or an absolute 路径 如果未填写，the 默认为 a relative 路径: {inputName}_animatedGraphic_{definition}.{格式}。",
									},
								},
							},
						},
						"snapshot_by_time_offset_task_set": {
							Type:        schema.TypeList,
							Optional:    true,
							Description: "Screenshot the task 列表 the video according to the time point。",
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"definition": {
										Type:        schema.TypeInt,
										Required:    true,
										Description: "Specified time point screenshot 模板 ID",
									},
									"ext_time_offset_set": {
										Type: schema.TypeSet,
										Elem: &schema.Schema{
										...
										Optional:    true,
										Description: "Screenshot time point list，the time point supports two formats: s and %:;When the string ends with s，it means that the time point is （秒）， such as 3.5s means that the time point is the 3.5th second.When the string ends with %，it means that the time point is the percentage of the video duration，such as 10% means that the time point is the first 10% of the time in the video。",
									},
									"time_offset_set": {
										Type: schema.TypeSet,
										Elem: &schema.Schema{
									...
									"output_storage": {
										...
										MaxItems:    1,
										Optional:    true,
										Description: "The target storage of the file after the screenshot at the time point，如果未填写，it will inherit the OutputStorage 值 of the upper layer.注意：此字段可能返回 null，表示无法获取有效值。",
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"type": {
													Type:        schema.TypeString,
									...
									"output_object_path": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "The 输出路径 of the picture file after the snapshot at the time point can be a relative 路径 or an absolute 路径 如果未填写，the 默认为 a relative 路径: `{inputName}_snapshotByTimeOffset_{definition}_{number}.{格式}`。",
									},
									"object_number_format": {
										Type:        schema.TypeList,
										MaxItems:    1,
										Optional:    true,
										Description: "Rules for the `{number}` variable in the 输出路径 after the screenshot at the time point.注意：此字段可能返回 null，表示无法获取有效值。",
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"initial_value": {
													Type:        schema.TypeInt,
												...
												"increment": {
													Type:        schema.TypeInt,
													Optional:    true,
													Description: "The growth step of `{number}` variable，默认为 1。",
												},
												"min_length": {
													Type:        schema.TypeInt,
													Optional:    true,
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									"output_object_path": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "The 输出路径 of the image file after sampling the screenshot，which can be a relative 路径 or an absolute 路径 如果未填写，the 默认为 a relative 路径: `{inputName}_sampleSnapshot_{definition}_{number}.{格式}`。",
									},
									"object_number_format": {
										Type:        schema.TypeList,
										MaxItems:    1,
										Optional:    true,
										Description: "Rules for the `{number}` variable in the 输出路径 after sampling the screenshot.注意：此字段可能返回 null，表示无法获取有效值。",
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"initial_value": {
													Type:        schema.TypeInt,
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									"output_storage": {
										...
										MaxItems:    1,
										Optional:    true,
										Description: "The target storage of the file after the sprite image is intercepted，如果未填写，it will inherit the OutputStorage 值 of the upper layer.注意：此字段可能返回 null，表示无法获取有效值。",
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"type": {
													Type:        schema.TypeString,
									...
									"output_object_path": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "After capturing the sprite image，the 输出路径 of the sprite image file can be a relative 路径 or an absolute 路径 如果未填写，the 默认为 a relative 路径: `{inputName}_imageSprite_{definition}_{number}.{格式}`。",
									},
									"web_vtt_object_name": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "After capturing the sprite image，the 输出路径 of the Web VTT file can only be a relative 路径 如果未填写，the 默认为 a relative 路径: `{inputName}_imageSprite_{definition}.{格式}`。",
									},
									"object_number_format": {
										Type:        schema.TypeList,
										MaxItems:    1,
										Optional:    true,
										Description: "Rules for the `{number}` variable in the 输出路径 after intercepting the Sprite image.注意：此字段可能返回 null，表示无法获取有效值。",
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"initial_value": {
													Type:        schema.TypeInt,
						...
						"adaptive_dynamic_streaming_task_set": {
							Type:        schema.TypeList,
							Optional:    true,
							Description: "Transfer Adaptive 代码 Stream Task List。",
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"definition": {
										Type:        schema.TypeInt,
									...
									"output_storage": {
										...
										MaxItems:    1,
										Optional:    true,
										Description: "The target storage of the file after converting to the adaptive 代码 stream，如果未填写，it will inherit the OutputStorage 值 of the upper layer.注意：此字段可能返回 null，表示无法获取有效值。",
										Elem: &schema.Resource{
											Schema: map[string]*schema.Schema{
												"type": {
													Type:        schema.TypeString,
									...
									"output_object_path": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "After converting to an adaptive stream，the 输出路径 of the manifest file can be a relative 路径 or an absolute 路径 如果未填写，the 默认为 a relative 路径: `{inputName}_adaptiveDynamicStreaming_{definition}.{格式}`。",
									},
									"sub_stream_object_name": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "After converting to an adaptive stream，the 输出路径 of the sub-stream file can only be a relative 路径 如果未填写，the 默认为 a relative 路径: {inputName}_adaptiveDynamicStreaming_{definition}_{subStreamNumber}.{格式}`。",
									},
									"segment_object_name": {
										Type:        schema.TypeString,
										Optional:    true,
										Description: "After converting to an adaptive stream (only HLS)，the 输出路径 of the fragmented file can only be a relative 路径 如果未填写，the 默认为 a relative 路径: `{inputName}_adaptiveDynamicStreaming_{definition}_{subStreamNumber}_{segmentNumber}.{格式}`。",
									},
								},
							},
						},
			...
			"ai_content_review_task": {
				...
				Type:        schema.TypeList,
				MaxItems:    1,
				Description: "Video 内容 Moderation 类型 Task Parameters。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"definition": {
							Type:        schema.TypeInt,
			...
			"ai_analysis_task": {
				...
				Type:        schema.TypeList,
				MaxItems:    1,
				Description: "Video 内容 Analysis 类型 Task Parameters。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"definition": {
							Type:        schema.TypeInt,
						...
						"extended_parameter": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Extension parameter whose 值 is a serialized json string.Note: This parameter is a customized demand parameter，which requires offline docking.注意：此字段可能返回 null，表示无法获取有效值。",
						},
					},
				},
			},
			...
			"ai_recognition_task": {
				...
				Type:        schema.TypeList,
				MaxItems:    1,
				Description: "Video 内容 recognition 类型 task parameters。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"definition": {
							Type:        schema.TypeInt,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"cmq_region": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "地域 of CMQ or TDMQ-CMQ，such as sh，bj，etc。",
						},
						"topic_name": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Valid when the model is a Topic，indicating the topic 名称 CMQ or TDMQ-CMQ that receives event notifications。",
						},
						"queue_name": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Valid when the model is Queue，indicating the queue 名称 CMQ or TDMQ-CMQ that receives the event notification。",
						},
						"notify_mode": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "The 模式 of the workflow notification，the possible values are Finish and Change，leaving blank means Finish。",
						},
						"notify_type": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Notification 类型，可选 值:CMQ: offline，it is recommended to switch to TDMQ-CMQ.TDMQ-CMQ: 消息 queue.URL: When the URL is specified，the HTTP callback is pushed to the 地址 specified by NotifyUrl，the callback 协议 is http+json，and the package body 内容 is the same as the output parameters of the parsing event notification interface.SCF: not recommended，additional configuration of SCF in the console 为必填项.Note: CMQ is the default when not filled or empty，if you need to use other types，you need to fill in the corresponding 类型 值",
						},
						"notify_url": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "HTTP callback 地址，必填 when NotifyType is URL",
						},
					},
				},
			},
			...
			"task_priority": {
				...
				Type:        schema.TypeInt,
				Default:     0,
				Description: "The 优先级 of the workflow，the larger the 值，the higher the 优先级，the 值 range is -10 to 10，and blank means 0。",
			},
		},
	}
}
