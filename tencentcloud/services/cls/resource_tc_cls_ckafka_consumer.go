	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"effective": {
				...
				Computed:    true,
				Type:        schema.TypeBool,
				Description: "whether the delivery task is effective.",
			},

			"role_arn": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "role access description name for cross-account access.",
			},
			...
			"advanced_config": {
				...
				Type:        schema.TypeList,
				MaxItems:    1,
				Description: "advanced configuration for ckafka consumer.",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"partition_hash_status": {
							Type:        schema.TypeBool,
							Optional:    true,
							Description: "whether to enable partition hash. true: enable; false: disable.",
						},
						"partition_fields": {
							Type: schema.TypeSet,
							Elem: &schema.Schema{
							...
							Optional:    true,
							Description: "fields for partition hash, max 5 fields.",
						},
					},
				},
			},
...
func resourceTencentCloudClsCkafkaConsumerCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceTencentCloudClsCkafkaConsumerUpdate(d, meta)
}
