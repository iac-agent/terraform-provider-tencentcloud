	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"compression": {
				...
				Computed:    true,
				Type:        schema.TypeInt,
				Description: "压缩方式：0-NONE、2-SNAPPY、3-LZ4。",
			},
			"consumer_content": {
				Optional:    true,
				Computed:    true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"format": {
							...
							Optional:    true,
							Computed:    true,
							Description: "内容格式：0-原始内容，1-JSON。",
						},
						"enable_tag": {
						...
						"tag_transaction": {
							...
							Optional:    true,
							Computed:    true,
							Description: "标签数据处理方式：1-不扁平化，2-扁平化。",
						},
						"json_type": {
							Type:        schema.TypeInt,
							Optional:    true,
							Computed:    true,
							Description: "消费数据JSON格式：1-不转义，2-转义。",
						},
					},
				},
			},
			"topic_id": {
				Computed:    true,
				Type:        schema.TypeString,
				Description: "KafkaConsumer消费时使用的Topic参数。",
			},
		},
	}
}
