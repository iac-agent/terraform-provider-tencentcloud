	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"subscription_name": {
				...
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "Subscriber 名称，其中 可以 contain up 到 128 字符。",
			},
			"remark": {
				Optional:    true,
				ForceNew:    true,
			...
			"auto_create_policy_topic": {
				...
				Default:     false,
				Description: "是否automatically create dead letter 主题 和 retry letter 主题. true: yes; false: 无(默认值)。",
			},
			"auto_delete_policy_topic": {
				...
				Type:        schema.TypeBool,
				Default:     false,
				Description: "是否automatically delete dead letter 主题 和 retry letter 主题. Setting 是 仅 allowed 当 `auto_create_policy_topic` 是 true. 默认为 false。",
			},
		},
	}
}
