	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"subscription_name": {
				...
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "Subscriber 名称，which can contain up to 128 characters。",
			},
			"remark": {
				Optional:    true,
				ForceNew:    true,
			...
			"auto_create_policy_topic": {
				...
				Default:     false,
				Description: "是否automatically create a dead letter topic and a retry letter topic. true: yes; false: no(默认值)。",
			},
			"auto_delete_policy_topic": {
				...
				Type:        schema.TypeBool,
				Default:     false,
				Description: "是否automatically delete a dead letter topic and a retry letter topic. Setting is only allowed when `auto_create_policy_topic` is true. 默认为 false。",
			},
		},
	}
}
