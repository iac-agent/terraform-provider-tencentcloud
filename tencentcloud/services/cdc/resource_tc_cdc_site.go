	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"fiber_type": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "Site Fiber 类型 Using optical fiber 类型 to connect the CDC device to the network SM(Single-模式) or MM(Multi-模式) fibers are available。",
			},
			"optical_standard": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "Site Optical Standard. Optical standard 用于connect the CDC device to the network This field depends on the uplink speed，optical fiber 类型，and distance to upstream equipment. Allow 值: `SM`，`MM`。",
			},
			"power_connectors": {
				Optional:    true,
				Type:        schema.TypeString,
			...
			"power_feed_drop": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "Site Power Feed Drop. Whether power is supplied from above or below the rack. Allow 值: `UP`，`DOWN`。",
			},
			"max_weight": {
				Optional:    true,
				Computed:    true,
				Type:        schema.TypeInt,
				Description: "Site Max 权重 capacity (KG)。",
			},
			"power_draw_kva": {
				Optional:    true,
				Computed:    true,
			...
			"optional_address_line": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "Detailed 地址 of the site area (to be added)。",
			},
			"need_help": {
				Optional:    true,
				Computed:    true,
