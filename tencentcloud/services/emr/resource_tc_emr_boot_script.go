...
func ResourceTencentCloudEmrBootScript() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudEmrBootScriptCreate,
		Read:   resourceTencentCloudEmrBootScriptRead,
		Update: resourceTencentCloudEmrBootScriptUpdate,
		Delete: resourceTencentCloudEmrBootScriptDelete,
...
func resourceTencentCloudEmrBootScriptCreate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_emr_boot_script.create")()
	...
	return resourceTencentCloudEmrBootScriptUpdate(d, meta)
}

func resourceTencentCloudEmrBootScriptRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_emr_boot_script.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		log.Printf("[WARN]%s resource `tencentcloud_emr_boot_script` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		d.SetId("")
		return nil
	}
	...
	for _, s := range rawSettings {
		...
		if s.RunOrder != nil {
			settingsMap["run_order"] = int(*s.RunOrder)
		}
...
func resourceTencentCloudEmrBootScriptUpdate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_emr_boot_script.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if reqErr != nil {
		log.Printf("[CRITAL]%s update emr boot script failed, reason:%+v", logId, reqErr)
		return reqErr
	}

	return resourceTencentCloudEmrBootScriptRead(d, meta)
}

func resourceTencentCloudEmrBootScriptDelete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_emr_boot_script.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	request.InstanceId = helper.String(parts[0])
	request.BootType = helper.String(parts[1])
	request.PreExecutedFileSettings = []*emr.PreExecuteFileSetting{}
	...
	if reqErr != nil {
		log.Printf("[CRITAL]%s delete emr boot script failed, reason:%+v", logId, reqErr)
		return reqErr
	}
