	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"topic_name": {
				...
				Required:    true,
				ForceNew:    true,
				Description: "名称 CKafka topic. It must start with a letter，the rest can contain letters，numbers and dashes(-)。",
			},
			"partition_num": {
				Type:        schema.TypeInt,
				Required:    true,
			...
			"ip_white_list": {
				...
				Optional:    true,
				Description: "Ip whitelist，quota 限制，必填 when enableWhileList=true。",
			...
			"retention": {
				...
				Description:  "消息 can be selected. Retention time，unit is ms，the current minimum 值 is 60000ms。",
			},
			...
			"clean_up_policy": {
				...
				Optional:    true,
				Default:     "delete",
				Description: "Clear log policy，log clear 模式，默认为 `delete`. `delete`: logs are deleted according to the storage time. `compact`: logs are compressed according to the 键 `compact，delete`: logs are compressed according to the 键 and will be deleted according to the storage time。",
			},
			...
			"message_storage_location": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "消息 storage location。",
			},
			"create_time": {
				Type:        schema.TypeString,
				Computed:    true,
			...
			"forward_status": {
				Type:        schema.TypeInt,
				Computed:    true,
				Description: "Data backup cos 状态 有效值：`0`，`1`. `1`: do not open data backup，`0`: open data backup。",
			},
			"segment_bytes": {
				Type:        schema.TypeInt,
				Computed:    true,
