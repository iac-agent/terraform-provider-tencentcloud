	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"resource_type": {
				...
				Default:     "TOPIC",
				ForceNew:    true,
				Description: "ACL 资源类型 Valid values are `UNKNOWN`，`ANY`，`TOPIC`，`GROUP`，`CLUSTER`，`TRANSACTIONAL_ID`. and `TOPIC` by default. Currently，only `TOPIC` is available，and other fields will be 用于future ACLs compatible with open-来源 Kafka。",
			},
			"resource_name": {
				Type:        schema.TypeString,
				Required:    true,
			...
			"operation_type": {
				...
				Required:    true,
				ForceNew:    true,
				Description: "ACL operation 模式 有效值：`UNKNOWN`，`ANY`，`ALL`，`READ`，`WRITE`，`CREATE`，`DELETE`，`ALTER`，`DESCRIBE`，`CLUSTER_ACTION`，`DESCRIBE_CONFIGS` and `ALTER_CONFIGS`。",
			},
			"permission_type": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     "ALLOW",
				ForceNew:    true,
				Description: "ACL permission 类型 有效值：`UNKNOWN`，`ANY`，`DENY`，`ALLOW`. and `ALLOW` by default. Currently，CKafka supports `ALLOW` (equivalent to allow list)，and other fields will be 用于future ACLs compatible with open-来源 Kafka。",
			},
			"host": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     "*",
				ForceNew:    true,
				Description: "The 默认为 *，which means that any 主机 can access it. Support filling in IP or network segment，and support `;`separation。",
			},
			"principal": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     "*",
				ForceNew:    true,
				Description: "用户 list. The 默认值为 `*`，which means that any 用户 can access. The current 用户 can only be one included in the 用户 list. For example: `root` meaning 用户 root can access。",
			},
		},
	}
}
