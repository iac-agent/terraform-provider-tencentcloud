	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"resource_type": {
				...
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "Acl 资源类型，currently only supports Topic，enumeration 值 list{Topic}。",
			},
			...
			"pattern_type": {
				...
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "Match 类型，currently supports prefix matching and preset strategy，enumeration 值 list{PREFIXED/PRESET}。",
			},
			...
			"rule_list": {
				...
				Description: "列表 configured ACL rules。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"permission_type": {
							...
							Required:    true,
							ForceNew:    true,
							Description: "permission 类型，(Deny|Allow)。",
						},
						"host": {
							Type:        schema.TypeString,
							Required:    true,
							ForceNew:    true,
							Description: "The 默认为 *，which means that any 主机 can be accessed. Currently，ckafka does not support 主机 and ip network segment。",
						},
						"principal": {
							Type:        schema.TypeString,
							Required:    true,
							ForceNew:    true,
							Description: "用户 list，the 默认为 用户:，which means that any 用户 can access，and the current 用户 can only be the 用户 included in the 用户 list. The input 格式 needs to be prefixed with [用户:]. For example，用户 A is passed in as 用户:A。",
						},
					},
				},
			},
			...
			"pattern": {
				...
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "A 值 representing the prefix that the prefix matches。",
			},

			"is_applied": {
				Optional:    true,
				Type:        schema.TypeInt,
				Description: "是否preset ACL rule is applied to the newly added topic。",
			},
		},
	}
}
...
func resourceTencentCloudCkafkaAclRuleCreate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_ckafka_acl_rule.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseCkafkaClient().CreateAclRule(request)
		if e != nil {
	...
	d.SetId(instanceId + tccommon.FILED_SP + ruleName)
...
func resourceTencentCloudCkafkaAclRuleRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_ckafka_acl_rule.read")()
	defer tccommon.InconsistentCheck(d, meta)()
...
func resourceTencentCloudCkafkaAclRuleUpdate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_ckafka_acl_rule.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseCkafkaClient().ModifyAclRule(request)
		if e != nil {
			return tccommon.RetryError(e)
		} else {
			log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
...
func resourceTencentCloudCkafkaAclRuleDelete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_ckafka_acl_rule.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
