	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"version_id": {
				...
				ForceNew:    true,
				Required:    true,
				Description: "版本 of secret. The maximum length is 64 bytes. The version_id can only contain English letters，numbers，underscore and hyphen '-'. The first character must be a letter or number。",
			},
			"secret_binary": {
				Type:         schema.TypeString,
				Optional:     true,
				ExactlyOneOf: []string{"secret_string"},
				Description:  "The base64-encoded binary secret. secret_binary and secret_string must be set only one，and the maximum support is 4096 bytes. When secret 状态 is `已禁用`，this field will not update anymore。",
			},
			"secret_string": {
				Type:         schema.TypeString,
				Optional:     true,
				ExactlyOneOf: []string{"secret_binary"},
				Description:  "The string text of secret. secret_binary and secret_string must be set only one，and the maximum support is 4096 bytes. When secret 状态 is `已禁用`，this field will not update anymore。",
			},
		},
	}
}
