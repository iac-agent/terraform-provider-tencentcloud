...
func NatGatewaySnatPara() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"resource_type": {
			...
			Required:    true,
			ForceNew:    true,
			Description: "Resource 类型. 有效 值:" + NAT_GATEWAY_TYPE_SUBNET + ", " + NAT_GATEWAY_TYPE_NETWORK_INTERFACE + ".",
		},
		"subnet_id": {
			Type:        schema.TypeString,
			Optional:    true,
			ForceNew:    true,
			Description: "Subnet 实例 ID, 必填 当 `resource_type` 是" + NAT_GATEWAY_TYPE_SUBNET + ".",
		},
		"subnet_cidr_block": {
			Type:        schema.TypeString,
			Optional:    true,
			ForceNew:    true,
			Description: "IPv4 CIDR 的 子网, 必填 当 `resource_type` 是" + NAT_GATEWAY_TYPE_SUBNET + ".",
		},
		"instance_id": {
			Type:        schema.TypeString,
			Optional:    true,
			ForceNew:    true,
			Description: "实例 ID, 必填 当 `resource_type` 是" + NAT_GATEWAY_TYPE_NETWORK_INTERFACE + ".",
		},
		"instance_private_ip_addr": {
			Type:        schema.TypeString,
			Optional:    true,
			ForceNew:    true,
			Description: "Private IPs 的 实例's primary ENI, 必填 当 `resource_type` 是" + NAT_GATEWAY_TYPE_NETWORK_INTERFACE + ".",
		},
		"public_ip_addr": {
...
func resourceTencentCloudNatGatewaySnatCreate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_nat_gateway_snat.create")()
...
func resourceTencentCloudNatGatewaySnatRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_nat_gateway_snat.read")()
...
func resourceTencentCloudNatGatewaySnatDelete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_nat_gateway_snat.delete")()
