...
func ResourceTencentCloudSecurityGroupRuleSet() *schema.Resource {
	ruleElem := map[string]*schema.Schema{
		...
		"cidr_block": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "An IP 地址 network or CIDR segment. NOTE: `cidr_block`，`ipv6_cidr_block`，`source_security_id` and `address_template_*` are exclusive and cannot be set in the same time; One of `cidr_block`，`ipv6_cidr_block`，`source_security_id` and `address_template_*` must be set。",
		},
		"ipv6_cidr_block": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "An IPV6 地址 network or CIDR segment，and conflict with `source_security_id` and `address_template_*`. NOTE: One of `cidr_block`，`ipv6_cidr_block`，`source_security_id` and `address_template_*` must be set。",
		},
		"source_security_id": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "ID nested security group，and conflicts with `cidr_block` and `address_template_*`. NOTE: One of `cidr_block`，`ipv6_cidr_block`，`source_security_id` and `address_template_*` must be set。",
		},
		"address_template_id": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "指定Address 模板 ID like `ipm-xxxxxxxx`，conflict with `source_security_id` and `cidr_block`. NOTE: One of `cidr_block`，`ipv6_cidr_block`，`source_security_id` and `address_template_*` must be set。",
		},
		"address_template_group": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "指定Group ID 地址 template like `ipmg-xxxxxxxx`，conflict with `source_security_id` and `cidr_block`. NOTE: One of `cidr_block`，`ipv6_cidr_block`，`source_security_id` and `address_template_*` must be set。",
		},
		"service_template_id": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "指定Protocol 模板 ID like `ppm-xxxxxxxx`，conflict with `协议` and `端口`。",
		},
		"service_template_group": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "指定Group ID 协议 模板 ID like `ppmg-xxxxxxxx`，conflict with `协议` and `端口`。",
		},
		"protocol": {
			Type:        schema.TypeString,
			Optional:    true,
			Computed:    true,
			Description: "类型 IP 协议 有效值：`TCP`，`UDP`，`ICMP`，`ICMPv6` and `ALL`. 默认为 all types 协议，and conflicts with `service_template_*`。",
		},
		"port": {
			Type:        schema.TypeString,
			Optional:    true,
			Computed:    true,
			Description: "Range of the 端口 The available 值 can be `all`，a single 端口，or a 端口 range. E.g. `80`，`80,90`，`80-90` or `all`. Note: If the `协议` 值 is set to `ALL`，the `端口` 值 also needs to be set to `all`. 默认为 all ports，and conflicts with `service_template_*`。",
		},
		"policy_index": {
			Type:        schema.TypeInt,
			Computed:    true,
			Description: "The security group rule 索引 number，whose 值 dynamically changes with changes in security group rules。",
		},
	}
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"ingress": {
				...
				Optional:    true,
				Description: "列表 ingress rule. NOTE: this block is ordered，the first rule has the highest 优先级",
				Elem:        &schema.Resource{Schema: ruleElem},
			},
			"egress": {
				Type:        schema.TypeList,
				Optional:    true,
				Description: "列表 egress rule. NOTE: this block is ordered，the first rule has the highest 优先级",
				Elem:        &schema.Resource{Schema: ruleElem},
			},
			"version": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "Security policies 版本，auto increment for every update。",
			},
		},
	}
}
