	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"name": {
				...
				Optional:    true,
				Description: "名称 地址 template group to query。",
			},
			"id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "Id of the 地址 template group to query。",
			},
			"result_output_file": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"group_list": {
				...
				Computed:    true,
				Description: "Information 列表 the dedicated 地址 template groups。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"id": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "Id of the 地址 template group。",
						},
						"name": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "名称 地址 template group。",
						},
						"template_ids": {
							Type: schema.TypeList,
							Elem: &schema.Schema{
							...
							Computed:    true,
							Description: "ID set of the 地址 template。",
						},
					},
				},
			},
...
func dataSourceTencentCloudAddressTemplateGroupsRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_address_template_groups.read")()
	...
	if ok && output.(string) != "" {
		if e := tccommon.WriteToFile(output.(string), templateGroupList); e != nil {
			return e
		}
	}
