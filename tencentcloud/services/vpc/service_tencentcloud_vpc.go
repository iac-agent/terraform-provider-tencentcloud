...
func (me *VpcService) CreateServiceTemplate(ctx context.Context, name string, services []interface{}, key *string, value *string) (templateId string, errRet error) {
	logId := tccommon.GetLogId(ctx)
	...
	// Add tags if key and value are provided
	if key != nil && value != nil {
		tag := &vpc.Tag{
			Key:   key,
			Value: value,
		}
		request.Tags = []*vpc.Tag{tag}
	}
...
func (me *VpcService) DescribeVpcPeerConnectManagerById(ctx context.Context, peeringConnectionId string) (PeerConnectManager *vpc.PeerConnection, errRet error) {
	...
	PeerConnectManager = response.Response.PeerConnectionSet[0]
	return
}
