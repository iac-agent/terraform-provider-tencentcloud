	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"vpc_id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "The 私有网络 ID for NAT Gateway。",
			},
			"name": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidateStringLengthInRange(1, 60),
				Description:  "The 名称 for NAT Gateway。",
			},
			"state": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "NAT gateway 状态 有效值：0，1，2. 0: Running，1: Unavailable，2: Be in arrears and out of service。",
			},
			"max_concurrent": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "The upper 限制 of concurrent connection of NAT gateway，for example: `1000000`，`3000000`，`10000000`。",
			},
			"bandwidth": {
				Type:        schema.TypeInt,
				Optional:    true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"state": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "NAT gateway 状态，`0`: Running，`1`: Unavailable，`2`: Be in arrears and out of service。",
						},
						"max_concurrent": {
							Type:        schema.TypeInt,
							Computed:    true,
						...
						"create_time": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "The 创建时间 of the NAT gateway。",
						},
					},
				},
			},
