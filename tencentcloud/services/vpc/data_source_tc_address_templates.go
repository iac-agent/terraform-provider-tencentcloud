	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"name": {
				...
				Optional:    true,
				Description: "名称 地址 template 到 查询。",
			},
			"id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "ID 地址 template 到 查询。",
			},
			"result_output_file": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"template_list": {
				...
				Computed:    true,
				Description: "Information 列表 dedicated 地址 templates。",
...
func dataSourceTencentCloudAddressTemplatesRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_address_templates.read")()
