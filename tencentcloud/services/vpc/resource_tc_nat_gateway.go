	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"max_concurrent": {
				...
				Description:  "The upper 限制 of concurrent connection of NAT gateway. 有效值：`1000000`，`3000000`，`10000000`. 默认为 `1000000`. When the 值 of parameter `nat_product_version` is 2，which is the standard NAT 类型，this parameter does not need to be filled in and 默认为 `2000000`。",
			},
			"bandwidth": {
				Type:         schema.TypeInt,
				Optional:     true,
				...
				Description:  "The maximum public network output bandwidth of NAT gateway (unit: Mbps). 有效值：`20`，`50`，`100`，`200`，`500`，`1000`，`2000`，`5000`. 默认为 `100`. When the 值 of parameter `nat_product_version` is 2，which is the standard NAT 类型，this parameter does not need to be filled in and 默认为 `5000`。",
			},
			"assigned_eip_set": {
				Type:     schema.TypeSet,
				Required: true,
				...
				MinItems:    1,
				Description: "EIP IP 地址 set bound to the gateway. The 值 of at least 1 and at most 10 if do not apply for a whitelist。",
			},
			"zone": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "The availability 可用区，such as `ap-guangzhou-3`。",
			},
			"subnet_id": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"stock_public_ip_addresses_bandwidth_out": {
				...
				Computed:    true,
				ForceNew:    true,
				Description: "The elastic public IP bandwidth 值 (unit: Mbps) for binding NAT gateway. When this parameter is not filled in，it 默认为 the bandwidth 值 of the elastic public IP，and for some users，it 默认为 the bandwidth 限制 of the elastic public IP of that 用户 类型",
			},
			"tags": {
				Type:        schema.TypeMap,
				Optional:    true,
