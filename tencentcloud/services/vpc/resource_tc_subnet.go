	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"vpc_id": {
				...
				Required:    true,
				ForceNew:    true,
				Description: "ID VPC 到 是 associated。",
			},
			"availability_zone": {
				Type:        schema.TypeString,
				Required:    true,
				ForceNew:    true,
				Description: "availability 可用区 within 其中 子网 should 是 创建。",
			},
			"name": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidateStringLengthInRange(1, 60),
				Description:  "名称 子网 到 是 创建。",
			},
			"cidr_block": {
				Type:         schema.TypeString,
				Required:     true,
			...
			"is_multicast": {
				...
				Optional:    true,
				Default:     true,
				Description: "表示是否multicast 是 已启用 默认值为 'true'. We recommend disabling these features 如果 they 是 不 applicable 到 your 环境。",
			},
			"cdc_id": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"route_table_id": {
				...
				Optional:    true,
				Computed:    true,
				Description: "ID routing 表 到 其中 子网 should 是 associated。",
			},
			"tags": {
				Type:        schema.TypeMap,
				Optional:    true,
