	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"vpc_id": {
				...
				Required:    true,
				ForceNew:    true,
				Description: "ID VPC to be associated。",
			},
			"availability_zone": {
				Type:        schema.TypeString,
				Required:    true,
				ForceNew:    true,
				Description: "The availability 可用区 within which the subnet should be created。",
			},
			"name": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidateStringLengthInRange(1, 60),
				Description:  "名称 subnet to be created。",
			},
			"cidr_block": {
				Type:         schema.TypeString,
				Required:     true,
			...
			"is_multicast": {
				...
				Optional:    true,
				Default:     true,
				Description: "表示是否multicast is 已启用 The 默认值为 'true'. We recommend disabling these features if they are not applicable to your environment。",
			},
			"cdc_id": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"route_table_id": {
				...
				Optional:    true,
				Computed:    true,
				Description: "ID routing table to which the subnet should be associated。",
			},
			"tags": {
				Type:        schema.TypeMap,
				Optional:    true,
			...
			"create_time": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "创建时间 of subnet resource。",
			},
		},
	}
}
