	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"check_associate": {
				...
				Computed:    true,
				ForceNew:    true,
				Description: "是否enable verification of the submachine or network card range during HAVIP drift. Not 已启用 by default。",
			},
			"vip": {
				Type:         schema.TypeString,
				Optional:     true,
			...
			"state": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "State of the HA VIP Valid 值: `AVAILABLE`，`UNBIND`。",
			},
			"instance_id": {
				Type:        schema.TypeString,
				Computed:    true,
