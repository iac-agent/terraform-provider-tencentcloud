	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"redis_id": {
				...
				ForceNew:    true,
				Required:    true,
				Description: "ID redis 实例 到 其中 策略 将 是 applied。",
			},
			"backup_time": {
				Type:     schema.TypeString,
				Required: true,
				...
				Description: "Specifys what 时间 备份 操作 should take place. And 时间间隔 should 是 一个 hour。",
			},
			"backup_period": {
				Type:        schema.TypeSet,
				Optional:    true,
				...
				Description: "Specifys 其中 day 备份 操作 should take place. 有效值：`Monday`，`Tuesday`，`Wednesday`，`Thursday`，`Friday`，`Saturday` 和 `Sunday`。",
			},
		},
	}
}
