	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"region": {
				...
				Optional:    true,
				Description: "名称 地域 如果 此 值 是 不 集合， 当前 地域 getting 从 provider's 配置 将 是 使用。",
			},
			"type_id": {
				Type:         schema.TypeInt,
				Optional:     true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"type_id": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "实例类型 Which redis 类型 支持 在 此 可用区",
						},
						"type": {
							Type:        schema.TypeString,
							Computed:    true,
						...
						"version": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "版本 描述 可用 实例. 可能的值：`Redis 3.2`，`Redis 4.0`。",
						},
						"mem_sizes": {
							...
							Description: "内存 卷 的 可用 实例(在 MB)。",
						},
						"shard_memories": {
							Type:        schema.TypeList,
							Description: "内存 卷 列表 可用 实例 分片(在 MB)。",
							Computed:    true,
							Elem:        &schema.Schema{Type: schema.TypeInt},
						},
						"redis_shard_nums": {
							...
							Computed:    true,
							Elem:        &schema.Schema{Type: schema.TypeInt},
							Description: "support numbers 的 实例 分片。",
						},
						"redis_replicas_nums": {
							Type:        schema.TypeList,
							Computed:    true,
							Elem:        &schema.Schema{Type: schema.TypeInt},
							Description: "support numbers 的 实例 copies。",
						},
					},
				},
			},
...
func dataSourceTencentRedisZoneConfigRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_redis_zone_config.read")()
	...
	if output, ok := d.GetOk("result_output_file"); ok && output.(string) != "" {
		...
		if err := tccommon.WriteToFile(output.(string), allZonesConfigs); err != nil {
