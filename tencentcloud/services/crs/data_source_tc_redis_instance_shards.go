	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"filter_slave": {
				...
				Description: "是否filter out slave 信息。",
			},

			"instance_shards": {
				Computed:    true,
				Type:        schema.TypeList,
				Description: "实例 分片 列表 信息。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"runid": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "节点 ID 实例 runtime。",
						},
						"connected": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "Service 状态: 0-down;1-在。",
						},
					},
				},
			},
...
func dataSourceTencentCloudRedisInstanceShardsRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_redis_instance_shards.read")()
