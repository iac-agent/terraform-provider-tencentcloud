...
func LayerContent() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"cos_bucket_name": {
			...
			Optional:    true,
			Description: "Cos 存储桶 名称 SCF layer，such as `cos-1234567890`，conflict with `zip_file`。",
		},
		"cos_object_name": {
			Type:         schema.TypeString,
			Optional:     true,
			ValidateFunc: tccommon.ValidateStringSuffix(".zip", ".jar"),
			Description:  "Cos object 名称 SCF layer，should have suffix `.zip` or `.jar`，conflict with `zip_file`。",
		},
		"cos_bucket_region": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "Cos 存储桶 地域 of the SCF layer，conflict with `zip_file`。",
		},
		// zip upload
		"zip_file": {
			Type:        schema.TypeString,
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"content": {
				...
				Required:    true,
				MaxItems:    1,
				Description: "The 来源 代码 of layer。",
			...
			"create_time": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "The 创建时间 of layer。",
			},
			"code_sha_256": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "The 代码 类型 layer。",
			},
			"location": {
				Type:        schema.TypeString,
				Computed:    true,
			...
			"status": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "The current 状态 layer。",
			},
		},
	}
}
...
func resourceTencentCloudScfLayerCreate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_scf_layer.create")()
	logId := tccommon.GetLogId(tccommon.ContextNil)
	var (
		scfService   = ScfService{client: meta.(tccommon.ProviderMeta).GetAPIV3Conn()}
	...
	d.SetId(*layerName + tccommon.FILED_SP + helper.Int64ToStr(*layerVersion))
...
func resourceTencentCloudScfLayerRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_scf_layer.read")()
...
func resourceTencentCloudScfLayerDelete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_scf_layer.delete")()
