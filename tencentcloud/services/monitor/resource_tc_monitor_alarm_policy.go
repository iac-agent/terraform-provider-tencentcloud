...
func AlarmPolicyRule() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"notice_frequency": {
			...
			Optional:    true,
			Computed:    true,
			Description: "Alarm interval （秒）。",
		},
		"is_power_notice": {
			Type:        schema.TypeInt,
			Optional:    true,
			Computed:    true,
			Description: "是否alarm frequency increases exponentially。",
		},
		"filter": {
			...
			Elem: &schema.Resource{
				Schema: map[string]*schema.Schema{
					"type": {
						Type:        schema.TypeString,
						Optional:    true,
						Description: "Filter 条件类型 有效值：DIMENSION (uses dimensions for filtering)。",
					},
					"dimensions": {
						Type:        schema.TypeString,
						Optional:    true,
		...
		"description": {
			...
			Optional:    true,
			Computed:    true,
			Description: "Metric display 名称，which is used in the output parameter。",
		},
		"unit": {
			Type:        schema.TypeString,
			Optional:    true,
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"monitor_type": {
				...
				Required:    true,
				ForceNew:    true,
				Description: "类型 monitor。",
			},
			"namespace": {
				Type:        schema.TypeString,
				Required:    true,
				ForceNew:    true,
				Description: "类型 alarm。",
			},
			"remark": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidateStringLengthInRange(0, 100),
				Description:  "The 备注 of policy group。",
			},
			"enable": {
				Type:        schema.TypeInt,
				Default:     1,
			...
			"project_id": {
				...
				ForceNew:    true,
				Default:     -1,
				Description: "项目 ID For products with different projects，a 值 other than -1 must be passed in。",
			},
			//nolint:misspell
			"conditon_template_id": {
				...
				Optional:    true,
				ForceNew:    true,
				Description: "ID trigger condition template。",
			},
			"conditions": {
				Type:        schema.TypeList,
				Optional:    true,
				MaxItems:    1,
				Computed:    true,
				Description: "A 列表 metric trigger condition。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"is_union_rule": {
							Type:         schema.TypeInt,
			...
			"event_conditions": {
				...
				Optional:    true,
				Computed:    true,
				Description: "A 列表 event trigger condition。",
				Elem: &schema.Resource{
					Schema: AlarmPolicyRule(),
				},
			},
			"notice_ids": {
				Type:        schema.TypeList,
				Optional:    true,
				Description: "列表 notification rule IDs。",
				Elem: &schema.Schema{
					Type:        schema.TypeString,
					Description: "ID notification rule to be queried。",
				},
			},
			"trigger_tasks": {
				Type:        schema.TypeList,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"type": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Triggered task 类型",
						},
						"task_config": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Configuration information in JSON 格式",
						},
					},
				},
			},
			"policy_tag": {
				...
				Optional:    true,
				ForceNew:    true,
				Description: "Policy 标签 to bind object。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"key": {
							Type:        schema.TypeString,
			...
			"group_by": {
				Type:        schema.TypeSet,
				Optional:    true,
				Description: "Aggregate dimension list，指定which dimension keys to use for group by。",
				Elem: &schema.Schema{
					Type: schema.TypeString,
				},
			},
			...
			"create_time": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "The alarm policy 创建时间。",
			},
			"update_time": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "The alarm policy 更新时间。",
			},
		},
	}
}
