	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"resource_id": {
				...
				Required:    true,
				ForceNew:    true,
				Description: "ID resource that the layer 7 rule works for。",
			},
			"resource_type": {
				Type:         schema.TypeString,
				Required:     true,
				...
				ForceNew:     true,
				Description:  "类型 resource that the layer 7 rule works for，valid 值 is `bgpip`。",
			},
			"domain": {
				Type:         schema.TypeString,
				Required:     true,
				...
				Description:  "域名 that the layer 7 rule works for. Valid string length ranges from 0 to 80。",
			},
			"protocol": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidateAllowedStringValue(DAYU_L7_RULE_PROTOCOL),
				Description:  "协议 of the rule. 有效值：`http`，`https`。",
			},
			"name": {
				Type:        schema.TypeString,
				Required:    true,
			...
			"source_list": {
				...
				Elem: &schema.Schema{
					Type:        schema.TypeString,
					Description: "来源 IP or 域名，valid 格式 of ip is like `1.1.1.1:60` or `1.1.1.1` and valid 格式 of 主机 来源 is like `abc.com` or `abc.com:80`。",
				},
				MinItems:    1,
				MaxItems:    16,
				Description: "来源 列表 the rule，it can be a set of ip sources or a set of 域名 sources. The 数量 items ranges from 1 to 16。",
			},
			"ssl_id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "SSL ID，when the `协议` is `https`，the field should be set with valid SSL id。",
			},
			"health_check_switch": {
				Type:        schema.TypeBool,
				Optional:    true,
				Computed:    true,
				Description: "表示是否health check is 已启用 The 默认为 `false`。",
			},
			"health_check_interval": {
				...
				Description:  "Interval time of health check. Valid 值 ranges: [10~60]sec. The 默认为 15 sec。",
			},
			...
			"health_check_code": {
				...
				Description:  "HTTP 状态 代码 The 默认为 `26`. Valid 值 ranges: [1~31]. `1` means the 返回值 '1xx' is health. `2` means the 返回值 '2xx' is health. `4` means the 返回值 '3xx' is health. `8` means the 返回值 '4xx' is health. `16` means the 返回值 '5xx' is health. If you want multiple return codes to indicate health，need to add the corresponding values。",
			},
			"health_check_path": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "路径 of health check. The 默认为 `/`。",
			},
			"health_check_method": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "Methods of health check. The 默认为 'HEAD'，the available 值 are 'HEAD' and 'GET'。",
			},
			//computed
			"rule_id": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "ID layer 7 rule。",
			},
			"status": {
				Type:        schema.TypeInt,
				Computed:    true,
				Description: "状态 rule. `0` for create/modify success，`2` for create/modify fail，`3` for delete success，`5` for delete failed，`6` for waiting to be created/modified，`7` for waiting to be deleted and 8 for waiting to get SSL ID。",
			},
		},
	}
}
