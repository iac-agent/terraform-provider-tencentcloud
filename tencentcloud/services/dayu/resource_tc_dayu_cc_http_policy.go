	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"resource_type": {
				...
				ForceNew:     true,
				Description:  "类型 resource that the CC self-define http policy works for，valid values are `bgpip`，`bgp`，`bgp-multip` and `net`。",
			},
			"name": {
				Type:         schema.TypeString,
				Required:     true,
				...
				Description:  "名称 CC self-define http policy. Length should between 1 and 20。",
			},
			"smode": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "Match 模式，and valid values are `matching`，`speedlimit`. Note: the speed 限制 类型 CC self-define policy can only set one。",
			},
			"frequency": {
				Type:         schema.TypeInt,
				Optional:     true,
				...
				Description:  "Max frequency per minute，only valid when `smode` is `speedlimit`，the valid 值 ranges from 1 to 10000。",
			},
			"action": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "操作 模式，only valid when `smode` is `matching`. Valid values are `alg` and `drop`。",
			},
			"switch": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"skey": {
							...
							Description:  "键 of the rule. 有效值：`主机`，`cgi`，`ua`，`referer`。",
						},
						"operator": {
							Type:         schema.TypeString,
							Optional:     true,
							...
							Description:  "操作者 of the rule. 有效值：`include`，`not_include`，`equal`。",
						},
						"value": {
							Type:         schema.TypeString,
							Optional:     true,
							...
							Description:  "Rule 值，then length should be less than 31 bytes。",
						},
					},
				},
				Description: "Rule 列表 the CC self-define http policy， only valid when `smode` is `matching`。",
			},
			"ip": {
				Type:         schema.TypeString,
				Computed:     true,
