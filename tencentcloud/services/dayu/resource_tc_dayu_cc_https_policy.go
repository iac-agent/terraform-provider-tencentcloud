	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"resource_type": {
				...
				ForceNew:     true,
				Description:  "类型 resource that the CC self-define https policy works for，valid 值 is `bgpip`。",
			},
			"name": {
				Type:         schema.TypeString,
				Required:     true,
				...
				Description:  "名称 CC self-define https policy. Length should between 1 and 20。",
			},
			"domain": {
				Type:        schema.TypeString,
				Required:    true,
				ForceNew:    true,
				Description: "域名 that the CC self-define https policy works for，only valid when `协议` is `https`。",
			},
			"rule_id": {
				Type:        schema.TypeString,
				Required:    true,
				ForceNew:    true,
				Description: "Rule ID 域名 that the CC self-define https policy works for，only valid when `协议` is `https`。",
			},
			"switch": {
			...
			"action": {
				...
				Description:  "操作 模式 Valid values are `alg` and `drop`。",
			},
			"rule_list": {
				Type:     schema.TypeSet,
				Required: true,
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"skey": {
							...
							Description:  "键 of the rule. Valid values are `cgi`，`ua` and `referer`。",
						},
						"operator": {
							Type:         schema.TypeString,
							Required:     true,
							ValidateFunc: tccommon.ValidateAllowedStringValue(DAYU_CC_POLICY_CHECK_OP_HTTPS),
							Description:  "操作者 of the rule. Valid values are `include` and `equal`。",
						},
						"value": {
							Type:         schema.TypeString,
							Required:     true,
