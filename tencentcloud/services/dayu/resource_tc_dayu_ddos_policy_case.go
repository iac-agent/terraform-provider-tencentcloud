	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"resource_type": {
				...
				ForceNew:     true,
				Description:  "类型 resource that the DDoS policy case works for. 有效值：`bgpip`，`bgp` and `bgp-multip`。",
			},
			"name": {
				Type:         schema.TypeString,
				Required:     true,
				...
				Description:  "名称 DDoS policy case. Length should between 1 and 64。",
			},
			"platform_types": {
				Type: schema.TypeSet,
				Elem: &schema.Schema{
					...
					Description:  "Platform of the DDoS policy case. 有效值：`PC`，`MOBILE`，`TV` and `SERVER`。",
				},
				Required:    true,
			...
			"app_protocols": {
				...
				Elem: &schema.Schema{
					...
					Description:  "App 协议 of the DDoS policy case. 有效值：`tcp`，`udp`，`icmp` and `all`。",
				},
				Required:    true,
				Description: "App 协议 set of the DDoS policy case。",
			},
			"app_type": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidateAllowedStringValue(DAYU_APP_TYPE), //to see the max
				Description:  "App 类型 DDoS policy case. 有效值：`WEB`，`GAME`，`APP` and `OTHER`。",
			},
			"tcp_start_port": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidatePort,
				Description:  "Start 端口 of the TCP service. Valid 值 ranges: (0~65535)。",
			},
			"tcp_end_port": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidatePort,
				Description:  "End 端口 of the TCP service. Valid 值 ranges: (0~65535). It must be greater than `tcp_start_port`。",
			},
			"udp_start_port": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidatePort,
				Description:  "Start 端口 of the UDP service. Valid 值 ranges: (0~65535)。",
			},
			"udp_end_port": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidatePort,
				Description:  "End 端口 of the UDP service. Valid 值 ranges: (0~65535). It must be greater than `udp_start_port`。",
			},
			"has_abroad": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidateAllowedStringValue(DAYU_BOOL_FLAG),
				Description:  "Indicate 是否service involves overseas or not. 有效值：`no` and `yes`。",
			},
			"has_initiate_tcp": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidateAllowedStringValue(DAYU_BOOL_FLAG),
				Description:  "Indicate 是否service actively initiates TCP requests or not. 有效值：`no` and `yes`。",
			},
			"has_initiate_udp": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidateAllowedStringValue(DAYU_BOOL_FLAG),
				Description:  "Indicate 是否actively initiate UDP requests or not. 有效值：`no` and `yes`。",
			},
			"has_vpn": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidateAllowedStringValue(DAYU_BOOL_FLAG),
				Description:  "Indicate 是否service involves VPN service or not. 有效值：`no` and `yes`。",
			},
			"peer_tcp_port": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidatePort,
				Description:  "The 端口 that actively initiates TCP requests. Valid 值 ranges: (1~65535)。",
			},
			"peer_udp_port": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidatePort,
				Description:  "The 端口 that actively initiates UDP requests. Valid 值 ranges: (1~65535)。",
			},
			"tcp_footprint": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidateStringLengthInRange(1, 512),
				Description:  "The fixed 签名 of TCP 协议 load，valid 值 length is range from 1 to 512。",
			},
			"udp_footprint": {
				Type:         schema.TypeString,
				Optional:     true,
			...
			"min_tcp_package_len": {
				...
				Description:  "The 最小长度TCP 消息 package，valid 值 length should be greater than 0 and less than 1500。",
			},
			"max_tcp_package_len": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidateStringLengthInRange(1, 1499),
				Description:  "The max length of TCP 消息 package，valid 值 length should be greater than 0 and less than 1500. It should be greater than `min_tcp_package_len`。",
			},
			"min_udp_package_len": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidateStringLengthInRange(1, 1499),
				Description:  "The 最小长度UDP 消息 package，valid 值 length should be greater than 0 and less than 1500。",
			},
			"max_udp_package_len": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidateStringLengthInRange(1, 1499),
				Description:  "The max length of UDP 消息 package，valid 值 length should be greater than 0 and less than 1500. It should be greater than `min_udp_package_len`。",
			},
			//computed
			"create_time": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "创建时间 of the DDoS policy case。",
			},
			"scene_id": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "ID DDoS policy case。",
			},
		},
	}
}
