	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"ddos_threshold": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "DDoS cleaning threshold，值[0，60，80，100，150，200，250，300，400，500，700，1000]; When the 值 is set to 0，it means that the 默认值为 adopted。",
			},
			"ddos_level": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "Protection class，值 [`low`，`middle`，`high`]。",
			},
			"black_white_ips": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"action": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "操作，可选 values: drop，transmit，forward。",
						},
						"d_port_start": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "The destination 端口 starts，and the 值 range is 0~65535。",
						},
						"d_port_end": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "The destination 端口 ends，and the 值 range is 0~65535。",
						},
						"s_port_start": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "The 来源 端口 starts，and the 值 range is 0~65535。",
						},
						"s_port_end": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "The 来源 端口 ends，and the acceptable 值 ranges from 0 to 65535。",
						},
						"priority": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Policy 优先级，the lower the number，the higher the 级别，the higher the rule matches，taking a 值 of 1-1000.Note: This field may return null，indicating that a valid 值 could not be retrieved。",
						},
						"forward_protocol": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "协议 类型，desirable values tcp，udp，all。",
						},
					},
				},
				Description: "端口 ACL policy for DDoS protection。",
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"drop_icmp": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "ICMP block，值 [0 (block off)，1 (block on)]。",
						},
						"drop_tcp": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "TCP block，值 [0 (block off)，1 (block on)]。",
						},
						"drop_udp": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "UDP block，值 [0 (block off)，1 (block on)]。",
						},
						"drop_other": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Other block，值 [0 (block off)，1 (block on)]。",
						},
					},
				},
				Description: "协议 block configuration for DDoS protection。",
			},
			"water_print_config": {
				Type:     schema.TypeList,
				Optional: true,
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"offset": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Watermark 偏移量，取值范围：[0-100]。",
						},
						"open_status": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "是否为enabled，值 [0 (manual open)，1 (immediate operation)]。",
						},
						"listeners": {
							Type:     schema.TypeList,
							Required: true,
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"frontend_port": {
										Type:        schema.TypeInt,
										Required:    true,
										Description: "Lower 限制 of forwarding listening 端口 Values: [1-65535]。",
									},
									"forward_protocol": {
										Type:        schema.TypeString,
										Required:    true,
										Description: "Forwarding 协议，值 [TCP，UDP]。",
									},
									"frontend_port_end": {
										Type:        schema.TypeInt,
										Required:    true,
										Description: "Upper 限制 of forwarding listening 端口 Values: [1-65535]。",
									},
								},
							},
							Description: "列表 forwarding listeners to which the watermark belongs。",
						},
						"verify": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Watermark check 模式，值 [`checkall`(normal 模式)，`shortfpcheckall`(simplified 模式)]。",
						},
					},
				},
				Description: "Water print 配置",
			},
			"ddos_connect_limit": {
				Type:     schema.TypeList,
				MaxItems: 1,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"sd_new_limit": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "The 限制 on the 数量 news per second based on 来源 IP + destination IP。",
						},
						"sd_conn_limit": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Concurrent connection control based on 来源 IP + destination IP。",
						},
						"dst_new_limit": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "限制 on the 数量 news per second based on the destination IP。",
						},
						"dst_conn_limit": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Concurrent connection control based on destination IP+ destination 端口",
						},
						"bad_conn_threshold": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Based on connection suppression trigger threshold，值 range [0,4294967295]。",
						},
						"syn_rate": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Anomalous connection detection condition，percentage of syn ack，值 range [0,100]。",
						},
						"syn_limit": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Anomaly connection detection condition，syn threshold，值 range [0,100]。",
						},
						"conn_timeout": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Abnormal connection detection condition，connection timeout，值 range [0,65535]。",
						},
						"null_conn_enable": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Abnormal connection detection conditions，empty connection guard switch，值 range[0,1]。",
						},
					},
				},
			...
			"ddos_ai": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "AI protection switch，take the 值 [`on`，`off`]。",
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"action": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Block 操作，take the 值 [`drop`，`trans`]。",
						},
						"area_list": {
							Type:        schema.TypeList,
						...
						"region_type": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "可用区 类型，值 [oversea (overseas),china (domestic),customized (custom 地域)]。",
						},
					},
				},
			...
			"ddos_speed_limit_config": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"protocol_list": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "IP 协议 numbers，take the 值[ ALL (all protocols),TCP (tcp 协议),UDP (udp 协议),SMP (smp 协议),1; 2-100 (custom 协议 number range，up to 8)]。",
						},
						"dst_port_list": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "列表 端口 ranges，up to 8，multiple; Separated，the range is represented with -; this 端口 range must be filled in; fill in the style 1:0-65535，style 2:80; 443; 1000-2000。",
						},
						"mode": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Speed 限制 模式，take the 值 [1 (speed 限制 based on 来源 IP),2 (speed 限制 based on destination 端口)]。",
						},
						"packet_rate": {
							Type:        schema.TypeInt,
							Required:    true,
				...
				Description: "Access speed 限制 configuration for DDoS protection。",
			},
			"packet_filters": {
				Type:     schema.TypeList,
				Optional: true,
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"action": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "操作，take the 值 [drop,transmit,drop_black (discard and black out),drop_rst (Interception),drop_black_rst (intercept and block),forward]。",
						},
						"protocol": {
							Type:        schema.TypeString,
							Required:    true,
						...
						"s_port_start": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Start the 来源 端口，take the 值 0~65535。",
						},
						"s_port_end": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "End 来源 端口，take the 值 1~65535，must be greater than or equal to the starting 来源 端口",
						},
						"d_port_start": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "From the destination 端口，take the 值 0~65535。",
						},
						"d_port_end": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "The end destination 端口，take the 值 1~65535，which must be greater than or equal to the starting destination 端口",
						},
						"pktlen_min": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Minimum 消息 length，1-1500。",
						},
						"pktlen_max": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "The maximum 消息 length，taken from 1 to 1500，must be greater than or equal to the minimum 消息 length。",
						},
						"str": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Detect values，键 strings or regular expressions，take the 值 [When the detection 类型 is sunday，please fill in the string or hexadecimal bytecode，for example \x313233 corresponds to the hexadecimal bytecode of the string `123`;When the detection 类型 is pcre，please fill in the regular expression string;]。",
						},
						"str2": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "The second detection 值，the 键 string or regular expression，takes the 值 [When the detection 类型 is sunday，please fill in the string or hexadecimal bytecode，for example \x313233 corresponds to the hexadecimal bytecode of the string `123`;When the detection 类型 is pcre，please fill in the regular expression string;]。",
						},
						"match_logic": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "When there is a second detection condition，the and/or relationship with the first detection condition，takes the 值 [And (and relationship),none (fill in this 值 when there is no second detection condition)]。",
						},
						"match_type": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Detection 类型，值 [sunday (keyword),pcre (regular expression)]。",
						},
						"match_type2": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "The second 类型 detection，takes the 值 [sunday (keyword),pcre (regular expression)]。",
						},
						"match_begin": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Detect position，take the 值 [begin_l3 (IP header),begin_l4 (TCP/UDP header),begin_l5 (T load)，no_match (mismatch)]。",
						},
						"match_begin2": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "The second detection position. take the 值 [begin_l3 (IP header),begin_l4 (TCP/UDP header),begin_l5 (T load)，no_match (mismatch)]。",
						},
						"depth": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Detection depth from the detection position，值 [0,1500]。",
						},
						"depth2": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Second detection depth starting from the second detection position，值 [0,1500]。",
						},
						"offset": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "偏移量 from detection position，值 range [0，Depth]。",
						},
						"offset2": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "偏移量 from the second detection position，值 range [0,Depth2]。",
						},
						"is_not": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "是否include the detected 值，take the 值 [0 (included),1 (not included)]。",
						},
						"is_not2": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "是否second detection 包含detected 值，the 值 [0 (included),1 (not included)]。",
						},
					},
				},
