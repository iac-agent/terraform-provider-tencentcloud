	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"ddos_threshold": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "DDoS cleaning 阈值，值[0，60，80，100，150，200，250，300，400，500，700，1000]; 当 值 是 集合 到 0，它 表示 该 默认值为 adopted。",
			},
			"ddos_level": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"black_white_ips": {
				...
				Description: "DDoS-protected IP blacklist 和 whitelist。",
			},
			"acls": {
				Type:     schema.TypeList,
				Optional: true,
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"action": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "操作，可选 值: drop，transmit，forward。",
						},
						"d_port_start": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "destination 端口 starts，和 值 范围 是 0~65535。",
						},
						"d_port_end": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "destination 端口 结束，和 值 范围 是 0~65535。",
						},
						"s_port_start": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "来源 端口 starts，和 值 范围 是 0~65535。",
						},
						"s_port_end": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "来源 端口 结束，和 acceptable 值 ranges 从 0 到 65535。",
						},
						"priority": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Policy 优先级， lower 数量， higher 级别， higher 规则 matches，taking 值 的 1-1000.注意: 此 字段 可能 返回 null，indicating 该 有效 值 could 不 是 retrieved。",
						},
						"forward_protocol": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "协议 类型，desirable 值 tcp，udp，all。",
						},
					},
				},
				Description: "端口 ACL 策略 对于 DDoS protection。",
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"drop_icmp": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "ICMP block，值 [0 (block 关闭)，1 (block 在)]。",
						},
						"drop_tcp": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "TCP block，值 [0 (block 关闭)，1 (block 在)]。",
						},
						"drop_udp": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "UDP block，值 [0 (block 关闭)，1 (block 在)]。",
						},
						"drop_other": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Other block，值 [0 (block 关闭)，1 (block 在)]。",
						},
					},
				},
				Description: "协议 block 配置 对于 DDoS protection。",
			},
			"water_print_config": {
				Type:     schema.TypeList,
				Optional: true,
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"frontend_port": {
										Type:        schema.TypeInt,
										Required:    true,
										Description: "Lower 限制 的 forwarding listening 端口 Values: [1-65535]。",
									},
									"forward_protocol": {
										Type:        schema.TypeString,
										Required:    true,
									...
									"frontend_port_end": {
										Type:        schema.TypeInt,
										Required:    true,
										Description: "Upper 限制 的 forwarding listening 端口 Values: [1-65535]。",
									},
								},
							},
							Description: "列表 forwarding listeners 到 其中 水印 belongs。",
						},
						"verify": {
							Type:        schema.TypeString,
							Required:    true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"sd_new_limit": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "限制 在 数量 news per second based 在 来源 IP + destination IP。",
						},
						"sd_conn_limit": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Concurrent 连接 control based 在 来源 IP + destination IP。",
						},
						"dst_new_limit": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "限制 在 数量 news per second based 在 destination IP。",
						},
						"dst_conn_limit": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Concurrent 连接 control based 在 destination IP+ destination 端口",
						},
						"bad_conn_threshold": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Based 在 连接 suppression 触发器 阈值，值 范围 [0,4294967295]。",
						},
						"syn_rate": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Anomalous 连接 detection condition，percentage 的 syn ack，值 范围 [0,100]。",
						},
						"syn_limit": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Anomaly 连接 detection condition，syn 阈值，值 范围 [0,100]。",
						},
						"conn_timeout": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Abnormal 连接 detection condition，连接 超时，值 范围 [0,65535]。",
						},
						"null_conn_enable": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Abnormal 连接 detection conditions，空 连接 guard switch，值 范围[0,1]。",
						},
					},
				},
				Description: "DDoS 连接 suppression options。",
			},
			"ddos_ai": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "AI protection switch，take 值 [`在`，`关闭`]。",
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"action": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Block 操作，take 值 [`drop`，`trans`]。",
						},
						"area_list": {
							Type:        schema.TypeList,
							Elem:        &schema.Schema{Type: schema.TypeInt},
							Required:    true,
							Description: "当 RegionType 是 customized， AreaList 必须 是 filled 在，和 最大 的 128 必须 是 filled 在。",
						},
						"region_type": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "可用区 类型，值 [oversea (overseas),china (domestic),customized (自定义 地域)]。",
						},
					},
				},
				Description: "DDoS-protected area block 配置。",
			},
			"ddos_speed_limit_config": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"protocol_list": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "IP 协议 numbers，take 值[ ALL (all protocols),TCP (tcp 协议),UDP (udp 协议),SMP (smp 协议),1; 2-100 (自定义 协议 数量 范围，up 到 8)]。",
						},
						"dst_port_list": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "列表 端口 ranges，up 到 8，多个; Separated， 范围 是 represented 使用 -; 此 端口 范围 必须 是 filled 在; fill 在 style 1:0-65535，style 2:80; 443; 1000-2000。",
						},
						"mode": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Speed 限制 模式，take 值 [1 (speed 限制 based 在 来源 IP),2 (speed 限制 based 在 destination 端口)]。",
						},
						"packet_rate": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Packet 速率 pps。",
						},
						"bandwidth": {
							Type:        schema.TypeInt,
							Required:    true,
				...
				Description: "Access speed 限制 配置 对于 DDoS protection。",
			},
			"packet_filters": {
				Type:     schema.TypeList,
				Optional: true,
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"action": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "操作，take 值 [drop,transmit,drop_black (discard 和 black out),drop_rst (Interception),drop_black_rst (intercept 和 block),forward]。",
						},
						"protocol": {
							Type:        schema.TypeString,
							Required:    true,
						...
						"s_port_start": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Start 来源 端口，take 值 0~65535。",
						},
						"s_port_end": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "End 来源 端口，take 值 1~65535，必须 是 greater 比 或 equal 到 starting 来源 端口",
						},
						"d_port_start": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "From destination 端口，take 值 0~65535。",
						},
						"d_port_end": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "end destination 端口，take 值 1~65535，其中 必须 是 greater 比 或 equal 到 starting destination 端口",
						},
						"pktlen_min": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Minimum 消息 长度，1-1500。",
						},
						"pktlen_max": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "最大 消息 长度，taken 从 1 到 1500，必须 是 greater 比 或 equal 到 最小 消息 长度。",
						},
						"str": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Detect 值，键 strings 或 regular expressions，take 值 [当 detection 类型 是 sunday，please fill 在 字符串 或 hexadecimal bytecode，对于 示例 \x313233 corresponds 到 hexadecimal bytecode 的 字符串 `123`;当 detection 类型 是 pcre，please fill 在 regular expression 字符串;]。",
						},
						"str2": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "second detection 值， 键 字符串 或 regular expression，takes 值 [当 detection 类型 是 sunday，please fill 在 字符串 或 hexadecimal bytecode，对于 示例 \x313233 corresponds 到 hexadecimal bytecode 的 字符串 `123`;当 detection 类型 是 pcre，please fill 在 regular expression 字符串;]。",
						},
						"match_logic": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "当 there 是 second detection condition， 和/或 relationship 使用 first detection condition，takes 值 [And (和 relationship),none (fill 在 此 值 当 there 是 无 second detection condition)]。",
						},
						"match_type": {
							Type:        schema.TypeString,
							Required:    true,
						...
						"match_type2": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "second 类型 detection，takes 值 [sunday (keyword),pcre (regular expression)]。",
						},
						"match_begin": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Detect position，take 值 [begin_l3 (IP 头部),begin_l4 (TCP/UDP 头部),begin_l5 (T load)，no_match (mismatch)]。",
						},
						"match_begin2": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "second detection position. take 值 [begin_l3 (IP 头部),begin_l4 (TCP/UDP 头部),begin_l5 (T load)，no_match (mismatch)]。",
						},
						"depth": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Detection depth 从 detection position，值 [0,1500]。",
						},
						"depth2": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Second detection depth starting 从 second detection position，值 [0,1500]。",
						},
						"offset": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "偏移量 从 detection position，值 范围 [0，Depth]。",
						},
						"offset2": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "偏移量 从 second detection position，值 范围 [0,Depth2]。",
						},
						"is_not": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "是否include detected 值，take 值 [0 (included),1 (不 included)]。",
						},
						"is_not2": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "是否second detection 包含detected 值， 值 [0 (included),1 (不 included)]。",
						},
					},
				},
				Description: "Feature filtering 规则 对于 DDoS protection。",
			},
		},
	}
}
