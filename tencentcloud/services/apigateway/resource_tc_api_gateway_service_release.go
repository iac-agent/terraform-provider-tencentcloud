	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"service_id": {
				...
				Required:    true,
				ForceNew:    true,
				Description: "ID API gateway service。",
			},
			"environment_name": {
				Type:         schema.TypeString,
				Required:     true,
				...
				Description:  "API gateway service environment 名称 to be released. 有效值：`test`，`prepub`，`release`。",
			},
			"release_desc": {
				Type:        schema.TypeString,
				Required:    true,
				ForceNew:    true,
				Description: "This release 描述 API gateway service。",
			},
			"release_version": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "The release 版本",
			},
		},
	}
}
