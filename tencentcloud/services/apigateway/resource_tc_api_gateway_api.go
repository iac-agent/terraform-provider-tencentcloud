	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"service_id": {
				...
				Required:    true,
				ForceNew:    true,
				Description: "唯一 ID 服务 其中 API 是 located. Refer 到 资源 `tencentcloud_api_gateway_service`。",
			},
			"api_name": {
				Type:        schema.TypeString,
				Required:    true,
			...
			"target_services": {
				Optional:    true,
				Type:        schema.TypeList,
				Description: "Target 类型 backend 资源 信息. (Internal testing stage)。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"vm_ip": {
							Type:        schema.TypeString,
			...
			"target_services_load_balance_conf": {
				Optional:    true,
				Type:        schema.TypeInt,
				Description: "Target 类型 load balancing 配置. (Internal testing stage)。",
			},
			"target_services_health_check_conf": {
				Optional:    true,
				Type:        schema.TypeList,
				MaxItems:    1,
				Description: "Target health check 配置. (Internal testing stage)。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"is_health_check": {
							Type:        schema.TypeBool,
			...
			"target_namespace_id": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "Tsf serverless 命名空间 ID. (In 内部 testing)。",
			},
			"user_type": {
				Optional:    true,
				Type:        schema.TypeString,
			...
			"eiam_app_type": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "EIAM 应用 类型",
			},
			"eiam_auth_type": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "EIAM 应用 authentication 类型 支持 AuthenticationOnly，Authentication，和 Authorization。",
			},
			"eiam_app_id": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "EIAM 应用 ID。",
			},
			"token_timeout": {
				Optional:    true,
				Type:        schema.TypeInt,
				Description: "effective 时间 的 EIAM 应用 令牌，measured （秒）， 默认为 7200 秒。",
			},
			"owner": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "所有者 的 resources。",
			},
			"release_limit": {
				Type:        schema.TypeInt,
				Optional:    true,
				Computed:    true,
				Description: "API QPS 值 Enter positive 数量 到 限制 API 查询 速率 per second `QPS`。",
			},
			"pre_limit": {
				Type:        schema.TypeInt,
				Optional:    true,
...
func resourceTencentCloudAPIGatewayAPIRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_api_gateway_api.read")()
...
func resourceTencentCloudAPIGatewayAPIUpdate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_api_gateway_api.update")()
...
func resourceTencentCloudAPIGatewayAPIDelete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_api_gateway_api.delete")()
