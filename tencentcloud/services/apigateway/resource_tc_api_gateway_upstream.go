	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"upstream_type": {
				...
				Description:  "Backend 访问 类型，取值范围：IP_PORT，K8S。",
			},
			"retries": {
				Optional:    true,
				Type:        schema.TypeInt,
			...
			"upstream_host": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "主机 请求 头部 forwarded 通过 网关 到 backend。",
			},
			"nodes": {
				Optional:    true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"weight": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "权重 [0，100]，0 是 已禁用",
						},
						"vm_instance_id": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "CVM 实例 ID注意：此字段可能返回 null，表示无法获取有效值。",
						},
						"tags": {
							Type:        schema.TypeSet,
						...
						"service_name": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "K8S 容器 服务 name注意：此字段可能返回 null，表示无法获取有效值。",
						},
						"name_space": {
							Type:        schema.TypeString,
							Optional:    true,
						...
						"cluster_id": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "ID TKE cluster注意：此字段可能返回 null，表示无法获取有效值。",
						},
						"source": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "来源 的 Node，取值范围：K8S注意：此字段可能返回 null，表示无法获取有效值。",
						},
						"unique_service_name": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Unique 服务名称 recorded internally 通过 API gateway注意：此字段可能返回 null，表示无法获取有效值。",
						},
					},
				},
			},
			"health_checker": {
				...
				Type:        schema.TypeList,
				MaxItems:    1,
				Description: "Health check 配置，currently 仅 支持 VPC channels。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"enable_active_check": {
							Type:        schema.TypeBool,
							Required:    true,
							Description: "Identify whether 活跃 health checks 是 已启用",
						},
						"enable_passive_check": {
							Type:        schema.TypeBool,
							Required:    true,
							Description: "Identify whether passive health checks 是 已启用",
						},
						"healthy_http_status": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "HTTP 状态 代码 该 决定a successful 请求 during health check。",
						},
						"unhealthy_http_status": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "HTTP 状态 代码 该 决定a failed 请求 during health check。",
						},
						"tcp_failure_threshold": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "TCP continuous 错误 阈值. 0 表示disabling TCP checking. 取值范围：[0，254]。",
						},
						"timeout_threshold": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Continuous 超时 阈值. 0 表示disabling 超时 checking. 取值范围：[0，254]。",
						},
						"http_failure_threshold": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "HTTP continuous 错误 阈值. 0 表示 HTTP checking 是 已禁用 取值范围：[0，254]。",
						},
						"active_check_http_path": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Detect requested 路径 during 活跃 health checks. 默认值 是&#39;/&#39;。",
						},
						"active_check_timeout": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "detection 请求 对于 活跃 health check timed out （秒）。 默认为 5 秒。",
						},
						"active_check_interval": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "时间间隔 对于 活跃 health checks 是 5 秒 通过 默认值。",
						},
						"unhealthy_timeout": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "automatic recovery 时间 的 abnormal 节点 状态，（秒）。 当 仅 passive checking 是 已启用，它 必须 是 集合 到 值&gt;0，otherwise passive exception 节点 将 不 是 able 到 recover. 默认为 30 秒。",
						},
					},
				},
			},
			"k8s_service": {
				Optional:    true,
				Type:        schema.TypeList,
				Description: "Configuration 的 K8S 容器 服务。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"weight": {
							Type:        schema.TypeInt,
						...
						"namespace": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Container 命名空间。",
						},
						"service_name": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "名称 容器 服务。",
						},
						"port": {
							Type:        schema.TypeInt,
							Required:    true,
...
func resourceTencentCloudAPIGatewayUpstreamCreate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_api_gateway_upstream.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseAPIGatewayClient().CreateUpstream(request)
		if e != nil {
...
func resourceTencentCloudAPIGatewayUpstreamRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_api_gateway_upstream.read")()
	defer tccommon.InconsistentCheck(d, meta)()
...
func resourceTencentCloudAPIGatewayUpstreamUpdate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_api_gateway_upstream.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseAPIGatewayClient().ModifyUpstream(request)
		if e != nil {
			return tccommon.RetryError(e)
		} else {
			log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
	...
	if d.HasChange("tags") {
		...
		resourceName := tccommon.BuildTagResourceName("apigateway", "upstreamId", tcClient.Region, upstreamId)
...
func resourceTencentCloudAPIGatewayUpstreamDelete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_api_gateway_upstream.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
