	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"access_key_type": {
				...
				Type:         schema.TypeString,
				Description:  "键 类型，支持 both auto 和 manual (自定义 keys)，默认为 auto。",
			},
			"access_key_id": {
				Optional:     true,
				Computed:     true,
				...
				Description:  "用户 defined 键 ID，必填 当 access_key_type 是 manual. 长度 是 5-50 字符，consisting 的 letters，numbers，和 English underscores。",
			},
			"access_key_secret": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "用户-defined 键 必须 是 passed 当 access_key_type 是 manual. 长度 是 10-50 字符，consisting 的 letters，numbers，和 English underscores。",
			},
			// Computed values.
			"modify_time": {
				Type:        schema.TypeString,
