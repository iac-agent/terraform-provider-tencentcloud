	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"filters": {
				...
				Description: "Filter instances using one or more criteria. Valid filter names: `db-instance-id`: Filter by instance ID (in string 格式). `db-instance-名称`: Filter by 实例名称 (in string 格式). `db-instance-ip`: Filter by instance VPC IP (in string 格式). `base-backup-id`: Filter by base backup ID (in string 格式)。",
			...
			"base_backup_set": {
				Computed:    true,
				Type:        schema.TypeList,
				Description: "列表 full backup details。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"db_instance_id": {
							Type:        schema.TypeString,
