	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"name": {
				...
				Optional:    true,
				Description: "名称 postgresql instance to be query。",
			},
			"id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "ID postgresql instance to be query。",
			},
			"project_id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "项目 ID postgresql instance to be query。",
			},
			"instance_list": {
				Type:        schema.TypeList,
				Computed:    true,
				Deprecated:  "It has been deprecated from version 1.82.64. Please use `db_instance_set` instead.",
				Description: "A 列表 postgresql instances. Each element 包含following attributes。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"id": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "ID postgresql instance。",
						},
						"name": {
							Type:        schema.TypeString,
							Computed:    true,
						...
						"charge_type": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "Pay 类型 postgresql instance。",
						},
						"auto_renew_flag": {
							Type:        schema.TypeInt,
							Computed:    true,
						...
						"storage_type": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "Storage 类型 有效值：`PHYSICAL_LOCAL_SSD` (local SSD)，`CLOUD_PREMIUM` (premium cloud disk)，`CLOUD_SSD` (cloud SSD)，`CLOUD_HSSD` (enhanced cloud SSD)。",
						},
						"memory": {
							Type:        schema.TypeInt,
							Computed:    true,
						...
						"root_user": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "Instance root 账号 名称，默认值为 `root`。",
						},
						...
						"private_access_ip": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "IP 地址 for private access。",
						},
						"private_access_port": {
							Type:        schema.TypeInt,
							Computed:    true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"region": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "Instance 地域 such as ap-guangzhou，which corresponds to the`地域` field in `RegionSet`。",
						},
						"zone": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "Instance AZ such as ap-guangzhou-3，which corresponds to the `可用区` field of `ZoneSet`。",
						},
						"vpc_id": {
							Type:        schema.TypeString,
							Computed:    true,
						...
						"db_instance_status": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "实例状态，including: `applying` (applying)，`init` (to be initialized)，`initing` (initializing)，`running` (running)，`limited run` (restricted operation)，`isolating` (isolating)，`isolated` (isolated)，`disisolating` (de-isolating)，`recycling` (recycling)，`recycled` (recycled)，`job running` (task executing)，`offline` (offline)，`migrating` (migrating)，`expanding` (scaling out)，`waitSwitch` (waiting to switch)，`switching` (switching)，`readonly` (readonly)，`restarting` (restarting)，`network changing` (network modification in progress)，`upgrading` (kernel 版本 upgrading)，`audit-switching` (审计状态 changing)，`primary-switching` (primary-secondary switching)，`offlining` (offline)，`deployment changing` (modify az)，`cloning` (restoring data)，`parameter modifying` (parameter modification in progress)，`log-switching` (log 状态 change)，`restoring` (recovering)，and `expanding` (scaling out)。",
						},
						"db_instance_memory": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "Assigned instance memory size （GB）。",
						},
						"db_instance_storage": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "Assigned instance storage capacity （GB）。",
						},
						...
						"db_major_version": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "PostgreSQL major 版本 number. 指定version information that can be obtained from the [DescribeDBVersions](https://www.tencentcloud.comom/document/api/409/89018?from_cn_redirect=1) api. currently supports major versions 10，11，12，13，14，15。",
						},
						"db_version": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "数量 the major PostgreSQL community 版本 and minor 版本，such as 12.4，which can be queried by the [DescribeDBVersions](https://intl.cloud.tencent.com/document/api/409/89018?from_cn_redirect=1) API。",
						},
						"db_kernel_version": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "PostgreSQL kernel 版本，such as v12.7_r1.8. 版本 information can be obtained from [DescribeDBVersions](https://www.tencentcloud.comom/document/api/409/89018?from_cn_redirect=1)。",
						},
						"db_instance_type": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "实例类型，which includes:\n<li>primary: primary instance </li>\n<li>readonly: read-only instance</li>\n<li>guard: disaster recovery instance</li>\n<li>temp: temporary instance</li>。",
						},
						"db_instance_version": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "Instance 版本 Valid 值: `standard` (dual-server high-availability; one-primary-one-standby)。",
						},
						"db_charset": {
							Type:        schema.TypeString,
							Computed:    true,
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									"net_type": {
										Type:        schema.TypeString,
										Computed:    true,
										Description: "Network 类型 1: inner (private network 地址)，2: public (public network 地址)。",
									},
									"status": {
										Type:        schema.TypeString,
										Computed:    true,
										Description: "Network connection 状态 有效值：`initing` (never 已启用 before)，`opened` (已启用)，`closed` (已禁用)，`opening` (enabling)，`closing` (disabling)。",
									},
									"vpc_id": {
										Type:        schema.TypeString,
										Computed:    true,
										Description: "私有网络 ID 指定ID of the virtual private cloud。",
									},
									"subnet_id": {
										Type:        schema.TypeString,
										Computed:    true,
									...
									"protocol_type": {
										Type:        schema.TypeString,
										Computed:    true,
										Description: "指定protocol 类型 to connect to the database. currently supported: postgresql，mssql (mssql compatible syntax)。",
									},
								},
							},
						},
						...
						"tag_list": {
							Type:        schema.TypeList,
							Computed:    true,
							Description: "Describes the 标签 information associated with the instance。",
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"tag_key": {
										Type:        schema.TypeString,
						...
						"read_only_instance_num": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "指定number of read-only instances。",
						},
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									"vpc_status": {
										Type:        schema.TypeInt,
										Computed:    true,
										Description: "Network 状态 有效值：1-applying，2-活跃，3-deleting，4-deleted。",
									},
								},
							},
						},
						"support_ipv6": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "是否instance supports IPv6:\n<li>`0`: no</li>\n<li>`1`: yes</li>\n默认值：0。",
						},
						"expanded_cpu": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "数量 CPU 核数 that have been elastically scaled out。",
						},
						"deletion_protection": {
							Type:        schema.TypeBool,
							Computed:    true,
							Description: "指定是否enable 删除保护 for the instance. valid values as follows:.\n-指定是否enable 删除保护 有效值：true (enable 删除保护).\n-指定是否disable 删除保护 有效值：false (disable 删除保护)。",
						},
						"root_user": {
							Type:        schema.TypeString,
							Computed:    true,
