				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"region_state": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "Availability 状态 UNAVAILABLE: unavailable，AVAILABLE: 可用。",
						},
						"support_international": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "是否resource 可以 是 purchased 在 此 地域 有效值：`0` (无)，`1` (yes).注意: 此 字段 可能 返回 `null`，indicating 该 无 有效 值 可以 是 获取。",
						},
					},
				},
			},
...
func dataSourceTencentCloudPostgresqlRegionsRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_postgresql_regions.read")()
