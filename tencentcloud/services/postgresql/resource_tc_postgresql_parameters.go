...
func resourceTencentCloudPostgresqlParametersRead(d *schema.ResourceData, meta interface{}) error {
	...
	if respData == nil {
		log.Printf("[WARN]%s resource `tencentcloud_postgresql_parameters` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		d.SetId("")
		return nil
	}
...
func resourceTencentCloudPostgresqlParametersUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if needChange {
		oldInterface, newInterface := d.GetChange("param_list")
		oldLists := oldInterface.(*schema.Set)
		newLists := newInterface.(*schema.Set)
		// remove := oldLists.Difference(newLists).List()
		add := newLists.Difference(oldLists).List()
		if len(add) > 0 {
