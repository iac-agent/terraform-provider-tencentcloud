	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"auth_client_ip": {
				Type:        schema.TypeString,
				Required:    true,
				Description: "A single IP or a single IP 地址 range such as 10.1.10.11 or 10.10.1.0/24 表示that all IPs are allowed. Please note that the IP entered should be CVM's private IP。",
			},
			"priority": {
				Type:         schema.TypeInt,
				Required:     true,
				ValidateFunc: tccommon.ValidateIntegerInRange(1, 100),
				Description:  "The 优先级 级别 of rule. Valid 值 ranges: (1~100). `1` 表示highest 优先级",
			},
			"rw_permission": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "Read and write permissions. Valid values are `RO` and `RW`. and 默认为 `RO`。",
			},
			"user_permission": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "The permissions of accessing users. Valid values are `all_squash`，`no_all_squash`，`root_squash` and `no_root_squash`. and 默认为 `root_squash`. `all_squash` 表示that all access users are mapped as anonymous users or 用户 groups; `no_all_squash` 表示that access users will match local users first and be mapped to anonymous users or 用户 groups after matching failed; `root_squash` 表示that map access root users to anonymous users or 用户 groups; `no_root_squash` 表示that access root users keep root 账号 permission。",
			},
		},
	}
}
