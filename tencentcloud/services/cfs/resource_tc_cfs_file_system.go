	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"net_interface": {
				...
				Description:  "Network 类型，Default `VPC`. 有效值：`VPC` 和 `CCN`. Select `VPC` 对于 Standard 或 High-Performance 文件 系统，和 `CCN` 对于 Standard Turbo 或 High-Performance Turbo 一个。",
			},
			"protocol": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				ForceNew:     true,
				Description:  "File 系统 协议 有效值：`NFS`，`CIFS`，`TURBO`. 如果此参数为空，`NFS` 是 使用 通过 默认值. For Turbo series，您 必须 集合 此 参数 到 `TURBO`。",
			},
			"storage_type": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				ForceNew:     true,
				Description:  "Storage 类型 文件 系统. 有效值：`SD` (Standard)，`HP` (High-Performance)，`TB` (Standard Turbo)，和 `TP` (High-Performance Turbo). 默认值：`SD`。",
			},
			"vpc_id": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"mount_ip": {
				...
				ForceNew:    true,
				Computed:    true,
				Description: "IP 的 mount point。",
			},
			"ccn_id": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "CCN 实例 ID (必填 如果 网络 类型 是 CCN)。",
			},
			"cidr_block": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "CCN IP 范围 使用 通过 CFS (必填 如果 网络 类型 是 CCN)，其中 不能 conflict 使用 other IP ranges bound 在 CCN。",
			},
			"capacity": {
				Type:        schema.TypeInt,
				Optional:    true,
				Computed:    true,
				Description: "File 系统 容量，在 GiB (必填 对于 Turbo series). For Standard Turbo， 最小 purchase 必填 是 40,960 GiB (40 TiB) 和 expansion increment 是 20,480 GiB (20 TiB). For High-Performance Turbo， 最小 purchase 必填 是 20,480 GiB (20 TiB) 和 expansion increment 是 10,240 GiB (10 TiB)。",
			},
			// computed
			"create_time": {
				Type:        schema.TypeString,
