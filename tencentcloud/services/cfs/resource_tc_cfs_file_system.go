	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"name": {
				...
				Optional:    true,
				Computed:    true,
				Description: "名称 a file system。",
			},
			"availability_zone": {
				Type:        schema.TypeString,
				Required:    true,
				ForceNew:    true,
				Description: "The available 可用区 that the file system locates at。",
			},
			"access_group_id": {
				Type:        schema.TypeString,
				Required:    true,
			...
			"net_interface": {
				...
				Description:  "Network 类型，Default `VPC`. 有效值：`VPC` and `CCN`. Select `VPC` for a Standard or High-Performance file system，and `CCN` for a Standard Turbo or High-Performance Turbo one。",
			},
			"protocol": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				ForceNew:     true,
				Description:  "File system 协议 有效值：`NFS`，`CIFS`，`TURBO`. 如果此参数为空，`NFS` is used by default. For the Turbo series，you must set this parameter to `TURBO`。",
			},
			"storage_type": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				ForceNew:     true,
				Description:  "Storage 类型 file system. 有效值：`SD` (Standard)，`HP` (High-Performance)，`TB` (Standard Turbo)，and `TP` (High-Performance Turbo). 默认值：`SD`。",
			},
			"vpc_id": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"ccn_id": {
				...
				Optional:    true,
				Computed:    true,
				Description: "CCN 实例 ID (必填 if the network 类型 is CCN)。",
			},
			"cidr_block": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "CCN IP range used by the CFS (必填 if the network 类型 is CCN)，which cannot conflict with other IP ranges bound in CCN。",
			},
			"capacity": {
				Type:        schema.TypeInt,
				Optional:    true,
				Computed:    true,
				Description: "File system capacity，in GiB (必填 for the Turbo series). For Standard Turbo，the minimum purchase 必填 is 40,960 GiB (40 TiB) and the expansion increment is 20,480 GiB (20 TiB). For High-Performance Turbo，the minimum purchase 必填 is 20,480 GiB (20 TiB) and the expansion increment is 10,240 GiB (10 TiB)。",
			},
			// computed
			"create_time": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "创建时间 of the file system。",
			},
			"fs_id": {
				Type:        schema.TypeString,
				Computed:    true,
