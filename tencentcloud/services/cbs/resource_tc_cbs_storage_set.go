	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"disk_count": {
				...
				Optional:    true,
				ForceNew:    true,
				Description: "The 数量 disks to be purchased. Default 1。",
			},
			"charge_type": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "The charge 类型 CBS instance. Support `POSTPAID_BY_HOUR` and `DEDICATED_CLUSTER_PAID`. The 默认为 `POSTPAID_BY_HOUR`。",
			},
			"availability_zone": {
				Type:        schema.TypeString,
				Required:    true,
