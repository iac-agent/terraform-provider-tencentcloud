	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"snapshot_policy_name": {
				...
				Description:  "名称 快照 策略. 最大 长度 可以 不 exceed 60 bytes。",
			},
			"repeat_weekdays": {
				...
				Description: "Periodic 快照 是 已启用 有效值：[0，1，2，3，4，5，6]. 0 表示 Sunday，1-6 表示 Monday 到 Saturday。",
			},
			"repeat_hours": {
				Type:     schema.TypeList,
				Required: true,
				...
				Description: "Trigger times 的 periodic 快照. 有效 值 ranges: (0~23). 0 表示 00:00，和 so 在。",
			},
			"retention_days": {
				Type:        schema.TypeInt,
				Optional:    true,
				Default:     7,
				Description: "Retention days 的 快照，和 默认值为 7。",
			},
		},
	}
}
