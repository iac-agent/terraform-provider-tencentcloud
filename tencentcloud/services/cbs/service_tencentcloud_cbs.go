...
func (me *CbsService) DescribeDiskConfigQuota(ctx context.Context, cvmInfo map[string]interface{}) (diskConfigSet []*cbs.DiskConfig, errRet error) {
	...
	// InquiryType: use provided value, default to INQUIRY_CVM_CONFIG for backward compatibility
	...
	// InstanceFamilies: use top-level instance_families if provided, otherwise use family from cvmInfo
	...
	if err != nil {
		errRet = err
		log.Printf("[CRITAL]%s describe cbs DiskConfigQuota failed, reason:%+v", logId, err)
		return
	}
	return
}
