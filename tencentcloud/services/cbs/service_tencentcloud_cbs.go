...
func (me *CbsService) DescribeDiskConfigQuota(ctx context.Context, cvmInfo map[string]interface{}) (diskConfigSet []*cbs.DiskConfig, errRet error) {
	...
	// Handle Zones: use zones_override if provided, otherwise derive from availability_zone
	if v, ok := cvmInfo["zones_override"]; ok {
	...
	// Handle Memory: use memory_override if provided, otherwise derive from memory_size
	if v, ok := cvmInfo["memory_override"]; ok {
	...
	// Handle DiskTypes: use disk_types_override if provided, otherwise use disk_types from cbs_filter
	if v, ok := cvmInfo["disk_types_override"]; ok {
		request.DiskTypes = helper.Strings(v.([]string))
	} else if v, ok := cvmInfo["disk_types"]; ok {
		request.DiskTypes = helper.Strings(v.([]string))
	}
