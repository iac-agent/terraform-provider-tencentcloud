...
func DataSourceTencentCloudRumFetchUrl() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudRumFetchUrlRead,
		Schema: map[string]*schema.Schema{
			...
			"status": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "HTTP protocol response 代码.",
			},

			"ret": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "Remote 请求 response 正文 代码.",
			},

			"net_status": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "网络 状态 其中 数据 报告 takes place.`0`: noraml, `1`: week, `2`: disconnection, `3`: unknown.",
			},
...
func dataSourceTencentCloudRumFetchUrlRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_rum_fetch_url.read")()
	...
	if v, ok := d.GetOk("ret"); ok {
		paramMap["Ret"] = helper.String(v.(string))
	}

	if v, ok := d.GetOk("net_status"); ok {
		paramMap["NetStatus"] = helper.String(v.(string))
	}
	...
	err := resource.Retry(tccommon.ReadRetryTimeout, func() *resource.RetryError {
		response, e := service.DescribeRumFetchUrlByFilter(ctx, paramMap)
		if e != nil {
