...
func DataSourceTencentCloudRumPerformancePage() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudRumPerformancePageRead,
		Schema: map[string]*schema.Schema{
			...
			"type": {
				...
				Description: "Query Data Type. `pagepv`: CostType 查询 通过 pagepv, `allcount`: CostType 查询 allcount, `falls`: CostType 查询 通过 falls, `samp`: CostType 查询 通过 samp, `day`: CostType 查询 通过 day, `nettype`: CostType 查询 通过 nettype, `performance`: CostType 查询 通过 performance `版本`: CostType sort 通过 版本, `平台`: CostType sort 通过 平台, `isp`: CostType sort 通过 isp, `地域`: CostType sort 通过 地域, `device`: CostType sort 通过 device, `browser`: CostType sort 通过 browser, `ext1`: CostType sort 通过 ext1, `ext2`: CostType sort 通过 ext2, `ext3`: CostType sort 通过 ext3, `ret`: CostType sort 通过 ret, `状态`: CostType sort 通过 状态, `从`: CostType sort 通过 从, `url`: CostType sort 通过 url, `env`: CostType sort 通过 env.",
			},
...
func dataSourceTencentCloudRumPerformancePageRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_rum_performance_page.read")()
	...
	err := resource.Retry(tccommon.ReadRetryTimeout, func() *resource.RetryError {
		response, e := service.DescribeRumPerformancePageByFilter(ctx, paramMap)
		if e != nil {
