	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"config_id_list": {
				...
				Description: "配置 ID 列表. Query all items 如果未传入，low 优先级",
			},

			"config_name": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "配置 名称 Exact 查询. Query all items 如果未传入。",
			},

			"config_version": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "配置 版本 Exact 查询. Query all items 如果未传入。",
			},

			"result": {
				Computed:    true,
				Type:        schema.TypeList,
				Description: "Paginated 全局 配置 列表. 注意：此字段可能返回 null，表示无法获取有效值。",
...
func dataSourceTencentCloudTsfApplicationPublicConfigRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_tsf_application_public_config.read")()
