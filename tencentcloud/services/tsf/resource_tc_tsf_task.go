	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"task_name": {
				Required:    true,
				Type:        schema.TypeString,
				Description: "任务 名称，任务 长度 64 字符。",
			},

			"task_content": {
				Required:    true,
				Type:        schema.TypeString,
				Description: "任务 内容，长度 限制 65536 bytes。",
			},
			...
			"task_type": {
				Required:    true,
				Type:        schema.TypeString,
				Description: "任务 类型，java。",
			},

			"time_out": {
				Required:    true,
				Type:        schema.TypeInt,
				Description: "任务 超时，时间 单位 ms。",
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"rule_type": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "触发器 规则 类型，Cron/Repeat。",
						},
						"expression": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Cron 类型 规则，cron expression。",
						},
						"repeat_interval": {
							Type:        schema.TypeInt,
							Optional:    true,
			...
			"retry_interval": {
				...
				Computed:    true,
				Type:        schema.TypeInt,
				Description: "重试间隔，0 &amp;lt;= RetryInterval &amp;lt;= 600000，时间 单位 ms。",
			},
			...
			"shard_arguments": {
				...
				Computed:    true,
				Type:        schema.TypeList,
				Description: "Fragmentation 参数。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"shard_key": {
							Type:        schema.TypeInt,
							Required:    true,
							Description: "Sharding 参数 KEY，整数，范围 [1,1000]。",
						},
						"shard_value": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Shard 参数 VALUE。",
						},
					},
				},
			},
			...
			"success_operator": {
				...
				Computed:    true,
				Type:        schema.TypeString,
				Description: "操作者 到 judge success 的 任务。",
			},
			...
			"success_ratio": {
				...
				Computed:    true,
				Type:        schema.TypeString,
				Description: "阈值 对于 judging success 速率 的 任务，such 作为 100。",
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"sub_task_concurrency": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "Subtask 单个-machine 并发 限制， 默认值为 2。",
						},
					},
				},
			},
			...
			"task_argument": {
				...
				Computed:    true,
				Type:        schema.TypeString,
				Description: "任务 参数， 长度 限制 是 10000 字符。",
			},

			"task_state": {
				Computed:    true,
				Type:        schema.TypeString,
				Description: "是否enable 任务，ENABLED/DISABLED。",
			},
			...
			"belong_flow_ids": {
				...
				Description: "ID 工作流 到 其中 它 belongs。",
			},

			"task_log_id": {
				Computed:    true,
				Type:        schema.TypeString,
				Description: "任务 history ID。",
			},
...
func resourceTencentCloudTsfTaskCreate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_tsf_task.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseTsfClient().CreateTask(request)
		if e != nil {
...
func resourceTencentCloudTsfTaskRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_tsf_task.read")()
	defer tccommon.InconsistentCheck(d, meta)()
...
func resourceTencentCloudTsfTaskUpdate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_tsf_task.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseTsfClient().ModifyTask(request)
		if e != nil {
			return tccommon.RetryError(e)
		} else {
			log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
...
func resourceTencentCloudTsfTaskDelete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_tsf_task.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
