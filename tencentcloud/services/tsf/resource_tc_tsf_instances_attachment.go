	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"image_id": {
				...
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "Operating 系统 镜像 ID。",
			},
			...
			"key_id": {
				...
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "Associated 键 对于 系统 reinstallation。",
			},
			...
			"sg_id": {
				...
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "Security 组 setting。",
			},
			...
			"instance_import_mode": {
				...
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "Cloud 服务器 import 模式，必填 对于 virtual machine clusters，不 必填 对于 容器 clusters. R: Reinstall TSF 系统 镜像，M: Manual installation 的 agent。",
			},
			...
			"instance_advanced_settings": {
				...
				MaxItems:    1,
				Description: "Additional 实例 参数 信息。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"mount_target": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Data 磁盘 mount point，数据 disks 是 不 mounted 通过 默认值. Data disks 使用 formatted ext3，ext4，xfs 文件 systems 将 是 mounted directly，other 文件 systems 或 unformatted 数据 disks 将 是 automatically formatted 作为 ext4 和 mounted. Please back up your 数据! 此 setting does 不 take effect 对于 云 servers 使用 无 数据 disks 或 多个 数据 disks. 注意：此字段可能返回 null，表示无法获取有效值。",
						},
						"docker_graph_path": {
							Type:        schema.TypeString,
							Required:    true,
...
func resourceTencentCloudTsfInstancesAttachmentCreate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_tsf_instances_attachment.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		result, e := meta.(tccommon.ProviderMeta).GetAPIV3Conn().UseTsfClient().AddClusterInstances(request)
		if e != nil {
...
func resourceTencentCloudTsfInstancesAttachmentRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_tsf_instances_attachment.read")()
	defer tccommon.InconsistentCheck(d, meta)()
...
func resourceTencentCloudTsfInstancesAttachmentDelete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_tsf_instances_attachment.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
