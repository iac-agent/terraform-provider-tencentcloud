...
// https://tcloud4api.woa.com/document/product/1455/934285?!preview&!document=1#4.-.E7.A4.BA.E4.BE.8B
var SystemGeneratedKey = []string{"notebook.workspace.mapping", "notebook.engineClusters", "notebook.workspaceEnvs", "ftp.file.name", "notebook.script.scriptmd5",
	"notebook.task.name", "notebook.scriptEnvs", "tenantId", "region", "bucket", "notebook.scriptId", "isGitTask", "git.repo.filepath", "notebook.source", "git.config.id"}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"task_type_id": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Task 类型 ID: `26`: OfflineSynchronization; `30`: Python; `32`: DLC SQL; `35`: Shell; `38`: Shell Form 模式; `46`: DLC Spark; `50`: DLC PySpark; `130`: Branch Node; `131`: Merged Node; `132`: Notebook; `133`: SSH; `137`: For-each; `139`: DLC Spark Streaming; `140`: Run Workflow。",
						},
						"workflow_id": {
							Type:        schema.TypeString,
							Required:    true,
						...
						"owner_uin": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Task 所有者 ID，默认为 the current 用户",
						},
						"task_description": {
							Type:        schema.TypeString,
							Optional:    true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"task_ext_configuration_list": {
							...
							Optional:    true,
							Computed:    true,
							Description: fmt.Sprintf("Task extended attribute configuration list. %v cannot be customized; they are generated by the system.", SystemGeneratedKey),
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"param_key": {
										Type:        schema.TypeString,
						...
						"task_ext_configuration_system_list": {
							Type:        schema.TypeList,
							Computed:    true,
							Description: fmt.Sprintf("Configuration %v generated by the system.", SystemGeneratedKey),
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"param_key": {
										Type:        schema.TypeString,
						...
						"yarn_queue": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Resource pool queue 名称 Obtain via DescribeProjectClusterQueues。",
						},
						"source_service_id": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "来源 data 来源 IDs，separated by semicolons (;). Obtain via DescribeDataSourceWithoutInfo。",
						},
						"source_service_type": {
							Type:        schema.TypeString,
							Optional:    true,
							Computed:    true,
							Description: "The 来源 data 来源 类型 needs to be obtained through DescribeDataSourceWithoutInfo。",
						},
						"source_service_name": {
							Type:        schema.TypeString,
							Optional:    true,
							Computed:    true,
							Description: "The 来源 数据源名称 needs to be obtained through DescribeDataSourceWithoutInfo.。",
						},
						"target_service_id": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Target data 来源 IDs，separated by semicolons (;). Obtain via DescribeDataSourceWithoutInfo。",
						},
						"target_service_type": {
							Type:        schema.TypeString,
							Optional:    true,
							Computed:    true,
							Description: "The target data 来源 类型 needs to be obtained through DescribeDataSourceWithoutInfo。",
						},
						"target_service_name": {
							Type:        schema.TypeString,
							Optional:    true,
							Computed:    true,
							Description: "The target 数据源名称，which needs to be obtained through DescribeDataSourceWithoutInfo。",
						},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"run_priority_type": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "Task scheduling 优先级 Run 优先级: `4`-High; `5`-Medium; `6`-Low. 默认值：6。",
						},
						"retry_wait_minute": {
							Type:        schema.TypeInt,
							Optional:    true,
						...
						"max_retry_number": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "Retry policy. Maximum retry attempts. 默认值：4。",
						},
						...
						"wait_execution_total_ttl_minute": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "Timeout handling policy. Total wait timeout in minutes. 默认值：-1。",
						},
						"allow_redo_type": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Rerun & backfill configuration. 默认值：ALL. ALL: rerun or backfill allowed after success or failure; FAILURE: not allowed after success，allowed after failure; NONE: not allowed after success or failure。",
						},
						"param_task_out_list": {
							Type:        schema.TypeList,
							Optional:    true,
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									"data_flow_type": {
										Type:        schema.TypeString,
										Required:    true,
										Description: "Input/output table 类型: input stream: `UPSTREAM`，output stream: `DOWNSTREAM`。",
									},
									"table_physical_id": {
										Type:        schema.TypeString,
										Required:    true,
			...
			"operate_inform": {
				...
				Optional:    true,
				Description: "Should task operations trigger notifications to downstream task owners，`true`: Notify; `false`: Do not notify; 如果未指定，the 默认为 `false`. This parameter is only used during deletion。",
			},
			"delete_mode": {
				Type:        schema.TypeBool,
				Optional:    true,
				Description: "Task deletion method: `true`: does not force downstream task instances to fail; `false`: forces downstream task instances to fail; 默认为 `false` 如果未指定，and is only used during deletion。",
			},
		},
	}
}
	...
	if needChange {
		...
		if triggerTaskConfigurationMap, ok := helper.InterfacesHeadMap(d, "trigger_task_configuration"); ok {
			...
			if v, ok := triggerTaskConfigurationMap["task_ext_configuration_system_list"]; ok {
				taskExtConfigurationSet := v.([]interface{})
				for _, item := range taskExtConfigurationSet {
					taskExtConfigurationListMap := item.(map[string]interface{})
					taskExtParameter := wedatav20250806.TaskExtParameter{}
					if v, ok := taskExtConfigurationListMap["param_key"].(string); ok && v != "" {
