	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"data_backfill_range_list": {
				...
				Required:    true,
				ForceNew:    true,
				Description: "指定data time configuration for the backfill task。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"start_date": {
							...
							Optional:    true,
							ForceNew:    true,
							Description: "Start date in yyyy-MM-dd 格式 表示start from 00:00:00 on the specified date。",
						},
						"end_date": {
							Type:        schema.TypeString,
							Optional:    true,
							ForceNew:    true,
							Description: "End date in the 格式 yyyy-MM-dd，表示ending at 23:59:59 of the specified date。",
						},
						"execution_start_time": {
							Type:        schema.TypeString,
							Optional:    true,
							ForceNew:    true,
							Description: "开始时间 of each day between [StartDate，EndDate] in HH:mm 格式 effective for tasks with a 周期 of hours or less。",
						},
						"execution_end_time": {
							Type:        schema.TypeString,
							Optional:    true,
							ForceNew:    true,
							Description: "结束时间 point between [StartDate，EndDate] in HH:mm 格式 effective for tasks with a 周期 of hours or less。",
						},
					},
				},
			},
			...
			"data_backfill_plan_name": {
				...
				ForceNew:    true,
				Computed:    true,
				Description: "Backfill plan 名称 如果为空，a string of characters is randomly generated by system。",
			},
			...
			"check_parent_type": {
				...
				ForceNew:    true,
				Computed:    true,
				Description: "Check parent task 类型 有效值：NONE (do not check ALL)，ALL (check ALL upstream parent tasks)，MAKE_SCOPE (only check in the currently selected tasks of the backfill plan). 默认值：NONE (do not check)。",
			},
			...
			"skip_event_listening": {
				...
				ForceNew:    true,
				Computed:    true,
				Description: "指定是否ignore event dependency for backfill. default true。",
			},
			...
			"redefine_self_workflow_dependency": {
				...
				ForceNew:    true,
				Computed:    true,
				Description: "Custom workflow self-dependency. 有效值：yes or no. if not configured，use the original workflow self-dependency。",
			},
			...
			"redefine_parallel_num": {
				...
				Optional:    true,
				ForceNew:    true,
				Description: "Customizes the degree of 并发 for instance running. if without configuring，use the existing self-dependent of the task。",
			},
			...
			"scheduler_resource_group_id": {
				...
				Optional:    true,
				ForceNew:    true,
				Description: "Scheduling resource group id. 如果为空，表示usage of the original task scheduling execution resource group。",
			},
			...
			"integration_resource_group_id": {
				...
				Optional:    true,
				ForceNew:    true,
				Description: "Integration task resource group id. 表示usage of the original task scheduling execution resource group 如果为空。",
			},
			...
			"redefine_param_list": {
				...
				Optional:    true,
				ForceNew:    true,
				Description: "Custom parameter. re-指定task's parameters to facilitate the execution of new logic by replenished instances。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"k": {
							Type:        schema.TypeString,
						...
						"v": {
							...
							Optional:    true,
							ForceNew:    true,
							Description: "The 值 do not pass SQL (the request will be deemed as an attack on the api). if needed，transcode the SQL with Base64 and decode it。",
						},
					},
				},
			},
			...
			"data_time_order": {
				...
				ForceNew:    true,
				Computed:    true,
				Description: "Backfill Execution 顺序 - The execution 顺序 for backfill instances based on their data time. Effective only when both conditions are met:\n\n1. Must be the same cycle task.\n\n2. 优先级 is given to dependency 顺序 If no dependencies apply，execution follows the configured 顺序\n\nValid values:\n\n-NORMAL: No specific 顺序 (default)\n\n-ORDER: Execute in chronological 顺序\n\n-REVERSE: Execute in reverse chronological 顺序",
			},
