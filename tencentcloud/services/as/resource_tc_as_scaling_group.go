	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"scaling_group_name": {
				...
				Description:  "名称 a scaling group。",
			},
			"configuration_id": {
				Type:        schema.TypeString,
				Required:    true,
			...
			"max_size": {
				...
				Description:  "最大CVM instances. Valid 值 ranges: (0~2000)。",
			},
			"min_size": {
				Type:         schema.TypeInt,
				Required:     true,
				ValidateFunc: tccommon.ValidateIntegerInRange(0, 2000),
				Description:  "最小CVM instances. Valid 值 ranges: (0~2000)。",
			},
			"vpc_id": {
				Type:        schema.TypeString,
				Required:    true,
			...
			"project_id": {
				...
				Optional:    true,
				Default:     0,
				Description: "指定to which project the scaling group belongs。",
			},
			"subnet_ids": {
			...
			"zones": {
				...
				Optional:    true,
				Elem:        &schema.Schema{Type: schema.TypeString},
				Description: "列表 available zones，for Basic network it 为必填项。",
			},
			"default_cooldown": {
				Type:        schema.TypeInt,
				Optional:    true,
				Default:     300,
				Description: "Default cooldown time in second，and 默认值为 `300`。",
			},
			"desired_capacity": {
				Type:        schema.TypeInt,
				Optional:    true,
			...
			"forward_balancer_ids": {
				...
				Description:   "列表 application load balancers，which can't be specified with `load_balancer_ids` together。",
			...
			"termination_policies": {
				...
				Computed:    true,
				MaxItems:    1,
				Description: "Available values for termination policies. 有效值：OLDEST_INSTANCE and NEWEST_INSTANCE。",
				Elem: &schema.Schema{
					Type:    schema.TypeString,
			...
			"retry_policy": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "Available values for retry policies. 有效值：IMMEDIATE_RETRY and INCREMENTAL_INTERVALS。",
			...
			"scaling_mode": {
				...
				Computed:    true,
				Optional:    true,
				Description: "表示scaling 模式 which creates and terminates instances (classic method)，or method first tries to start stopped instances (wake up stopped) to perform scaling operations. 可用值：`CLASSIC_SCALING`，`WAKE_UP_STOPPED_SCALING`. 默认值：`CLASSIC_SCALING`。",
			},
			...
			"replace_mode": {
				...
				Computed:    true,
				Optional:    true,
				Description: "Replace 模式 of unhealthy replacement service. 有效值：RECREATE: Rebuild an instance to replace the original unhealthy instance. RESET: Performing a system reinstallation on unhealthy instances to keep information such as data disks，private IP addresses，and instance IDs unchanged. The instance login settings，HostName，enhanced services，and UserData will remain consistent with the current launch configuration. 默认值：RECREATE. 注意：此字段可能返回 null，表示无法获取有效值。",
			},
			"desired_capacity_sync_with_max_min_size": {
				Type:        schema.TypeBool,
				Computed:    true,
				Optional:    true,
				Description: "The expected 数量 instances is synchronized with the maximum and minimum values. The 默认值为 `False`. This parameter is effective only in the scenario where the expected number is not passed in when modifying the scaling group interface. True: When modifying the maximum or minimum 值，if there is a conflict with the current expected number，the expected number is adjusted synchronously. For example，when modifying，if the minimum 值 2 is passed in and the current expected number is 1，the expected number is adjusted synchronously to 2; False: When modifying the maximum or minimum 值，if there is a conflict with the current expected number，an 错误信息 is displayed indicating that the modification is not allowed。",
			},
			"priority_scale_in_unhealthy": {
				Type:        schema.TypeBool,
				Optional:    true,
				Computed:    true,
				Description: "是否enable 优先级 for unhealthy instances during scale-in operations. If set to `true`，unhealthy instances will be removed first when scaling in。",
			},
			"health_check_type": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "Health check 类型 instances in a scaling group.<br><li>CVM: confirm whether an instance is healthy based on the network 状态 If the pinged instance is unreachable，the instance will be considered unhealthy. For more information，see [Instance Health Check](https://intl.cloud.tencent.com/document/product/377/8553?from_cn_redirect=1)<br><li>CLB: confirm whether an instance is healthy based on the CLB health check 状态 For more information，see [Health Check Overview](https://intl.cloud.tencent.com/document/product/214/6097?from_cn_redirect=1).<br>If the parameter is set to `CLB`，the scaling group will check both the network 状态 and the CLB health check 状态 If the network check 表示unhealthy，the `HealthStatus` field will return `UNHEALTHY`. If the CLB health check 表示unhealthy，the `HealthStatus` field will return `CLB_UNHEALTHY`. If both checks indicate unhealthy，the `HealthStatus` field will return `UNHEALTHY|CLB_UNHEALTHY`. 默认值：`CLB`。",
			},
			"lb_health_check_grace_period": {
				Type:        schema.TypeInt,
				Optional:    true,
				Computed:    true,
				Description: "Grace 周期 of the CLB health check during which the `IN_SERVICE` instances added will not be marked as `CLB_UNHEALTHY`.<br>Valid range: 0-7200，（秒）。 默认值：`0`。",
			},
			"tags": {
				Type:        schema.TypeMap,
				Optional:    true,
				Description: "标签 of a scaling group。",
			},
			...
			"instance_count": {
				Type:        schema.TypeInt,
				Computed:    true,
				Description: "Instance 数量 a scaling group。",
			},
			"create_time": {
				Type:        schema.TypeString,
				Computed:    true,
