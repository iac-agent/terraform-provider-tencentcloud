	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"honor_cooldown": {
				...
				Description: "是否check if the auto scaling group is in the cooldown 周期 默认值：false。",
			},
			...
			"trigger_source": {
				...
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "来源 that triggers the scaling policy. 有效值：API and CLOUD_MONITOR. 默认值：API. The 值 CLOUD_MONITOR is specific to the Cloud Monitor service。",
			},
		},
	}
}
