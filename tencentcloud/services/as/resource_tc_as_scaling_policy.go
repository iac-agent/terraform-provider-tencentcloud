	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"policy_name": {
				Type:        schema.TypeString,
				Required:    true,
				Description: "名称 a policy 用于define a reaction when an alarm is triggered。",
			},
			"adjustment_type": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidateAllowedStringValue(SCALING_GROUP_ADJUSTMENT_TYPE),
				Description:  "指定是否adjustment is an absolute number or a percentage of the current capacity. 有效值：`CHANGE_IN_CAPACITY`，`EXACT_CAPACITY` and `PERCENT_CHANGE_IN_CAPACITY`。",
			},
			"adjustment_value": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "Define the 数量 instances by which to scale.For `CHANGE_IN_CAPACITY` 类型 or PERCENT_CHANGE_IN_CAPACITY，a positive increment adds to the current capacity and a negative 值 removes from the current capacity. For `EXACT_CAPACITY` 类型，it defines an absolute 数量 the existing Auto Scaling group size。",
			},
			"comparison_operator": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidateAllowedStringValue(SCALING_GROUP_COMPARISON_OPERATOR),
				Description:  "比较运算符 有效值：`GREATER_THAN`，`GREATER_THAN_OR_EQUAL_TO`，`LESS_THAN`，`LESS_THAN_OR_EQUAL_TO`，`EQUAL_TO` and `NOT_EQUAL_TO`。",
			},
			"metric_name": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: tccommon.ValidateAllowedStringValue(SCALING_GROUP_METRIC_NAME),
				Description:  "名称 an indicator. 有效值：`CPU_UTILIZATION`，`MEM_UTILIZATION`，`LAN_TRAFFIC_OUT`，`LAN_TRAFFIC_IN`，`WAN_TRAFFIC_OUT` and `WAN_TRAFFIC_IN`。",
			},
			"threshold": {
				Type:        schema.TypeInt,
				Optional:    true,
			...
			"period": {
				...
				Description:  "Time 周期 in second. 有效值：`60` and `300`。",
			},
			"continuous_time": {
				Type:         schema.TypeInt,
				Optional:     true,
				ValidateFunc: tccommon.ValidateIntegerInRange(1, 10),
				Description:  "重试次数 Valid 值 ranges: (1~10)。",
			},
			"statistic": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "Statistic types. 有效值：`AVERAGE`，`MAXIMUM` and `MINIMUM`. 默认为 `AVERAGE`。",
			},
			"cooldown": {
				Type:        schema.TypeInt,
				Optional:    true,
				Computed:    true,
				Description: "Cooldwon time in second. 默认为 `300`。",
			},
			...
			"policy_type": {
				...
				ForceNew:    true,
				Computed:    true,
				Description: "Alarm triggering policy 类型，the default 类型 is SIMPLE. 取值范围：SIMPLE: Simple policy; TARGET_TRACKING: Target tracking policy。",
			},
			"predefined_metric_type": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "Predefined monitoring items，applicable only to target tracking policies，and 必填 in target tracking policy scenarios. 取值范围：ASG_AVG_CPU_UTILIZATION: Average CPU utilization; ASG_AVG_LAN_TRAFFIC_OUT: Average intranet outbound bandwidth; ASG_AVG_LAN_TRAFFIC_IN: Average intranet inbound bandwidth; ASG_AVG_WAN_TRAFFIC_OUT: Average internet outbound bandwidth; ASG_AVG_WAN_TRAFFIC_IN: Average internet inbound bandwidth。",
			},
			"target_value": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "Target 值，applicable only to target tracking strategies，and 必填 in target tracking strategy scenarios. ASG_AVG_CPU_UTILIZATION: [1，100)，单位：%; ASG_AVG_LAN_TRAFFIC_OUT: >0，单位：Mbps; ASG_AVG_LAN_TRAFFIC_IN: >0，单位：Mbps; ASG_AVG_WAN_TRAFFIC_OUT: >0，单位：Mbps; ASG_AVG_WAN_TRAFFIC_IN: >0，单位：Mbps。",
			},
			"estimated_instance_warmup": {
				Type:        schema.TypeInt,
				Optional:    true,
				Computed:    true,
				Description: "Instance warm-up time，（秒）， applicable only to target tracking strategies. 取值范围为 0-3600，with a default warm-up time of 300 seconds。",
			},
			"disable_scale_in": {
				Type:        schema.TypeBool,
				Optional:    true,
				Description: "是否disable scaling down applies only to the target tracking strategy; the 默认值为 false. 取值范围：true: The target tracking strategy only triggers scaling up; false: The target tracking strategy triggers both scaling up and scaling down。",
			},
		},
	}
}
