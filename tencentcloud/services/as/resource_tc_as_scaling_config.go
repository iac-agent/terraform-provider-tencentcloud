	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"configuration_name": {
				...
				Description:  "名称 a launch configuration。",
			},
			"image_id": {
				Type:         schema.TypeString,
				Optional:     true,
			...
			"image_family": {
				...
				Optional:     true,
				AtLeastOneOf: []string{"image_id", "image_family"},
				Description:  "Image Family 名称 Either Image ID or Image Family 名称 must be provided，but not both。",
			},
			"project_id": {
				Type:        schema.TypeInt,
				Optional:    true,
			...
			"system_disk_type": {
				...
				Description:  "类型 a CVM disk. 有效值：`CLOUD_PREMIUM` and `CLOUD_SSD`. 默认为 `CLOUD_PREMIUM`. valid when disk_type_policy is ORIGINAL。",
			},
			"system_disk_size": {
				Type:        schema.TypeInt,
				Optional:    true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"disk_type": {
							...
							Default:      SYSTEM_DISK_TYPE_CLOUD_PREMIUM,
							ValidateFunc: tccommon.ValidateAllowedStringValue(SYSTEM_DISK_ALLOW_TYPE),
							Description:  "Types of disk. 有效值：`CLOUD_PREMIUM` and `CLOUD_SSD`. valid when disk_type_policy is ORIGINAL。",
						},
						"disk_size": {
							Type:        schema.TypeInt,
							Optional:    true,
			...
			"internet_charge_type": {
				...
				Description:  "Charge types for network traffic. 有效值：`BANDWIDTH_PREPAID`，`TRAFFIC_POSTPAID_BY_HOUR` and `BANDWIDTH_PACKAGE`。",
			},
			...
			"is_keep_eip": {
				...
				Optional:    true,
				Computed:    true,
				Description: "是否delete the bound EIP when the instance is destroyed. Range of values: True: retain the EIP; False: not retain the EIP. Note that when the IPv4AddressType field 指定EIP 类型，the default behavior is not to retain the EIP. WanIP is unaffected by this field and will always be deleted with the instance. Changing this field configuration will take effect immediately for resources already bound to a scaling group。",
			},
			"password": {
				Type:          schema.TypeString,
				Optional:      true,
			...
			"keep_image_login": {
				...
				Description:   "指定是否keep original settings of a CVM image. And it can't be used with 密码 or key_ids together。",
			},
			"security_group_ids": {
				Type:        schema.TypeList,
				Optional:    true,
			...
			"enhanced_automation_tools_service": {
				Type:        schema.TypeBool,
				Optional:    true,
				Description: "To 指定是否enable cloud automation tools service。",
			},
			"user_data": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"disk_type_policy": {
				...
				Description:  "Policy of cloud disk 类型 有效值：`ORIGINAL` and `AUTOMATIC`. 默认为 `ORIGINAL`。",
			},
			"cam_role_name": {
				Type:        schema.TypeString,
				Optional:    true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"host_name": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "The 主机 名称 cloud server; dots (.) and dashes (-) cannot be used as the first and last characters of HostName，and cannot be used consecutively; Windows instances are not supported; other types (Linux，etc.) instances: the character length is [2，40]，it is allowed to support multiple dots，and there is a paragraph between the dots，and each paragraph is allowed to consist of letters (no uppercase and lowercase restrictions)，numbers and dashes (-). Pure numbers are not allowed。",
						},
						"host_name_style": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "The style of the 主机 名称 cloud server，the 值 range includes `ORIGINAL` and `UNIQUE`，the 默认为 `ORIGINAL`; `ORIGINAL`，the AS directly passes the HostName filled in the input parameter to the CVM，and the CVM may append a sequence to the HostName number，the HostName of the instance in the scaling group will conflict; `UNIQUE`，the HostName filled in as a parameter is equivalent to the 主机名 prefix，AS and CVM will expand it，and the HostName of the instance in the scaling group can be guaranteed to be unique。",
						},
					},
				},
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"instance_name_style": {
							...
							Description:  "类型 CVM 实例名称 有效值：`ORIGINAL` and `UNIQUE`. 默认为 `ORIGINAL`。",
						},
					},
				},
			},
			...
			"disaster_recover_group_ids": {
				...
				Optional:    true,
				Computed:    true,
				Description: "Placement 组 ID Only one is allowed。",
				Elem: &schema.Schema{
					Type: schema.TypeString,
				},
			},
			...
			"tags": {
				Type:        schema.TypeMap,
				Optional:    true,
				Description: "标签 of launch configuration。",
			},
			// Computed values
			"status": {
				Type:        schema.TypeString,
