	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"lifecycle_hook_name": {
				Type:        schema.TypeString,
				Required:    true,
				Description: "The 名称 lifecycle hook。",
			},
			"lifecycle_transition": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidateAllowedStringValue([]string{"INSTANCE_LAUNCHING", "INSTANCE_TERMINATING"}),
				Description:  "The instance state to which you want to attach the lifecycle hook. 有效值：`INSTANCE_LAUNCHING` and `INSTANCE_TERMINATING`。",
			},
			"default_result": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "Defines the 操作 the AS group should take when the lifecycle hook timeout elapses or if an unexpected failure occurs. 有效值：`CONTINUE` and `ABANDON`. The 默认值为 `CONTINUE`。",
			},
			"heartbeat_timeout": {
				Type:         schema.TypeInt,
				Optional:     true,
				...
				Description:  "Defines the amount of time，（秒）， that can elapse before the lifecycle hook times out. Valid 值 ranges: (30~7200). and 默认值为 `300`。",
			},
			"notification_metadata": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     "",
				Description: "包含additional information that you want to include any time AS sends a 消息 to the notification target。",
			},
			...
			"notification_queue_name": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "For CMQ_QUEUE 类型，a 名称 queue must be set。",
			},
			"notification_topic_name": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "For CMQ_TOPIC 类型，a 名称 topic must be set。",
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"command_id": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Remote 命令 ID It 为必填项 to execute a command。",
						},
						"parameters": {
							Type:        schema.TypeString,
							Optional:    true,
