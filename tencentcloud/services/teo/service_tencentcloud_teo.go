...
func (me *TeoService) DescribeTeoFunctionV4ById(ctx context.Context, zoneId string, functionId string) (ret *teo.Function, errRet error) {
	logId := tccommon.GetLogId(ctx)
	...
	for _, function := range response.Response.Functions {
		if function.FunctionId != nil && *function.FunctionId == functionId {
			ret = function
			break
		}
	}
