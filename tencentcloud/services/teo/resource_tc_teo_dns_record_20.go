...
func ResourceTencentCloudTeoDnsRecord20() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord20Create,
		Read:   resourceTencentCloudTeoDnsRecord20Read,
		Update: resourceTencentCloudTeoDnsRecord20Update,
		Delete: resourceTencentCloudTeoDnsRecord20Delete,
...
func resourceTencentCloudTeoDnsRecord20Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_20.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record 20 failed, reason:%+v", logId, err)
		return err
	}

	if response == nil || response.Response == nil || response.Response.RecordId == nil {
		return fmt.Errorf("create teo dns record 20 failed, response is nil")
	}
	...
	return resourceTencentCloudTeoDnsRecord20Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord20Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_20.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	respData, err := service.DescribeTeoDnsRecord20ById(ctx, zoneId, recordId)
	if err != nil {
		return err
	}
	if respData == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_dns_record_20` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord20Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_20.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record 20 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord20Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_20.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record 20 failed, reason:%+v", logId, err)
		return err
	}
