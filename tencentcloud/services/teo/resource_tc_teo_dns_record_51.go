...
func ResourceTencentCloudTeoDnsRecord51() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord51Create,
		Read:   resourceTencentCloudTeoDnsRecord51Read,
		Update: resourceTencentCloudTeoDnsRecord51Update,
		Delete: resourceTencentCloudTeoDnsRecord51Delete,
		...
		Schema: map[string]*schema.Schema{
			...
			"type": {
				Type:     schema.TypeString,
				Required: true,
				Description: "DNS record type. valid values are A, AAAA, MX, CNAME, TXT, NS, CAA, SRV. different record types, such as SRV and CAA records, " +
					"have different requirements for host record names and record value formats. for detailed descriptions and format examples of each record type, " +
					"please refer to: [introduction to dns record types](https://intl.cloud.tencent.com/document/product/1552/90453?from_cn_redirect=1#2f681022-91ab-4a9e-ac3d-0a6c454d954e).",
			},
			...
			"location": {
				...
				Optional:    true,
				Computed:    true,
				Description: "DNS record resolution route, not specified as default, indicates the default resolution route, which is effective for all regions. the resolution of line configuration is only applicable when the Type (DNS record type) is A, AAAA, or CNAME. the analysis of line configuration is only applicable to standard and enterprise packages. please refer to the analysis of line and corresponding code enumeration for values.",
			},
...
func resourceTencentCloudTeoDnsRecord51Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_51.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil || result.Response.RecordId == nil || *result.Response.RecordId == "" {
			return resource.NonRetryableError(fmt.Errorf("Create teo dns_record_51 failed, RecordId is nil or empty. logId: %s, d.Id(): %s.", logId, d.Id()))
		}
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns_record_51 failed, reason:%+v", logId, err)
		return err
	}
	...
	return resourceTencentCloudTeoDnsRecord51Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord51Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_51.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	respData, err := service.DescribeTeoDnsRecord51ById(ctx, zoneId, recordId)
	if err != nil {
		return err
	}
	if respData == nil {
		log.Printf("[CRUD] teo_dns_record_51 id=%s", d.Id())
		log.Printf("[WARN]%s resource `teo_dns_record_51` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		d.SetId("")
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord51Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_51.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
			...
			if result == nil || result.Response == nil {
				return resource.NonRetryableError(fmt.Errorf("Modify teo dns_record_51 failed, Response is nil."))
			}

			return nil
		})
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns_record_51 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord51Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_51.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			return resource.NonRetryableError(fmt.Errorf("Delete teo dns_record_51 failed, Response is nil."))
		}
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns_record_51 failed, reason:%+v", logId, err)
		return err
	}
