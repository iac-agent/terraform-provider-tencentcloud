...
func ResourceTencentCloudTeoDnsRecord17() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord17Create,
		Read:   resourceTencentCloudTeoDnsRecord17Read,
		Update: resourceTencentCloudTeoDnsRecord17Update,
		Delete: resourceTencentCloudTeoDnsRecord17Delete,
...
func resourceTencentCloudTeoDnsRecord17Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_17.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record 17 failed, reason:%+v", logId, err)
		return err
	}

	if response == nil || response.Response == nil || response.Response.RecordId == nil || *response.Response.RecordId == "" {
		return fmt.Errorf("create teo dns record 17 response is empty or record_id is empty")
	}
	...
	return resourceTencentCloudTeoDnsRecord17Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord17Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_17.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_dns_record_17` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord17Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_17.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record 17 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord17Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_17.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record 17 failed, reason:%+v", logId, err)
		return err
	}
