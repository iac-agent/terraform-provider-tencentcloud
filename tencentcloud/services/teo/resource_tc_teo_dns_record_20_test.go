...
// go test ./tencentcloud/services/teo/ -run "TestTeoDnsRecord20" -v -count=1 -gcflags="all=-l"

// TestTeoDnsRecord20_Create_Success tests Create calls API and sets ID
func TestTeoDnsRecord20_Create_Success(t *testing.T) {
	...
	// Mock DescribeDnsRecords for the Read call after Create
	...
	meta := newMockMeta()
	res := teo.ResourceTencentCloudTeoDnsRecord20()
...
// TestTeoDnsRecord20_Create_APIError tests Create handles API error
func TestTeoDnsRecord20_Create_APIError(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
...
// TestTeoDnsRecord20_Create_EmptyResponse tests Create handles empty RecordId
func TestTeoDnsRecord20_Create_EmptyResponse(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
	...
	err := res.Create(d, meta)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "create teo dns record 20 failed")
}

// TestTeoDnsRecord20_Read_Success tests Read retrieves DNS record data
func TestTeoDnsRecord20_Read_Success(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
...
// TestTeoDnsRecord20_Read_NotFound tests Read handles record not found
func TestTeoDnsRecord20_Read_NotFound(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
	...
	d.SetId("zone-1234567890#record-notexist")
...
// TestTeoDnsRecord20_Read_InvalidID tests Read handles broken ID
func TestTeoDnsRecord20_Read_InvalidID(t *testing.T) {
	res := teo.ResourceTencentCloudTeoDnsRecord20()
	d := schema.TestResourceDataRaw(t, res.Schema, map[string]interface{}{
		"zone_id": "zone-1234567890",
		"name":    "a.example.cn",
...
// TestTeoDnsRecord20_Update_ContentChange tests Update calls ModifyDnsRecords for content change
func TestTeoDnsRecord20_Update_ContentChange(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
	...
	// Mock DescribeDnsRecords for the Read call after Update
	patches.ApplyMethodFunc(teoClient, "DescribeDnsRecords", func(request *teov20220901.DescribeDnsRecordsRequest) (*teov20220901.DescribeDnsRecordsResponse, error) {
		resp := teov20220901.NewDescribeDnsRecordsResponse()
		resp.Response = &teov20220901.DescribeDnsRecordsResponseParams{
			TotalCount: ptrInt64(1),
...
// TestTeoDnsRecord20_Update_StatusChange tests Update calls ModifyDnsRecordsStatus for status change
func TestTeoDnsRecord20_Update_StatusChange(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
...
// TestTeoDnsRecord20_Delete_Success tests Delete removes DNS record
func TestTeoDnsRecord20_Delete_Success(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
...
// TestTeoDnsRecord20_Delete_APIError tests Delete handles API error
func TestTeoDnsRecord20_Delete_APIError(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
...
// TestTeoDnsRecord20_Schema validates schema definition
func TestTeoDnsRecord20_Schema(t *testing.T) {
	res := teo.ResourceTencentCloudTeoDnsRecord20()
