...
func ResourceTencentCloudTeoDnsRecord47() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord47Create,
		Read:   resourceTencentCloudTeoDnsRecord47Read,
		Update: resourceTencentCloudTeoDnsRecord47Update,
		Delete: resourceTencentCloudTeoDnsRecord47Delete,
...
func resourceTencentCloudTeoDnsRecord47Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_47.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns_record_47 failed, reason:%+v", logId, err)
		return err
	}

	log.Printf("[DEBUG]%s teo dns_record_47 create response, d.Id()=%s", logId, d.Id())

	if response.Response == nil || response.Response.RecordId == nil {
		return fmt.Errorf("create teo dns_record_47 failed, response is nil")
	}
	...
	return resourceTencentCloudTeoDnsRecord47Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord47Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_47.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	respData, err := service.DescribeTeoDnsRecord47ById(ctx, zoneId, recordId)
	if err != nil {
		return err
	}
	if respData == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `dns_record_47` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord47Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_47.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns_record_47 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord47Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_47.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns_record_47 failed, reason:%+v", logId, err)
		return err
	}
