...
func ResourceTencentCloudTeoDnsRecordV3() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecordV3Create,
		Read:   resourceTencentCloudTeoDnsRecordV3Read,
		Update: resourceTencentCloudTeoDnsRecordV3Update,
		Delete: resourceTencentCloudTeoDnsRecordV3Delete,
...
func resourceTencentCloudTeoDnsRecordV3Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v3.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			log.Printf("[CRUD] teo_dns_record_v3 create empty response, logId=%s, id=%s", logId, d.Id())
			return resource.NonRetryableError(fmt.Errorf("create teo_dns_record_v3 failed, Response is nil"))
		}

		log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())

		if result.Response.RecordId == nil || *result.Response.RecordId == "" {
			log.Printf("[CRUD] teo_dns_record_v3 create empty record id, logId=%s, id=%s", logId, d.Id())
			return resource.NonRetryableError(fmt.Errorf("create teo_dns_record_v3 failed, RecordId is nil"))
		}
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo_dns_record_v3 failed, reason:%+v", logId, err)
		return err
	}
	...
	return resourceTencentCloudTeoDnsRecordV3Read(d, meta)
}

func resourceTencentCloudTeoDnsRecordV3Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v3.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		log.Printf("[CRUD] teo_dns_record_v3 id=%s", d.Id())
		d.SetId("")
		return nil
	}
...
func resourceTencentCloudTeoDnsRecordV3Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v3.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
			...
			if result == nil || result.Response == nil {
				log.Printf("[CRUD] teo_dns_record_v3 update empty response, logId=%s, id=%s", logId, d.Id())
				return resource.NonRetryableError(fmt.Errorf("update teo_dns_record_v3 failed, Response is nil"))
			}
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo_dns_record_v3 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecordV3Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v3.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			log.Printf("[CRUD] teo_dns_record_v3 delete empty response, logId=%s, id=%s", logId, d.Id())
			return resource.NonRetryableError(fmt.Errorf("delete teo_dns_record_v3 failed, Response is nil"))
		}
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo_dns_record_v3 failed, reason:%+v", logId, err)
		return err
	}
