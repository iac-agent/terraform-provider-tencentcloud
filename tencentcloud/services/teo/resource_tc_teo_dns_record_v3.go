	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"status": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "DNS record resolution status. values: `enable` or `disable`.",
			},
...
func resourceTencentCloudTeoDnsRecordV3Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record v3 failed, reason:%+v", logId, err)
		return err
	}
...
func resourceTencentCloudTeoDnsRecordV3Read(d *schema.ResourceData, meta interface{}) error {
	...
	respData, err := service.DescribeTeoDnsRecordV3ById(ctx, zoneId, recordId)
	if err != nil {
		return err
	}
	if respData == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_dns_record_v3` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecordV3Update(d *schema.ResourceData, meta interface{}) error {
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record v3 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecordV3Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record v3 failed, reason:%+v", logId, err)
		return err
	}
