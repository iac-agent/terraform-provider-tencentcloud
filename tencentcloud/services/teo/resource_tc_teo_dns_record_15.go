...
func ResourceTencentCloudTeoDnsRecord15() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord15Create,
		Read:   resourceTencentCloudTeoDnsRecord15Read,
		Update: resourceTencentCloudTeoDnsRecord15Update,
		Delete: resourceTencentCloudTeoDnsRecord15Delete,
...
func resourceTencentCloudTeoDnsRecord15Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_15.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record 15 failed, reason:%+v", logId, err)
		return err
	}

	if response.Response.RecordId == nil || *response.Response.RecordId == "" {
		return fmt.Errorf("create teo dns record 15 failed, record id is empty")
	}
	...
	return resourceTencentCloudTeoDnsRecord15Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord15Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_15.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	respData, err := service.DescribeTeoDnsRecord15ById(ctx, zoneId, recordId)
	if err != nil {
		return err
	}
	if respData == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_dns_record_15` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord15Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_15.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record 15 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord15Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_15.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record 15 failed, reason:%+v", logId, err)
		return err
	}
