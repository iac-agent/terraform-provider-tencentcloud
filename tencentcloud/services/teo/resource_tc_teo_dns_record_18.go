...
func ResourceTencentCloudTeoDnsRecord18() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord18Create,
		Read:   resourceTencentCloudTeoDnsRecord18Read,
		Update: resourceTencentCloudTeoDnsRecord18Update,
		Delete: resourceTencentCloudTeoDnsRecord18Delete,
...
func resourceTencentCloudTeoDnsRecord18Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_18.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			return resource.NonRetryableError(fmt.Errorf("Create teo dns record 18 failed, Response is nil."))
		}
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record 18 failed, reason:%+v", logId, err)
		return err
	}
	...
	return resourceTencentCloudTeoDnsRecord18Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord18Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_18.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_dns_record_18` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord18Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_18.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record 18 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord18Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_18.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record 18 failed, reason:%+v", logId, err)
		return err
	}
