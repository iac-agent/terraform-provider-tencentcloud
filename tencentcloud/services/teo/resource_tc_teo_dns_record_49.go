...
func ResourceTencentCloudTeoDnsRecord49() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord49Create,
		Read:   resourceTencentCloudTeoDnsRecord49Read,
		Update: resourceTencentCloudTeoDnsRecord49Update,
		Delete: resourceTencentCloudTeoDnsRecord49Delete,
...
func resourceTencentCloudTeoDnsRecord49Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_49.create")()
	...
	reqErr := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			return resource.NonRetryableError(fmt.Errorf("Create teo dns_record_49 failed, Response is nil"))
		}
	...
	if reqErr != nil {
		log.Printf("[CRITAL]%s create teo dns_record_49 failed, reason:%+v", logId, reqErr)
		return reqErr
	}

	log.Printf("[DEBUG]%s create teo dns_record_49, logId: %s, id: %s", logId, logId, d.Id())

	if response.Response.RecordId == nil || *response.Response.RecordId == "" {
		return fmt.Errorf("Create teo dns_record_49 failed, RecordId is empty")
	}
	...
	return resourceTencentCloudTeoDnsRecord49Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord49Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_49.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		log.Printf("[CRUD] teo dns_record_49 id=%s", d.Id())
		d.SetId("")
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord49Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_49.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		reqErr := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
			...
			if result == nil || result.Response == nil {
				return resource.NonRetryableError(fmt.Errorf("Modify teo dns_record_49 failed, Response is nil"))
			}
		...
		if reqErr != nil {
			log.Printf("[CRITAL]%s update teo dns_record_49 failed, reason:%+v", logId, reqErr)
			return reqErr
		}
	}
...
func resourceTencentCloudTeoDnsRecord49Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_49.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	reqErr := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			return resource.NonRetryableError(fmt.Errorf("Delete teo dns_record_49 failed, Response is nil"))
		}
	...
	if reqErr != nil {
		log.Printf("[CRITAL]%s delete teo dns_record_49 failed, reason:%+v", logId, reqErr)
		return reqErr
	}
