	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"ips": {
				...
				Description: "When the fixed IP is enabled, this value will return the corresponding access IP; if the fixed IP is not enabled, this value is empty.",
			},

			"status": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "Layer 4 proxy instance status. `online`: Enabled; `offline`: Disabled; `progress`: Deploying; `stopping`: Disabling; `banned`: Blocked; `fail`: Deployment/Disabling failed.",
			},

			"l4_proxy_rule_count": {
				Type:        schema.TypeInt,
				Computed:    true,
				Description: "Number of forwarding rules under the Layer 4 proxy instance.",
			},
...
func resourceTencentCloudTeoL4ProxyCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo l4_proxy failed, reason:%+v", logId, err)
		return err
	}

	log.Printf("[DEBUG]%s create teo l4_proxy, d.Id()=%s, logId=%s", logId, d.Id(), logId)

	if response == nil || response.Response == nil || response.Response.ProxyId == nil || *response.Response.ProxyId == "" {
		return fmt.Errorf("create teo l4_proxy failed, empty ProxyId")
	}
...
func resourceTencentCloudTeoL4ProxyRead(d *schema.ResourceData, meta interface{}) error {
	...
	if respData == nil {
		log.Printf("[CRUD] teo l4_proxy id=%s", d.Id())
		d.SetId("")
	...
	if len(respData.Ips) > 0 {
		_ = d.Set("ips", helper.StringsInterfaces(respData.Ips))
	}
	...
	if respData.L4ProxyRuleCount != nil {
		_ = d.Set("l4_proxy_rule_count", respData.L4ProxyRuleCount)
	}
...
func resourceTencentCloudTeoL4ProxyUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo l4_proxy failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoL4ProxyDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo l4_proxy failed, reason:%+v", logId, err)
		return err
	}
