...
func ResourceTencentCloudTeoDnsRecordV7() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecordV7Create,
		Read:   resourceTencentCloudTeoDnsRecordV7Read,
		Update: resourceTencentCloudTeoDnsRecordV7Update,
		Delete: resourceTencentCloudTeoDnsRecordV7Delete,
...
func resourceTencentCloudTeoDnsRecordV7Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v7.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			return resource.NonRetryableError(fmt.Errorf("Create teo dns record v7 failed, Response is nil."))
		}
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record v7 failed, reason:%+v", logId, err)
		return err
	}
	...
	return resourceTencentCloudTeoDnsRecordV7Read(d, meta)
}

func resourceTencentCloudTeoDnsRecordV7Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v7.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_dns_record_v7` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecordV7Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v7.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record v7 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecordV7Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v7.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record v7 failed, reason:%+v", logId, err)
		return err
	}
