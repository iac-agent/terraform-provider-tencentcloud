...
func resourceTencentCloudTeoDnsRecord24Create(d *schema.ResourceData, meta interface{}) error {
	...
	if response.Response == nil || response.Response.RecordId == nil || *response.Response.RecordId == "" {
		return fmt.Errorf("create teo dns record 24 failed, RecordId is nil or empty")
	}
...
func resourceTencentCloudTeoDnsRecord24Read(d *schema.ResourceData, meta interface{}) error {
	...
	respData, err := service.DescribeTeoDnsRecord24ById(ctx, zoneId, recordId)
	if err != nil {
		return err
	}
	if respData == nil {
		log.Printf("[CRUD] teo_dns_record_24 id=%s", d.Id())
		d.SetId("")
