...
func ResourceTencentCloudTeoDnsRecordV8() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecordV8Create,
		Read:   resourceTencentCloudTeoDnsRecordV8Read,
		Update: resourceTencentCloudTeoDnsRecordV8Update,
		Delete: resourceTencentCloudTeoDnsRecordV8Delete,
...
func resourceTencentCloudTeoDnsRecordV8Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v8.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record v8 failed, reason:%+v", logId, err)
		return err
	}

	if response.Response == nil || response.Response.RecordId == nil {
		return fmt.Errorf("create teo dns record v8 response is empty")
	}
	...
	return resourceTencentCloudTeoDnsRecordV8Read(d, meta)
}

func resourceTencentCloudTeoDnsRecordV8Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v8.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	respData, err := service.DescribeTeoDnsRecordV8ById(ctx, zoneId, recordId)
	if err != nil {
		return err
	}
	if respData == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_dns_record_v8` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecordV8Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v8.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record v8 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecordV8Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_v8.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record v8 failed, reason:%+v", logId, err)
		return err
	}
