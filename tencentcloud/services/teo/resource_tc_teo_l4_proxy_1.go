...
func ResourceTencentCloudTeoL4Proxy1() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoL4Proxy1Create,
		Read:   resourceTencentCloudTeoL4Proxy1Read,
		Update: resourceTencentCloudTeoL4Proxy1Update,
		Delete: resourceTencentCloudTeoL4Proxy1Delete,
		...
		Schema: map[string]*schema.Schema{
			...
			"ips": {
				...
				Computed:    true,
				Description: "When fixed IP is enabled, this value will return the corresponding access IP; when not enabled, this value is empty.",
				Elem: &schema.Schema{
					Type: schema.TypeString,
				},
			},
			...
			"status": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "L4 proxy instance status. `online`: Enabled; `offline`: Disabled; `progress`: Deploying; `stopping`: Disabling; `banned`: Banned; `fail`: Deployment failed/Disablement failed.",
			},
			...
			"l4proxy_rule_count": {
				Type:        schema.TypeInt,
				Computed:    true,
				Description: "Number of forwarding rules under the L4 proxy instance.",
			},
...
func resourceTencentCloudTeoL4Proxy1Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_l4_proxy_1.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo l4_proxy_1 failed, reason:%+v", logId, err)
		return err
	}

	log.Printf("[DEBUG]%s create teo l4_proxy_1, d.Id()=%s", logId, d.Id())
	if response == nil || response.Response == nil {
		return fmt.Errorf("create teo l4_proxy_1 failed, empty response")
	}

	if response.Response.ProxyId == nil {
		return fmt.Errorf("create teo l4_proxy_1 failed, proxy_id is nil")
	}

	proxyId = *response.Response.ProxyId
	_ = d.Set("proxy_id", proxyId)

	if err := resourceTencentCloudTeoL4Proxy1CreatePostHandleResponse0(ctx, response); err != nil {
		return err
	}

	d.SetId(strings.Join([]string{zoneId, proxyId}, tccommon.FILED_SP))

	return resourceTencentCloudTeoL4Proxy1Read(d, meta)
}

func resourceTencentCloudTeoL4Proxy1Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_l4_proxy_1.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		log.Printf("[WARN]%s resource `teo_l4_proxy_1` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		d.SetId("")
		return nil
	}
	...
	if respData.Ips != nil {
		ips := make([]string, 0, len(respData.Ips))
		for _, ip := range respData.Ips {
			if ip != nil {
				ips = append(ips, *ip)
			}
		}
	...
	if respData.L4ProxyRuleCount != nil {
		_ = d.Set("l4proxy_rule_count", respData.L4ProxyRuleCount)
	}
...
func resourceTencentCloudTeoL4Proxy1Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_l4_proxy_1.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	immutableArgs := []string{"proxy_name", "area", "static_ip", "ddos_protection_config", "zone_id"}
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo l4_proxy_1 failed, reason:%+v", logId, err)
			return err
		}
	}

	if err := resourceTencentCloudTeoL4Proxy1UpdateOnExit(ctx); err != nil {
		return err
	}
...
func resourceTencentCloudTeoL4Proxy1Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_l4_proxy_1.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err := resourceTencentCloudTeoL4Proxy1DeletePostFillRequest0(ctx, request); err != nil {
		return err
	}
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo l4_proxy_1 failed, reason:%+v", logId, err)
		return err
	}
