...
func ResourceTencentCloudTeoDnsRecord48() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord48Create,
		Read:   resourceTencentCloudTeoDnsRecord48Read,
		Update: resourceTencentCloudTeoDnsRecord48Update,
		Delete: resourceTencentCloudTeoDnsRecord48Delete,
		...
		Schema: map[string]*schema.Schema{
			...
			"status": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "DNS record resolution status.",
			},
...
func resourceTencentCloudTeoDnsRecord48Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_48.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil || result.Response.RecordId == nil || *result.Response.RecordId == "" {
			return resource.NonRetryableError(fmt.Errorf("create teo_dns_record_48 failed, RecordId is empty"))
		}
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo_dns_record_48 failed, reason:%+v", logId, err)
		return err
	}

	log.Printf("[DEBUG]%s teo_dns_record_48 logId=%s, d.Id()=%s", logId, logId, d.Id())

	recordId = *response.Response.RecordId
	d.SetId(strings.Join([]string{zoneId, recordId}, tccommon.FILED_SP))

	return resourceTencentCloudTeoDnsRecord48Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord48Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_48.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		log.Printf("[CRUD] teo_dns_record_48 id=%s", d.Id())
		d.SetId("")
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord48Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_48.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo_dns_record_48 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord48Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_48.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo_dns_record_48 failed, reason:%+v", logId, err)
		return err
	}
