...
func ResourceTencentCloudTeoDnsRecord25() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord25Create,
		Read:   resourceTencentCloudTeoDnsRecord25Read,
		Update: resourceTencentCloudTeoDnsRecord25Update,
		Delete: resourceTencentCloudTeoDnsRecord25Delete,
...
func resourceTencentCloudTeoDnsRecord25Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_25.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record 25 failed, reason:%+v", logId, err)
		return err
	}

	if response.Response == nil || response.Response.RecordId == nil {
		return fmt.Errorf("create teo dns record 25 failed, response is nil")
	}
	...
	return resourceTencentCloudTeoDnsRecord25Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord25Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_25.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		log.Printf("[CRUD] teo_dns_record_25 id=%s", d.Id())
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_dns_record_25` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord25Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_25.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record 25 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord25Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_25.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record 25 failed, reason:%+v", logId, err)
		return err
	}
