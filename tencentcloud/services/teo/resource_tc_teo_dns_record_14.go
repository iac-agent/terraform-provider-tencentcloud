...
func ResourceTencentCloudTeoDnsRecord14() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord14Create,
		Read:   resourceTencentCloudTeoDnsRecord14Read,
		Update: resourceTencentCloudTeoDnsRecord14Update,
		Delete: resourceTencentCloudTeoDnsRecord14Delete,
...
func resourceTencentCloudTeoDnsRecord14Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_14.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil || result.Response.RecordId == nil || *result.Response.RecordId == "" {
			return resource.NonRetryableError(fmt.Errorf("create teo dns record 14 response is nil or RecordId is empty"))
		}
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record 14 failed, reason:%+v", logId, err)
		return err
	}
	...
	return resourceTencentCloudTeoDnsRecord14Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord14Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_14.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_dns_record_14` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord14Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_14.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	needChange := false
	mutableArgs := []string{"name", "content", "location", "ttl", "weight", "priority"}
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record 14 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord14Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_14.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record 14 failed, reason:%+v", logId, err)
		return err
	}
