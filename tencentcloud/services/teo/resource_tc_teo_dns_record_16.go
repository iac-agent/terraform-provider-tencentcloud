...
func ResourceTencentCloudTeoDnsRecord16() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord16Create,
		Read:   resourceTencentCloudTeoDnsRecord16Read,
		Update: resourceTencentCloudTeoDnsRecord16Update,
		Delete: resourceTencentCloudTeoDnsRecord16Delete,
...
func resourceTencentCloudTeoDnsRecord16Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_16.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record 16 failed, reason:%+v", logId, err)
		return err
	}

	if response.Response == nil || response.Response.RecordId == nil {
		return fmt.Errorf("create teo dns record 16 failed, record_id is empty")
	}
	...
	return resourceTencentCloudTeoDnsRecord16Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord16Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_16.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s read teo dns record 16 failed, reason:%+v", logId, err)
		return err
	}

	if response.Response == nil || response.Response.DnsRecords == nil || len(response.Response.DnsRecords) == 0 {
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_dns_record_16` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord16Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_16.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record 16 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord16Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_16.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record 16 failed, reason:%+v", logId, err)
		return err
	}
