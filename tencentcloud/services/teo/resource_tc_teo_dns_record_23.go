...
func ResourceTencentCloudTeoDnsRecord23() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoDnsRecord23Create,
		Read:   resourceTencentCloudTeoDnsRecord23Read,
		Update: resourceTencentCloudTeoDnsRecord23Update,
		Delete: resourceTencentCloudTeoDnsRecord23Delete,
		...
		Schema: map[string]*schema.Schema{
			...
			"type": {
				Type:        schema.TypeString,
				Required:    true,
				Description: "DNS record type. Valid values: <li>A: Points the domain name to an external IPv4 address, such as 8.8.8.8;</li><li>AAAA: Points the domain name to an external IPv6 address;</li><li>MX: Used for email servers. When there are multiple MX records, the lower the priority value, the higher the priority;</li><li>CNAME: Points the domain name to another domain name, which then resolves to the final IP address;</li><li>TXT: Identifies and describes the domain name, commonly used for domain verification and SPF records (anti-spam);</li><li>NS: If you need to delegate the subdomain to another DNS service provider for resolution, you need to add an NS record. The root domain cannot add NS records;</li><li>CAA: Specifies the CA that can issue certificates for this site;</li><li>SRV: Identifies a server using a service, commonly used in Microsoft's directory management.</li> Different record types, such as SRV and CAA records, have different requirements for host record names and record value formats. For detailed descriptions and format examples of each record type, please refer to: [Introduction to DNS Record Types](https://intl.cloud.tencent.com/document/product/1552/90453?from_cn_redirect=1#2f681022-91ab-4a9e-ac3d-0a6c454d954e).",
			},
			...
			"location": {
				...
				Computed:    true,
				Optional:    true,
				Description: "DNS record resolution route. If not specified, the default is Default, which means the default resolution route and is effective in all regions.\n\n- Resolution route configuration is only applicable when type (DNS record type) is A, AAAA, or CNAME.\n- Resolution route configuration is only applicable to Standard and Enterprise editions. For valid values, please refer to: [Resolution Routes and Corresponding Code Enumeration](https://intl.cloud.tencent.com/document/product/1552/112542?from_cn_redirect=1).",
			},
			...
			"status": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "DNS record resolution status. Valid values: <li>enable: Has taken effect;</li><li>disable: Has been disabled.</li>",
			},
...
func resourceTencentCloudTeoDnsRecord23Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_23.create")()
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record 23 failed, reason:%+v", logId, err)
		return err
	}

	if response.Response == nil || response.Response.RecordId == nil {
		return fmt.Errorf("create teo dns record 23 failed, response is nil")
	}
	...
	return resourceTencentCloudTeoDnsRecord23Read(d, meta)
}

func resourceTencentCloudTeoDnsRecord23Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_23.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `teo_dns_record_23` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		return nil
	}
...
func resourceTencentCloudTeoDnsRecord23Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_23.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record 23 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord23Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_dns_record_23.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record 23 failed, reason:%+v", logId, err)
		return err
	}
