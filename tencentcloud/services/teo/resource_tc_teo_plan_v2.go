...
func ResourceTencentCloudTeoPlanV2() *schema.Resource {
	return &schema.Resource{
		Create: ResourceTencentCloudTeoPlanV2Create,
		Read:   ResourceTencentCloudTeoPlanV2Read,
		Update: ResourceTencentCloudTeoPlanV2Update,
		Delete: ResourceTencentCloudTeoPlanV2Delete,
		...
		Schema: map[string]*schema.Schema{
			...
			"auto_use_voucher": {
				...
				Description:  "Whether to auto-use vouchers, the possible values are: `true`: yes; `false`: no. This parameter is only effective when PlanType is personal, basic, or standard. If not filled, the default value is false.",
			},
			...
			"renew_flag": {
				...
				Optional:     true,
				ValidateFunc: tccommon.ValidateAllowedStringValue([]string{"on", "off"}),
				Description:  "Auto-renewal flag for ModifyPlan, the values are: `on`: turn on automatic renewal; `off`: do not turn on automatic renewal.",
			},
			...
			"area": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "Service area, possible values are: mainland: Mainland China; overseas: Worldwide (excluding Mainland China); global: Worldwide (including Mainland China).",
			},

			"status": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "Package status, the values are: normal: normal status; expiring-soon: about to expire; expired: expired; isolated: isolated; overdue-isolated: overdue isolated.",
			},

			"pay_mode": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "Payment type, possible values: 0: post-payment; 1: pre-payment.",
			},
...
func ResourceTencentCloudTeoPlanV2Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_plan_v2.create")()
	...
	err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			return resource.NonRetryableError(fmt.Errorf("Create teo plan v2 failed, Response is nil."))
		}
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo plan v2 failed, reason:%+v", logId, err)
		return err
	}
	...
	return ResourceTencentCloudTeoPlanV2Read(d, meta)
}

func ResourceTencentCloudTeoPlanV2Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_plan_v2.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if respData == nil {
		log.Printf("[WARN]%s resource `tencentcloud_teo_plan_v2` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		d.SetId("")
		return nil
	}
	...
	if respData.PayMode != nil {
		_ = d.Set("pay_mode", helper.Int64ToStr(*respData.PayMode))
	}
...
func ResourceTencentCloudTeoPlanV2Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_plan_v2.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if d.HasChange("renew_flag") {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo plan v2 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func ResourceTencentCloudTeoPlanV2Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_plan_v2.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo plan v2 failed, reason:%+v", logId, err)
		return err
	}
