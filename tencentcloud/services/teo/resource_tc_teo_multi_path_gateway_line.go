	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"line_address": {
				Type:        schema.TypeString,
				Required:    true,
				Description: "Line address, format: ip:port.",
			},
			"proxy_id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "Layer 4 proxy instance ID, required when line_type is proxy.",
			},
			"rule_id": {
				Type:        schema.TypeString,
				Optional:    true,
...
func resourceTencentCloudTeoMultiPathGatewayLineRead(d *schema.ResourceData, meta interface{}) error {
	...
	respData, err := service.DescribeTeoMultiPathGatewayLineById(ctx, zoneId, gatewayId, lineId)
	if err != nil {
		return err
	}

	if respData == nil {
		log.Printf("[WARN]%s resource `tencentcloud_teo_multi_path_gateway_line` [%s] not found, please check if it has been deleted.\n", logId, d.Id())
		d.SetId("")
		return nil
	}

	if respData.LineId != nil {
		_ = d.Set("line_id", respData.LineId)
	}
...
func resourceTencentCloudTeoMultiPathGatewayLineUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if needChange {
		...
		reqErr := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
			...
			if result == nil || result.Response == nil {
				return resource.NonRetryableError(fmt.Errorf("Modify teo multi path gateway line failed, Response is nil."))
			}
...
func resourceTencentCloudTeoMultiPathGatewayLineDelete(d *schema.ResourceData, meta interface{}) error {
	...
	reqErr := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			return resource.NonRetryableError(fmt.Errorf("Delete teo multi path gateway line failed, Response is nil."))
		}
