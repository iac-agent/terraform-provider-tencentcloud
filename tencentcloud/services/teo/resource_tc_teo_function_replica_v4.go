...
func ResourceTencentCloudTeoFunctionReplicaV4() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudTeoFunctionReplicaV4Create,
		Read:   resourceTencentCloudTeoFunctionReplicaV4Read,
		Update: resourceTencentCloudTeoFunctionReplicaV4Update,
		Delete: resourceTencentCloudTeoFunctionReplicaV4Delete,
		...
		Schema: map[string]*schema.Schema{
			...
			"created_on": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "Edge function replica creation time.",
			},
			"modified_on": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "Edge function replica update time.",
			},
		},
	}
}

func resourceTencentCloudTeoFunctionReplicaV4Create(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_function_replica_v4.create")()
	...
	reqErr := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
		...
		if result == nil || result.Response == nil {
			return resource.NonRetryableError(fmt.Errorf("Create teo function replica v4 failed, Response is nil."))
		}

		if zoneId == "" || functionId == "" || replicaName == "" {
			return resource.NonRetryableError(fmt.Errorf("Create teo function replica v4 failed, id is empty, zoneId:%s, functionId:%s, replicaName:%s.", zoneId, functionId, replicaName))
		}
	...
	if reqErr != nil {
		log.Printf("[CRITAL]%s create teo function replica v4 failed, reason:%+v", logId, reqErr)
		return reqErr
	}

	log.Printf("[DEBUG]%s create teo function replica v4 success, logId:%s, id:%s", logId, logId, strings.Join([]string{zoneId, functionId, replicaName}, tccommon.FILED_SP))
	d.SetId(strings.Join([]string{zoneId, functionId, replicaName}, tccommon.FILED_SP))
	return resourceTencentCloudTeoFunctionReplicaV4Read(d, meta)
}

func resourceTencentCloudTeoFunctionReplicaV4Read(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_function_replica_v4.read")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if reqErr != nil {
		log.Printf("[CRITAL]%s read teo function replica v4 failed, reason:%+v", logId, reqErr)
		return reqErr
	}

	if response == nil || response.Response == nil {
		log.Printf("[CRUD] read teo function replica v4 empty response, id=%s", d.Id())
		d.SetId("")
		return nil
	}
	...
	if targetReplica == nil {
		log.Printf("[CRUD] read teo function replica v4 not found, id=%s", d.Id())
		d.SetId("")
		return nil
	}
...
func resourceTencentCloudTeoFunctionReplicaV4Update(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_function_replica_v4.update")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if needChange {
		...
		if reqErr != nil {
			log.Printf("[CRITAL]%s update teo function replica v4 failed, reason:%+v", logId, reqErr)
			return reqErr
		}
	}
...
func resourceTencentCloudTeoFunctionReplicaV4Delete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_teo_function_replica_v4.delete")()
	defer tccommon.InconsistentCheck(d, meta)()
	...
	if reqErr != nil {
		log.Printf("[CRITAL]%s delete teo function replica v4 failed, reason:%+v", logId, reqErr)
		return reqErr
	}
