...
// go test ./tencentcloud/services/teo/ -run "TestTeoDnsRecord18" -v -count=1 -gcflags="all=-l"

// TestTeoDnsRecord18_Create_Success tests Create calls API and sets ID
func TestTeoDnsRecord18_Create_Success(t *testing.T) {
	...
	patches.ApplyMethodFunc(teoClient, "CreateDnsRecordWithContext", func(_ context.Context, request *teov20220901.CreateDnsRecordRequest) (*teov20220901.CreateDnsRecordResponse, error) {
		...
		assert.Equal(t, "www", *request.Name)
	...
	meta := newMockMeta()
	res := teo.ResourceTencentCloudTeoDnsRecord18()
	...
	assert.Equal(t, "zone-12345678#record-abcdefghij", d.Id())
}

// TestTeoDnsRecord18_Create_APIError tests Create handles API error
func TestTeoDnsRecord18_Create_APIError(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
...
// TestTeoDnsRecord18_Create_NilResponse tests Create handles nil response
func TestTeoDnsRecord18_Create_NilResponse(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
...
// TestTeoDnsRecord18_Read_Success tests Read retrieves DNS record data
func TestTeoDnsRecord18_Read_Success(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
	...
	d.SetId("zone-12345678#record-abcdefghij")
...
// TestTeoDnsRecord18_Read_NotFound tests Read handles record not found
func TestTeoDnsRecord18_Read_NotFound(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
...
// TestTeoDnsRecord18_Read_BrokenId tests Read handles broken composite ID
func TestTeoDnsRecord18_Read_BrokenId(t *testing.T) {
	meta := newMockMeta()
	res := teo.ResourceTencentCloudTeoDnsRecord18()
	d := schema.TestResourceDataRaw(t, res.Schema, map[string]interface{}{
		"zone_id": "zone-12345678",
...
// TestTeoDnsRecord18_Update_Success tests Update with changed fields
func TestTeoDnsRecord18_Update_Success(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
	...
	patches.ApplyMethodFunc(teoClient, "ModifyDnsRecordsWithContext", func(_ context.Context, request *teov20220901.ModifyDnsRecordsRequest) (*teov20220901.ModifyDnsRecordsResponse, error) {
		...
		assert.Equal(t, "record-abcdefghij", *request.DnsRecords[0].RecordId)
...
// TestTeoDnsRecord18_Update_NoChange tests Update when fields are unchanged - ModifyDnsRecords may still be called
// since TestResourceDataRaw has no prior state, HasChange returns true
func TestTeoDnsRecord18_Update_NoChange(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
	...
	// DescribeDnsRecords should be called (for Read after update)
	patches.ApplyMethodFunc(teoClient, "DescribeDnsRecords", func(request *teov20220901.DescribeDnsRecordsRequest) (*teov20220901.DescribeDnsRecordsResponse, error) {
		resp := teov20220901.NewDescribeDnsRecordsResponse()
		resp.Response = &teov20220901.DescribeDnsRecordsResponseParams{
			TotalCount: ptrInt64(1),
...
// TestTeoDnsRecord18_Delete_Success tests Delete removes DNS record
func TestTeoDnsRecord18_Delete_Success(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
	...
	patches.ApplyMethodFunc(teoClient, "DeleteDnsRecordsWithContext", func(_ context.Context, request *teov20220901.DeleteDnsRecordsRequest) (*teov20220901.DeleteDnsRecordsResponse, error) {
		...
		assert.Equal(t, "record-abcdefghij", *request.RecordIds[0])
...
// TestTeoDnsRecord18_Delete_APIError tests Delete handles API error
func TestTeoDnsRecord18_Delete_APIError(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
...
// TestTeoDnsRecord18_Schema validates schema definition
func TestTeoDnsRecord18_Schema(t *testing.T) {
	res := teo.ResourceTencentCloudTeoDnsRecord18()
