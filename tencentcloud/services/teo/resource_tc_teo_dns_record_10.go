...
func resourceTencentCloudTeoDnsRecord10Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		log.Printf("[CRITAL]%s create teo dns record 10 failed, reason:%+v", logId, err)
		return err
	}

	if response.Response == nil || response.Response.RecordId == nil {
		return fmt.Errorf("Create teo dns record 10 failed, RecordId is nil.")
	}
...
func resourceTencentCloudTeoDnsRecord10Read(d *schema.ResourceData, meta interface{}) error {
	...
	respData, err := service.DescribeTeoDnsRecord10ById(ctx, zoneId, recordId)
	if err != nil {
		return err
	}
	if respData == nil {
...
func resourceTencentCloudTeoDnsRecord10Update(d *schema.ResourceData, meta interface{}) error {
	...
	if needChange {
		...
		if err != nil {
			log.Printf("[CRITAL]%s update teo dns record 10 failed, reason:%+v", logId, err)
			return err
		}
	}
...
func resourceTencentCloudTeoDnsRecord10Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		log.Printf("[CRITAL]%s delete teo dns record 10 failed, reason:%+v", logId, err)
		return err
	}
