	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"client_cert_info": {
				...
				Computed:    true,
				MaxItems:    1,
				Description: "In the edge mutual authentication scenario, this field represents the client's CA certificate, which is deployed in the EO node for the EO node to authenticate the client certificate. When used as an input parameter, it is left blank to indicate retaining the original configuration.",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"switch": {
							Type:        schema.TypeString,
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									"status": {
										Type:        schema.TypeString,
										Computed:    true,
										Description: "Certificate status. Values: `deployed`: Deployed; `processing`: Deploying; `applying`: Applying; `failed`: Application failed; `issued`: Binding failed.",
									},
								},
							},
						},
