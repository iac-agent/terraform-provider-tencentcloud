	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"bucket": {
				...
				Required:    true,
				ForceNew:    true,
				Description: "名称 a 存储桶 to use. 存储桶 格式 should be [custom 名称]-[appid]，for example `mycos-1258798060`。",
			},
			"key": {
				Type:        schema.TypeString,
				Required:    true,
				ForceNew:    true,
				Description: "The 名称 object once it is in the 存储桶",
			},
			"source": {
				Type:          schema.TypeString,
				Optional:      true,
				ConflictsWith: []string{"content"},
				Description:   "The 路径 to the 来源 file being uploaded to the 存储桶",
			},
			"content": {
				Type:          schema.TypeString,
				Optional:      true,
				ConflictsWith: []string{"source"},
				Description:   "Literal string 值 to use as the object 内容，which will be uploaded as UTF-8-encoded text。",
			},
			"acl": {
				Type:     schema.TypeString,
				Optional: true,
				...
				Description: "The canned ACL to apply. Available values include `private`，`public-read`，and `public-read-write`. 默认为 `private`。",
			},
			"cache_control": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "指定caching behavior along the request/reply chain. For further details，RFC2616 can be referred。",
			},
			"content_disposition": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"storage_class": {
				...
				Optional:    true,
				Computed:    true,
				Description: "Object storage 类型，Available values include `STANDARD_IA`，`MAZ_STANDARD_IA`，`INTELLIGENT_TIERING`，`MAZ_INTELLIGENT_TIERING`，`ARCHIVE`，`DEEP_ARCHIVE`. For more information，please refer to: https://cloud.tencent.com/document/product/436/33417。",
			},
			"etag": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "The ETag generated for the object (an MD5 sum of the object 内容)。",
			},
		},
	}
}
