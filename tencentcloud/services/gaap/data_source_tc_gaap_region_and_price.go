	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"package_type": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "类型 channel 包. `Thunder` 表示 standard channel 组，`Accelerator` 表示 game accelerator channel，和 `CrossBorder` 表示 cross-border channel。",
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"idc_type": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "类型 computer room，dc 表示 DataCenter 数据 center，ec 表示 EdgeComputing edge 节点。",
						},
						"feature_bitmap": {
							Type:        schema.TypeInt,
							Computed:    true,
			...
			"bandwidth_unit_price": {
				Computed:    true,
				Type:        schema.TypeList,
				Description: "proxy 带宽 费用 gradient 价格。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"bandwidth_range": {
							Type: schema.TypeSet,
							...
							Computed:    true,
							Description: "Band 宽度 Range。",
						},
						"bandwidth_unit_price": {
							Type:        schema.TypeFloat,
							Computed:    true,
							Description: "Band 宽度 Unit Price，Unit:yuan/Mbps/day。",
						},
						"discount_bandwidth_unit_price": {
							Type:        schema.TypeFloat,
							Computed:    true,
							Description: "Bandwidth discount 价格，单位:yuan/Mbps/day。",
						},
					},
				},
			},
...
func dataSourceTencentCloudGaapRegionAndPriceRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_gaap_region_and_price.read")()
