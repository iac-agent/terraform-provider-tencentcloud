	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"ip": {
				...
				ForceNew:      true,
				Description:   "IP of the GAAP realserver，conflict with `域名`。",
			},
			"domain": {
				Type:          schema.TypeString,
				Optional:      true,
				...
				ForceNew:      true,
				Description:   "域名 of the GAAP realserver，conflict with `ip`。",
			},
			"name": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidateStringLengthInRange(1, 30),
				Description:  "名称 GAAP realserver，the maximum length is 30。",
			},
			"project_id": {
				Type:        schema.TypeInt,
				Optional:    true,
				Default:     0,
				ForceNew:    true,
				Description: "ID project within the GAAP realserver，'0' means is default project。",
			},
			"tags": {
				Type:        schema.TypeMap,
				Optional:    true,
