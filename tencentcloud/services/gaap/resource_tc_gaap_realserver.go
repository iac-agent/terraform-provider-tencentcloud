	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"ip": {
				...
				ForceNew:      true,
				Description:   "IP 的 GAAP realserver，conflict 使用 `域名`。",
			},
			"domain": {
				Type:          schema.TypeString,
				Optional:      true,
				...
				ForceNew:      true,
				Description:   "域名 的 GAAP realserver，conflict 使用 `ip`。",
			},
			"name": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidateStringLengthInRange(1, 30),
				Description:  "名称 GAAP realserver， 最大 长度 是 30。",
			},
			"project_id": {
				Type:        schema.TypeInt,
				Optional:    true,
				Default:     0,
				ForceNew:    true,
				Description: "ID 项目 within GAAP realserver，'0' 表示 是 默认值 项目。",
			},
			"tags": {
				Type:        schema.TypeMap,
				Optional:    true,
