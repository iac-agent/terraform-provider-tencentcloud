	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"protocol": {
				...
				ForceNew:     true,
				Description:  "协议 of the layer4 listener. Valid 值: `TCP` and `UDP`。",
			},
			"name": {
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: tccommon.ValidateStringLengthInRange(1, 30),
				Description:  "名称 layer4 listener，the maximum length is 30。",
			},
			"port": {
				Type:         schema.TypeInt,
				Required:     true,
			...
			"scheduler": {
				...
				Description:  "Scheduling policy of the layer4 listener，默认值为 `rr`. Valid 值: `rr`，`wrr` and `lc`。",
			},
			"realserver_type": {
				Type:         schema.TypeString,
				Required:     true,
				...
				ForceNew:     true,
				Description:  "类型 realserver. Valid 值: `IP` and `DOMAIN`. NOTES: when the `协议` is specified as `TCP` and the `scheduler` is specified as `wrr`，the item can only be set to `IP`。",
			},
			"proxy_id": {
				Type:        schema.TypeString,
				Required:    true,
			...
			"health_check": {
				...
				Optional:    true,
				Default:     false,
				Description: "表示是否health check is enable，默认值为 `false`。",
			},
			"interval": {
				Type:         schema.TypeInt,
				Optional:     true,
			...
			"connect_timeout": {
				...
				Description:  "Timeout of the health check response，should less than interval，默认值为 2s. NOTES: Require less than `interval`。",
			},
			"healthy_threshold": {
				Type:         schema.TypeInt,
				Optional:     true,
				...
				Description:  "Health threshold，which 表示how many consecutive inspections are successful，the 来源 station is determined to be healthy. Range from 1 to 10. 默认值为 1。",
			},
			"unhealthy_threshold": {
				Type:         schema.TypeInt,
				Optional:     true,
				Default:      1,
				ValidateFunc: tccommon.ValidateIntegerInRange(1, 10),
				Description:  "Unhealthy threshold，which 表示how many consecutive check failures the 来源 station is considered unhealthy. Range from 1 to 10. 默认值为 1。",
			},
			"check_type": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "UDP origin server health 类型 PORT means check 端口，and PING means PING。",
			},
			"check_port": {
				Type:        schema.TypeInt,
				Optional:    true,
				Computed:    true,
				Description: "UDP origin station health check probe 端口",
			},
			"context_type": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "UDP 来源 station health check 端口 probe 消息类型: TEXT represents text. Only used when the health check 类型 is PORT。",
			},
			"send_context": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "UDP 来源 server health check 端口 detection sends messages. Only used when health check 类型 is PORT。",
			},
			"recv_context": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "UDP 来源 server health check 端口 detects received messages. Only used when the health check 类型 is PORT。",
			},
			"client_ip_method": {
				Type:         schema.TypeInt,
				Optional:     true,
				...
				Description:  "The way the listener gets the 客户端 IP，0 for TOA，1 for Proxy 协议，默认值为 0. NOTES: Only supports listeners of `TCP` 协议",
			},
			"realserver_bind_set": {
				Type:     schema.TypeSet,
				Optional: true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"weight": {
							...
							Description:  "Scheduling 权重，默认值为 `1`. The range of values is [1,100]。",
						},
					},
				},
			},
