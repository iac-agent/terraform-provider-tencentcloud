	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"sub_domain": {
				...
				Optional:    true,
				Default:     "@",
				Description: "The 主机 records，默认值为 `@`。",
			},
			"mx": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "MX 优先级，valid when the record 类型 is MX，range 1-20. Note: must set when record 类型 equal MX。",
			},
			"ttl": {
				Type:        schema.TypeInt,
				Optional:    true,
				Default:     600,
				Description: "TTL，the range is 1-604800，and the minimum 值 of different levels of 域名 names is different. 默认为 600。",
			},
			"weight": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "权重 information. An integer from 1 to 100. Only enterprise VIP 域名 names are available，does not pass this parameter，means that the 权重 information is not set。",
			},
			"status": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     "ENABLE",
				Description: "Records the initial state，with values ranging from ENABLE and DISABLE. The 默认为 ENABLE，and if DISABLE is passed in，resolution will not take effect and the limits of load balancing will not be verified。",
			},
			"monitor_status": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "The monitoring 状态 record。",
			},
			"remark": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "The 备注 of record。",
			},
			"record_id": {
				Type:        schema.TypeString,
				Computed:    true,
