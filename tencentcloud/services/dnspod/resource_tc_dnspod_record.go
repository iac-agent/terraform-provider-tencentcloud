	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"sub_domain": {
				...
				Optional:    true,
				Default:     "@",
				Description: "主机 records，默认值为 `@`。",
			},
			"mx": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "MX 优先级，有效 当 记录 类型 是 MX，范围 1-20. 注意: 必须 集合 当 记录 类型 equal MX。",
			},
			"ttl": {
				Type:        schema.TypeInt,
				Optional:    true,
				Default:     600,
				Description: "TTL， 范围 是 1-604800，和 最小 值 的 different levels 的 域名 names 是 different. 默认为 600。",
			},
			"weight": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "权重 信息. An 整数 从 1 到 100. Only enterprise VIP 域名 names 是 可用，does 不 pass 此 参数，表示 该 权重 信息 是 不 集合。",
			},
			"status": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     "ENABLE",
				Description: "Records initial state，使用 值 ranging 从 ENABLE 和 DISABLE. 默认为 ENABLE，和 如果 DISABLE 是 passed 在，resolution 将 不 take effect 和 limits 的 load balancing 将 不 是 verified。",
			},
			"monitor_status": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "监控 状态 记录。",
			},
			"remark": {
				Type:        schema.TypeString,
				Optional:    true,
