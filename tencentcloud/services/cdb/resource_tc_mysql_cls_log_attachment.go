	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"log_type": {
				...
				Description: "日志类型。支持“错误”或“slowlog”。",
			},
			"create_log_set": {
				Optional:    true,
				ForceNew:    true,
			...
			"create_log_topic": {
				...
				ForceNew:    true,
				Type:        schema.TypeBool,
				Description: "是否创建日志主题。",
			},
			"log_set": {
				Required:    true,
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "如果“create_log_set”为“true”，则使用日志集名称，否则使用日志集 ID。",
			},
			"log_topic": {
				Required:    true,
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "如果“create_log_topic”为“true”，则使用日志主题名称，否则使用日志主题 ID。",
			},
			"period": {
				Optional:    true,
				ForceNew:    true,
				Type:        schema.TypeInt,
				Description: "不填写时，日志主题的有效期默认为30天。",
			},
			"create_index": {
				Optional:    true,
				ForceNew:    true,
