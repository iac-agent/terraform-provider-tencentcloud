	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"member_uins": {
				...
				Description: "Uin 列表 delegated admins，Including up 到 20 items。",
			},
			"management_scope": {
				Optional:    true,
				Computed:    true,
				Type:        schema.TypeInt,
				Description: "Management 范围 的 delegated admin. 有效值：1 (all members)，2 (partial members). 默认值：`1`。",
			},
			"management_scope_uins": {
				Optional:    true,
				Type:        schema.TypeList,
				Elem:        &schema.Schema{Type: schema.TypeInt},
				Description: "Uin 列表 managed members. 此 参数 是 有效 当 `management_scope` 是 `2`。",
			},
			"management_scope_node_ids": {
				Optional:    true,
				Type:        schema.TypeList,
				Elem:        &schema.Schema{Type: schema.TypeInt},
				Description: "ID 列表 managed departments. 此 参数 是 有效 当 `management_scope` 是 `2`。",
			},
		},
	}
}
