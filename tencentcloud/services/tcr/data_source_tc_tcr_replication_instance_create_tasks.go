	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"replication_registry_id": {
				...
				Description: "synchronization 实例 ID，see RegistryId 在 DescribeReplicationInstances。",
			},

			"replication_region_id": {
				Required:    true,
				Type:        schema.TypeInt,
				Description: "synchronization 实例 地域 ID，see ReplicationRegionId 在 DescribeReplicationInstances。",
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"task_message": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "任务 状态 信息. 注意：此字段可能返回 null，表示无法获取有效值。",
						},
						"created_time": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "任务 start 名称",
						},
						"finished_time": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "任务 结束时间. 注意：此字段可能返回 null，表示无法获取有效值。",
						},
					},
				},
			},
...
func dataSourceTencentCloudTcrReplicationInstanceCreateTasksRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_tcr_replication_instance_create_tasks.read")()
