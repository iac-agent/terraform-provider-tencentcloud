	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"instance_id": {
				...
				Required:    true,
				Description: "ID instance that the 令牌 belongs to。",
			},
			"token_id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "ID TCR 令牌 to query。",
			},
			"result_output_file": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"token_list": {
				...
				Computed:    true,
				Description: "Information 列表 the dedicated TCR tokens。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"token_id": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "Id of TCR 令牌",
						},
						"enable": {
							Type:        schema.TypeBool,
							Computed:    true,
							Description: "Indicate that the 令牌 is 已启用 or not。",
						},
						"description": {
							Type:        schema.TypeString,
							Computed:    true,
...
func dataSourceTencentCloudTCRTokensRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_tcr_tokens.read")()
	...
	for i := range tokens {
		...
		ids = append(ids, instanceId+tccommon.FILED_SP+*token.Id)
	}
	...
	if ok && output.(string) != "" {
		if e := tccommon.WriteToFile(output.(string), tokenList); e != nil {
			return e
		}
	}
