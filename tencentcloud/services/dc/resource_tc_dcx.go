	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"dc_id": {
				...
				Description:  "ID DC to be queried，application deployment offline。",
			},
			"name": {
				Type:         schema.TypeString,
				Required:     true,
			...
			"dc_owner_account": {
				...
				ForceNew:    true,
				Computed:    true,
				Description: "Connection 所有者，who is the current customer by default. The developer 账号 ID should be entered for shared connections。",
			},
			"network_type": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "类型 network. Valid 值: `VPC`，`BMVPC` and `CCN`. The 默认值为 `VPC`。",
			},
			"network_region": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"route_type": {
				...
				Description:  "类型 route，and available values include BGP and STATIC. The 默认值为 `BGP`。",
			},
			"bgp_asn": {
				Type:        schema.TypeInt,
				Optional:    true,
				Computed:    true,
				ForceNew:    true,
				Description: "BGP ASN of the 用户 A 必填 field within BGP。",
			},
			"bgp_auth_key": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"route_filter_prefixes": {
				...
				Description: "Static route，the network 地址 of the 用户 IDC. It can be modified after setting but cannot be deleted. AN unable field within BGP。",
			},
			"vlan": {
				Type:        schema.TypeInt,
				Optional:    true,
			...
			"state": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "State of the dedicated tunnels. Valid 值: `PENDING`，`ALLOCATING`，`ALLOCATED`，`ALTERING`，`DELETING`，`DELETED`，`COMFIRMING` and `REJECTED`。",
			},
			"create_time": {
				Type:        schema.TypeString,
				Computed:    true,
