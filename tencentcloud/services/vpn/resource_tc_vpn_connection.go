	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"vpc_id": {
				...
				Description: "ID VPC. 必填 if vpn gateway is not in `CCN` 类型，and doesn't make sense for `CCN` vpn gateway。",
			},
			"is_ccn_type": {
				Type:        schema.TypeBool,
				Computed:    true,
				Description: "Indicate whether is ccn 类型 Modification of this field only impacts force new logic of `vpc_id`. If `is_ccn_type` is true，modification of `vpc_id` will be ignored。",
			},
			"customer_gateway_id": {
				Type:        schema.TypeString,
				Required:    true,
			...
			"ike_proto_encry_algorithm": {
				...
				Description: "Proto encrypt algorithm of the IKE operation specification. 有效值：`3DES-CBC`，`AES-CBC-128`，`AES-CBC-192`，`AES-CBC-256`，`DES-CBC`，`SM4`，`AES128GCM128`，`AES192GCM128`，`AES256GCM128`,`AES128GCM128`，`AES192GCM128`，`AES256GCM128`. 默认值为 `3DES-CBC`。",
			},
			"ike_proto_authen_algorithm": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IKE_PROPO_AUTHEN_ALGORITHM_MD5,
				Description: "Proto authenticate algorithm of the IKE operation specification. 有效值：`MD5`，`SHA`，`SHA-256`. Default 值 is `MD5`。",
			},
			"ike_exchange_mode": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IKE_EXCHANGE_MODE_MAIN,
				Description: "Exchange 模式 of the IKE operation specification. 有效值：`AGGRESSIVE`，`MAIN`. 默认值为 `MAIN`。",
			},
			"ike_local_identity": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IKE_IDENTITY_ADDRESS,
				Description: "Local identity way of IKE operation specification. 有效值：`ADDRESS`，`FQDN`. 默认值为 `ADDRESS`。",
			},
			"ike_remote_identity": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IKE_IDENTITY_ADDRESS,
				Description: "Remote identity way of IKE operation specification. 有效值：`ADDRESS`，`FQDN`. 默认值为 `ADDRESS`。",
			},
			"ike_local_address": {
				Type:          schema.TypeString,
				Optional:      true,
				ConflictsWith: []string{"ike_local_fqdn_name"},
				Description:   "Local 地址 of IKE operation specification，valid when ike_local_identity is `ADDRESS`，generally the 值 is `public_ip_address` of the related VPN gateway。",
			},
			"ike_remote_address": {
				Type:          schema.TypeString,
				Optional:      true,
				ConflictsWith: []string{"ike_remote_fqdn_name"},
				Description:   "Remote 地址 of IKE operation specification，valid when ike_remote_identity is `ADDRESS`，generally the 值 is `public_ip_address` of the related customer gateway。",
			},
			"ike_local_fqdn_name": {
				Type:          schema.TypeString,
				Optional:      true,
			...
			"ike_dh_group_name": {
				...
				Description: "DH 组名称 of the IKE operation specification. 有效值：`GROUP1`，`GROUP2`，`GROUP5`，`GROUP14`，`GROUP24`. 默认值为 `GROUP1`。",
			},
			"ike_sa_lifetime_seconds": {
				...
				Description:  "SA lifetime of the IKE operation specification，unit is `second`. The 值 ranges from 60 to 604800. 默认值为 86400 seconds。",
			},
			"ike_version": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     "IKEV1",
				Description: "版本 of the IKE operation specification，values: `IKEV1`，`IKEV2`. 默认值为 `IKEV1`。",
			},
			"ipsec_encrypt_algorithm": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IPSEC_ENCRY_ALGORITHM_3DESCBC,
				Description: "Encrypt algorithm of the IPSEC operation specification. 有效值：`3DES-CBC`，`AES-CBC-128`，`AES-CBC-192`，`AES-CBC-256`，`DES-CBC`，`SM4`，`NULL`，`AES128GCM128`，`AES192GCM128`，`AES256GCM128`. 默认值为 `3DES-CBC`。",
			},
			"ipsec_integrity_algorithm": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IPSEC_INTEGRITY_ALGORITHM_MD5,
				Description: "Integrity algorithm of the IPSEC operation specification. 有效值：`SHA1`，`MD5`，`SHA-256`. 默认值为 `MD5`。",
			},
			"ipsec_sa_lifetime_seconds": {
				...
				Description:  "SA lifetime of the IPSEC operation specification，unit is second. Valid 值 ranges: [180~604800]. 默认值为 3600 seconds。",
			},
			"ipsec_pfs_dh_group": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     "NULL",
				Description: "PFS DH group. Valid 值: `DH-GROUP1`，`DH-GROUP2`，`DH-GROUP5`，`DH-GROUP14`，`DH-GROUP24`，`NULL`. 默认值为 `NULL`。",
			},
			"ipsec_sa_lifetime_traffic": {
				...
				Description:  "SA lifetime of the IPSEC operation specification，unit is KB. The 值 should not be less then 2560. 默认值为 1843200。",
			},
			"tags": {
				Type:        schema.TypeMap,
				Optional:    true,
			...
			"dpd_enable": {
				...
				Description:  "指定是否enable DPD. 有效值：0 (disable) and 1 (enable)。",
			},
			"dpd_timeout": {
				Type:         schema.TypeInt,
				Optional:     true,
				...
				Description:  "DPD timeout 周期Valid 值 ranges: [30~60]，默认值：30; unit: second. If the request is not responded within this 周期，the peer end is considered not exists. This parameter is valid when the 值 of DpdEnable is 1。",
			},
			"dpd_action": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "The 操作 after DPD timeout. 有效值：clear (disconnect) and restart (try again). It is valid when DpdEnable is 1。",
			},
			"create_time": {
				Type:        schema.TypeString,
				Computed:    true,
			...
			"route_type": {
				...
				Description:  "Route 类型 VPN connection. Valid 值: `STATIC`，`StaticRoute`，`Policy`，`Bgp`。",
			},
			"negotiation_type": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "The default negotiation 类型 is `活跃`. 可选值：`活跃` (活跃 negotiation)，`passive` (passive negotiation)，`flowTrigger` (traffic negotiation)。",
			},
			// "route": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"local_bgp_ip": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Cloud BGP 地址 It must be allocated from within the BGP tunnel network segment。",
						},
						"remote_bgp_ip": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "用户 side BGP 地址 It must be allocated from within the BGP tunnel network segment。",
						},
					},
				},
			},
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"probe_type": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Detection 模式，默认为 `NQA`，cannot be modified。",
						},
						"probe_interval": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "Detection interval，Tencent Cloud's interval between two health checks，range [1000-5000]，单位：ms。",
						},
						"probe_threshold": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "Detection times，perform route switching after N consecutive health check failures，range [3-8]，单位：times。",
						},
						"probe_timeout": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "Detection timeout，range [10-5000]，单位：ms。",
						},
					},
				},
			},
			"state": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "State of the connection. Valid 值: `PENDING`，`AVAILABLE`，`DELETING`。",
			},
			"net_status": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "Net 状态 VPN connection. Valid 值: `AVAILABLE`。",
			},
			"enable_health_check": {
				Type:        schema.TypeBool,
				Optional:    true,
			...
			"health_check_local_ip": {
				...
				Optional:    true,
				Computed:    true,
				Description: "Health check the 地址 of this terminal。",
			},
			"health_check_remote_ip": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "Health check peer 地址",
			},
		},
	}
}
