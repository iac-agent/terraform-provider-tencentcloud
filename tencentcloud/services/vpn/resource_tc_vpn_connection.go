	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"vpc_id": {
				...
				Description: "ID VPC. 必填 如果 vpn 网关 是 不 在 `CCN` 类型，和 doesn't make sense 对于 `CCN` vpn 网关。",
			},
			"is_ccn_type": {
				Type:        schema.TypeBool,
				Computed:    true,
				Description: "Indicate whether 是 ccn 类型 Modification 的 此 字段 仅 impacts force new logic 的 `vpc_id`. 如果 `is_ccn_type` 是 true，modification 的 `vpc_id` 将 是 ignored。",
			},
			"customer_gateway_id": {
				Type:        schema.TypeString,
				Required:    true,
			...
			"security_group_policy": {
				Type:     schema.TypeSet,
				Optional: true,
				Description: "SPD 策略 组, 对于 示例: {\"10.0.0.5/24\":[\"172.123.10.5/16\"]}, 10.0.0.5/24 是 vpc intranet segment, 和 172.123.10.5/16 是 IDC 网络 segment." +
			...
			"ike_proto_encry_algorithm": {
				...
				Description: "Proto encrypt algorithm 的 IKE operation 规格. 有效值：`3DES-CBC`，`AES-CBC-128`，`AES-CBC-192`，`AES-CBC-256`，`DES-CBC`，`SM4`，`AES128GCM128`，`AES192GCM128`，`AES256GCM128`,`AES128GCM128`，`AES192GCM128`，`AES256GCM128`. 默认值为 `3DES-CBC`。",
			},
			"ike_proto_authen_algorithm": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IKE_PROPO_AUTHEN_ALGORITHM_MD5,
				Description: "Proto authenticate algorithm 的 IKE operation 规格. 有效值：`MD5`，`SHA`，`SHA-256`. Default 值 是 `MD5`。",
			},
			"ike_exchange_mode": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IKE_EXCHANGE_MODE_MAIN,
				Description: "Exchange 模式 的 IKE operation 规格. 有效值：`AGGRESSIVE`，`MAIN`. 默认值为 `MAIN`。",
			},
			"ike_local_identity": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IKE_IDENTITY_ADDRESS,
				Description: "Local identity way 的 IKE operation 规格. 有效值：`ADDRESS`，`FQDN`. 默认值为 `ADDRESS`。",
			},
			"ike_remote_identity": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IKE_IDENTITY_ADDRESS,
				Description: "Remote identity way 的 IKE operation 规格. 有效值：`ADDRESS`，`FQDN`. 默认值为 `ADDRESS`。",
			},
			"ike_local_address": {
				Type:          schema.TypeString,
				Optional:      true,
				ConflictsWith: []string{"ike_local_fqdn_name"},
				Description:   "Local 地址 的 IKE operation 规格，有效 当 ike_local_identity 是 `ADDRESS`，generally 值 是 `public_ip_address` 的 related VPN 网关。",
			},
			"ike_remote_address": {
				Type:          schema.TypeString,
				Optional:      true,
				ConflictsWith: []string{"ike_remote_fqdn_name"},
				Description:   "Remote 地址 的 IKE operation 规格，有效 当 ike_remote_identity 是 `ADDRESS`，generally 值 是 `public_ip_address` 的 related customer 网关。",
			},
			"ike_local_fqdn_name": {
				Type:          schema.TypeString,
				Optional:      true,
			...
			"ike_dh_group_name": {
				...
				Description: "DH 组名称 的 IKE operation 规格. 有效值：`GROUP1`，`GROUP2`，`GROUP5`，`GROUP14`，`GROUP24`. 默认值为 `GROUP1`。",
			},
			"ike_sa_lifetime_seconds": {
				...
				Description:  "SA lifetime 的 IKE operation 规格，单位 是 `second`. 值 ranges 从 60 到 604800. 默认值为 86400 秒。",
			},
			"ike_version": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     "IKEV1",
				Description: "版本 的 IKE operation 规格，值: `IKEV1`，`IKEV2`. 默认值为 `IKEV1`。",
			},
			"ipsec_encrypt_algorithm": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IPSEC_ENCRY_ALGORITHM_3DESCBC,
				Description: "Encrypt algorithm 的 IPSEC operation 规格. 有效值：`3DES-CBC`，`AES-CBC-128`，`AES-CBC-192`，`AES-CBC-256`，`DES-CBC`，`SM4`，`NULL`，`AES128GCM128`，`AES192GCM128`，`AES256GCM128`. 默认值为 `3DES-CBC`。",
			},
			"ipsec_integrity_algorithm": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     svcvpc.VPN_IPSEC_INTEGRITY_ALGORITHM_MD5,
				Description: "Integrity algorithm 的 IPSEC operation 规格. 有效值：`SHA1`，`MD5`，`SHA-256`. 默认值为 `MD5`。",
			},
			"ipsec_sa_lifetime_seconds": {
				...
				Description:  "SA lifetime 的 IPSEC operation 规格，单位 是 second. 有效 值 ranges: [180~604800]. 默认值为 3600 秒。",
			},
			"ipsec_pfs_dh_group": {
				Type:        schema.TypeString,
				Optional:    true,
				Default:     "NULL",
				Description: "PFS DH 组. 有效 值: `DH-GROUP1`，`DH-GROUP2`，`DH-GROUP5`，`DH-GROUP14`，`DH-GROUP24`，`NULL`. 默认值为 `NULL`。",
			},
			"ipsec_sa_lifetime_traffic": {
				...
				Description:  "SA lifetime 的 IPSEC operation 规格，单位 是 KB. 值 should 不 是 less then 2560. 默认值为 1843200。",
			},
			"tags": {
				Type:        schema.TypeMap,
				Optional:    true,
			...
			"dpd_enable": {
				...
				Description:  "指定是否enable DPD. 有效值：0 (disable) 和 1 (启用)。",
			},
			"dpd_timeout": {
				Type:         schema.TypeInt,
				Optional:     true,
				...
				Description:  "DPD 超时 周期Valid 值 ranges: [30~60]，默认值：30; 单位: second. 如果 请求 是 不 responded within 此 周期， peer end 是 considered 不 exists. 此 参数 是 有效 当 值 的 DpdEnable 是 1。",
			},
			"dpd_action": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "操作 after DPD 超时. 有效值：clear (disconnect) 和 restart (try again). It 是 有效 当 DpdEnable 是 1。",
			},
			"create_time": {
				Type:        schema.TypeString,
				Computed:    true,
			...
			"route_type": {
				...
				Description:  "Route 类型 VPN 连接. 有效 值: `STATIC`，`StaticRoute`，`Policy`，`Bgp`。",
			},
			"negotiation_type": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "默认值 negotiation 类型 是 `活跃`. 可选值：`活跃` (活跃 negotiation)，`passive` (passive negotiation)，`flowTrigger` (流量 negotiation)。",
			},
			// "route": {
			...
			// 	ForceNew:    true,
			// 	MaxItems:    1,
			// 	Description: "Create channel routing 信息.",
			// 	Elem: &schema.Resource{
			// 		Schema: map[string]*schema.Schema{
			// 			"destination_cidr_block": {
			// 				Type:        schema.TypeString,
			// 				Required:    true,
			// 				Description: "Destination IDC 网络 segment.",
			// 			},
			// 			"priority": {
			// 				Type:        schema.TypeInt,
			// 				Optional:    true,
			// 				Description: "Priority. Optional 值 [0, 100].",
			// 			},
			// 		},
			// 	},
			// },
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"local_bgp_ip": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Cloud BGP 地址 It 必须 是 allocated 从 within BGP tunnel 网络 segment。",
						},
						"remote_bgp_ip": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "用户 side BGP 地址 It 必须 是 allocated 从 within BGP tunnel 网络 segment。",
						},
					},
				},
			},
			"health_check_config": {
				...
				Computed:    true,
				MaxItems:    1,
				Description: "VPN channel health check 配置。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"probe_type": {
							Type:        schema.TypeString,
							Optional:    true,
							Description: "Detection 模式，默认为 `NQA`，不能 是 modified。",
						},
						"probe_interval": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "Detection 间隔，Tencent Cloud's 间隔 between two health checks，范围 [1000-5000]，单位：ms。",
						},
						"probe_threshold": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "Detection times，perform 路由 switching after N consecutive health check failures，范围 [3-8]，单位：times。",
						},
						"probe_timeout": {
							Type:        schema.TypeInt,
							Optional:    true,
							Description: "Detection 超时，范围 [10-5000]，单位：ms。",
						},
					},
				},
			},
			"state": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "State 的 连接. 有效 值: `PENDING`，`AVAILABLE`，`DELETING`。",
			},
			"net_status": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "Net 状态 VPN 连接. 有效 值: `AVAILABLE`。",
			},
			"enable_health_check": {
				Type:        schema.TypeBool,
				Optional:    true,
				Computed:    true,
				Description: "Whether intra-tunnel health checks 是 支持。",
			},
			"health_check_local_ip": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "Health check 地址 的 此 terminal。",
			},
