	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"instance_ids": {
				...
				Description: "列表 实例 IDs。",
			},

			"attach_detail_set": {
				Computed:    true,
				Type:        schema.TypeList,
				Description: "Mount 信息 列表。",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"attached_disk_count": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "数量 elastic 云 disks mounted 到 实例。",
						},
						"max_attach_count": {
							Type:        schema.TypeInt,
							Computed:    true,
							Description: "数量 elastic 云 disks 该 可以 是 mounted。",
						},
					},
				},
			},
...
func dataSourceTencentCloudLighthouseInstanceDiskNumRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_lighthouse_instance_disk_num.read")()
