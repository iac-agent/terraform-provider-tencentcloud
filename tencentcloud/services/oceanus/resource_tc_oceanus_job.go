	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"name": {
				...
				Description: "The 名称 job. It can be composed of Chinese，English，numbers，hyphens (-)，underscores (_)，and periods (.)，and the length cannot exceed 50 characters. Note that the 作业名称 cannot be the same as an existing job。",
			},
			"job_type": {
				Required:     true,
				Type:         schema.TypeInt,
				ValidateFunc: tccommon.ValidateAllowedIntValue(JOB_TYPE),
				Description:  "The 类型 job. 1 表示SQL job，and 2 表示JAR job。",
			},
			"cluster_type": {
				Required:     true,
				Type:         schema.TypeInt,
				ValidateFunc: tccommon.ValidateAllowedIntValue(CLUSTER_TYPE),
				Description:  "The 类型 cluster. 1 表示shared cluster，and 2 表示exclusive cluster。",
			},
			"cluster_id": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "When ClusterType=2，it 为必填项 to 指定ID of the exclusive cluster to which the job is submitted。",
			},
			"cu_mem": {
				Optional:     true,
				Type:         schema.TypeInt,
				...
				Description:  "Set the memory specification of each CU，（GB）。 It supports 2，4，8，and 16 (which needs to apply for the whitelist before use). The 默认为 4，that is，1 CU corresponds to 4 GB of running memory。",
			},
			"remark": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "The 备注 information of the job. It can be set arbitrarily。",
			},
			"folder_id": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "The folder ID to which the 作业名称 belongs. The root directory is root。",
			},
			"flink_version": {
				Optional:    true,
				Type:        schema.TypeString,
				Description: "The Flink 版本 that the job runs。",
			},
			"work_space_id": {
				Optional:    true,
				Type:        schema.TypeString,
