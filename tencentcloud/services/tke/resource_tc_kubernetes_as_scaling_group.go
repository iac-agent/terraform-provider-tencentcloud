...
func kubernetesAsScalingConfigPara() map[string]*schema.Schema {
	needSchema := map[string]*schema.Schema{
		...
		"system_disk_type": {
			...
			Description:  "类型 a CVM disk. Valid 值: `CLOUD_PREMIUM` and `CLOUD_SSD`. 默认为 `CLOUD_PREMIUM`。",
		},
		"system_disk_size": {
			Type:         schema.TypeInt,
			Optional:     true,
			...
			Elem: &schema.Resource{
				Schema: map[string]*schema.Schema{
					"disk_type": {
						...
						Default:      svcas.SYSTEM_DISK_TYPE_CLOUD_PREMIUM,
						ValidateFunc: tccommon.ValidateAllowedStringValue(svcas.SYSTEM_DISK_ALLOW_TYPE),
						Description:  "Types of disk. Valid 值: `CLOUD_PREMIUM` and `CLOUD_SSD`。",
					},
					"disk_size": {
						Type:        schema.TypeInt,
						Optional:    true,
...
func kubernetesAsScalingGroupPara() map[string]*schema.Schema {
	...
	asGroupSchema := map[string]*schema.Schema{
		...
		"max_size": {
			...
			Description:  "最大CVM instances (0~2000)。",
		},
		"min_size": {
			Type:         schema.TypeInt,
			Required:     true,
			ValidateFunc: tccommon.ValidateIntegerInRange(0, 2000),
			Description:  "最小CVM instances (0~2000)。",
		},
		"vpc_id": {
			Type:        schema.TypeString,
			Required:    true,
