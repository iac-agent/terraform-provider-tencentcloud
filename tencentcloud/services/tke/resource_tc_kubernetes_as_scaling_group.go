	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"labels": {
				...
				Optional:    true,
				ForceNew:    true,
				Description: "Labels 的 kubernetes AS Group 创建 nodes。",
			},
			"extra_args": {
				Type:        schema.TypeList,
				Optional:    true,
...
func kubernetesAsScalingConfigPara() map[string]*schema.Schema {
	needSchema := map[string]*schema.Schema{
		...
		"system_disk_type": {
			...
			Description:  "类型 CVM 磁盘. 有效 值: `CLOUD_PREMIUM` 和 `CLOUD_SSD`. 默认为 `CLOUD_PREMIUM`。",
		},
		"system_disk_size": {
			Type:         schema.TypeInt,
			Optional:     true,
			...
			Elem: &schema.Resource{
				Schema: map[string]*schema.Schema{
					"disk_type": {
						...
						Default:      svcas.SYSTEM_DISK_TYPE_CLOUD_PREMIUM,
						ValidateFunc: tccommon.ValidateAllowedStringValue(svcas.SYSTEM_DISK_ALLOW_TYPE),
						Description:  "Types 的 磁盘. 有效 值: `CLOUD_PREMIUM` 和 `CLOUD_SSD`。",
					},
					"disk_size": {
						Type:        schema.TypeInt,
						Optional:    true,
...
func kubernetesAsScalingGroupPara() map[string]*schema.Schema {
	...
	asGroupSchema := map[string]*schema.Schema{
		...
		"max_size": {
			...
			Description:  "最大CVM 实例 (0~2000)。",
		},
		"min_size": {
			Type:         schema.TypeInt,
			Required:     true,
			ValidateFunc: tccommon.ValidateIntegerInRange(0, 2000),
			Description:  "最小CVM 实例 (0~2000)。",
		},
		"vpc_id": {
			Type:        schema.TypeString,
			Required:    true,
		...
		"project_id": {
			...
			ForceNew:    true,
			Default:     0,
			Description: "Specifys 到 其中 项目 scaling 组 belongs。",
		},
		"subnet_ids": {
			Type:        schema.TypeList,
			Optional:    true,
		...
		"termination_policies": {
			...
			ForceNew:    true,
			MaxItems:    1,
			Description: "Available 值 对于 termination policies include `OLDEST_INSTANCE` 和 `NEWEST_INSTANCE`。",
			Elem: &schema.Schema{
				Type:    schema.TypeString,
				Default: svcas.SCALING_GROUP_TERMINATION_POLICY_OLDEST_INSTANCE,
				ValidateFunc: tccommon.ValidateAllowedStringValue([]string{svcas.SCALING_GROUP_TERMINATION_POLICY_OLDEST_INSTANCE,
					svcas.SCALING_GROUP_TERMINATION_POLICY_NEWEST_INSTANCE}),
			},
		},
		"retry_policy": {
			Type:        schema.TypeString,
			...
			Default:     svcas.SCALING_GROUP_RETRY_POLICY_IMMEDIATE_RETRY,
			ValidateFunc: tccommon.ValidateAllowedStringValue([]string{svcas.SCALING_GROUP_RETRY_POLICY_IMMEDIATE_RETRY,
				svcas.SCALING_GROUP_RETRY_POLICY_INCREMENTAL_INTERVALS}),
		},
		"tags": {
			Type:        schema.TypeMap,
			Optional:    true,
...
func resourceKubernetesAsScalingGroupRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_kubernetes_as_scaling_group.read")()
...
func resourceKubernetesAsScalingGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	defer tccommon.LogElapsed("resource.tencentcloud_kubernetes_as_scaling_group.create")()
	var (
		logId       = tccommon.GetLogId(tccommon.ContextNil)
		ctx         = context.WithValue(context.TODO(), tccommon.LogIdKey, logId)
	...
	err = resource.Retry(5*tccommon.ReadRetryTimeout, func() *resource.RetryError {
		...
		if scalingGroup != nil && *scalingGroup.InActivityStatus == svcas.SCALING_GROUP_NOT_IN_ACTIVITY_STATUS {
			return nil
		}
...
func resourceKubernetesAsScalingGroupUpdate(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_kubernetes_as_scaling_group.update")()
...
func resourceKubernetesAsScalingGroupDelete(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("resource.tencentcloud_kubernetes_as_scaling_group.delete")()
	...
	if *scalingGroup.InstanceCount > 0 || *scalingGroup.DesiredCapacity > 0 {
		if err := resource.Retry(tccommon.WriteRetryTimeout, func() *resource.RetryError {
			return tccommon.RetryError(asService.ClearScalingGroupInstance(ctx, asGroupId))
		}); err != nil {
			return err
		}
	}
