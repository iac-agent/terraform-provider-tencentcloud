	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"instance_id": {
				...
				ValidateFunc: tccommon.ValidateStringLengthInRange(1, 25),
				Description:  "The ID target resource to associate with the Elastic IP (EIP). Supported targets include a CVM instance，SaaS WAF instance，CLB instance，or a VPC endpoint.\nLimitation (GWLB VPC endpoint): Only an EIP in the bound state can be associated with a GWLB-类型 VPC endpoint through this field，enabling more advanced networking scenarios.\nMutual exclusivity: This field conflicts with `network_interface_id` and `private_ip`. Only one association target can be specified per request。",
			},
			"network_interface_id": {
				Type:         schema.TypeString,
				ForceNew:     true,
				...
				Description: "表示network interface id like `eni-xxxxxx`. This field is conflict with `instance_id`。",
			},
			"private_ip": {
				Type:         schema.TypeString,
				ForceNew:     true,
				...
				Description: "表示an IP belongs to the `network_interface_id`. This field is conflict with `instance_id`。",
			},
			...
			"cdc_id": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "ID dedicated cluster。",
			},
		},
	}
}
