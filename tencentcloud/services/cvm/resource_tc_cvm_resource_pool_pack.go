	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"zone": {
				...
				Description: "The availability 可用区 where the resource pool pack is located. 格式: ap-guangzhou-6。",
			},
			"instance_type": {
				Required:    true,
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "实例类型 for the resource pool pack. Only half-machine/full-machine specifications are supported. 格式: SA9.96XLARGE1152 (SA9 half-machine)。",
			},
			"period": {
				Required:    true,
				ForceNew:    true,
				Type:        schema.TypeInt,
				Description: "The 周期 of the resource pool pack in months. Range: 1-60。",
			},
			"resource_pool_pack_type": {
				...
				Computed:    true,
				Type:        schema.TypeString,
				Description: "Resource pool pack 类型 Options: EXCLUSIVE (exclusive，default)，SHARED (shared). Note: Only EXCLUSIVE is supported in the first phase。",
			},
			"auto_placement": {
				Optional:    true,
				ForceNew:    true,
				...
				Computed:    true,
				Description: "Auto placement switch. 默认值：true. When 已启用，the system will search for suitable pools in pools with this capability 已启用 when creating instances without specifying a resource pool。",
			},
			"dedicated_resource_pool_pack_name": {
				Optional:    true,
				ForceNew:    true,
				Type:        schema.TypeString,
				Description: "The 名称 resource pool pack. Length: 1-60 characters，supports Chinese，English，numbers，hyphens '-'，and underscores '_'。",
			},
			"renew_flag": {
				Optional:    true,
				ForceNew:    true,
				Type:        schema.TypeString,
				Computed:    true,
				Description: "自动续费标识 Options: NOTIFY_AND_AUTO_RENEW (notify and auto renew)，NOTIFY_AND_MANUAL_RENEW (notify and manual renew，default)，DISABLE_NOTIFY_AND_MANUAL_RENEW (do not notify and manual renew)。",
			},
			...
			"dedicated_resource_pack_id": {
				Computed:    true,
				Type:        schema.TypeString,
				Description: "The ID created resource pool pack。",
			},
			"status": {
				Computed:    true,
				Type:        schema.TypeString,
				Description: "Resource pool pack 状态 Values: CREATING (creating)，ACTIVE (running)，FAILED (creation failed)，RETIRED (expired)。",
			},
			"instance_family": {
				Computed:    true,
				Type:        schema.TypeString,
				Description: "Instance family. 格式: SA9。",
			},
			"start_time": {
				Computed:    true,
				Type:        schema.TypeString,
				Description: "Resource pool pack 创建时间. 格式: YYYY-MM-DDThh:mm:ssZ。",
			},
			"end_time": {
				Computed:    true,
				Type:        schema.TypeString,
				Description: "Resource pool pack 过期时间. 格式: YYYY-MM-DDThh:mm:ssZ。",
			},
		},
	}
}
