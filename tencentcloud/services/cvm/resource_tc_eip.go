	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"type": {
				...
				ForceNew:    true,
				Description: "类型 eip. Valid 值:  `EIP` and `AnycastEIP` and `HighQualityEIP` and `AntiDDoSEIP` and `ResidentialEIP`. 默认为 `EIP`。",
			},
			"anycast_zone": {
				Type:        schema.TypeString,
				Optional:    true,
				ForceNew:    true,
				Description: "The 可用区 of anycast. Valid 值: `ANYCAST_ZONE_GLOBAL` and `ANYCAST_ZONE_OVERSEAS`。",
			},
			"applicable_for_clb": {
				Type:        schema.TypeBool,
				Optional:    true,
				Description: "表示是否anycast eip can be associated to a CLB。",
			...
			"internet_service_provider": {
				...
				Optional:    true,
				ForceNew:    true,
				Description: "Internet service provider of eip. Valid 值: `BGP`，`CMCC`，`CTCC` and `CUCC`。",
			},
			"internet_charge_type": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "The charge 类型 eip. 有效值：`BANDWIDTH_PACKAGE`，`BANDWIDTH_POSTPAID_BY_HOUR`，`BANDWIDTH_PREPAID_BY_MONTH` and `TRAFFIC_POSTPAID_BY_HOUR`。",
			},
			"prepaid_period": {
				Type:         schema.TypeInt,
				Optional:     true,
				ValidateFunc: tccommon.ValidateAllowedIntValue(svcvpc.EIP_AVAILABLE_PERIOD),
				Description:  "周期 of instance. 默认值：`1`. Valid 值: `1`，`2`，`3`，`4`，`6`，`7`，`8`，`9`，`12`，`24`，`36`. NOTES: must set when `internet_charge_type` is `BANDWIDTH_PREPAID_BY_MONTH`。",
			},
			...
			"auto_renew_flag": {
				...
				Description:  "Auto 续费标识  0 - default state (manual renew); 1 - automatic renew; 2 - explicit no automatic renew. NOTES: Only supported prepaid EIP。",
			},
			...
			"internet_max_bandwidth_out": {
				...
				Optional:    true,
				Computed:    true,
				Description: "The bandwidth 限制 of EIP，unit is Mbps。",
			},
			"tags": {
				Type:        schema.TypeMap,
				Optional:    true,
			...
			"bandwidth_package_id": {
				...
				Optional:    true,
				Computed:    true,
				Description: "ID bandwidth package，it will set when `internet_charge_type` is `BANDWIDTH_PACKAGE`。",
			},
			"egress": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "Network egress. It 默认为 `center_egress1`. If you want to try the egress feature，please [提交 a ticket](https://console.cloud.tencent.com/workorder/category)。",
			},
			"anti_ddos_package_id": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "ID anti DDos package，it must set when `类型` is `AntiDDoSEIP`。",
			},
			"cdc_id": {
				Type:        schema.TypeString,
				Optional:    true,
