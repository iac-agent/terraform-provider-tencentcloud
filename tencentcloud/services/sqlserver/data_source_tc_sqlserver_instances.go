	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"id": {
				...
				Optional:    true,
				Description: "ID 的 SQL Server 实例 到 是 查询.",
			},
			"name": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "Name 的 SQL Server 实例 到 是 查询.",
			},
			"project_id": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "Project ID 的 SQL Server 实例 到 是 查询.",
			},
			"vpc_id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "Vpc ID 的 SQL Server 实例 到 是 查询.",
			},
			"subnet_id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "Subnet ID 的 SQL Server 实例 到 是 查询.",
			},
			"result_output_file": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"instance_list": {
				...
				Computed:    true,
				Description: "A 列表 的 SQL Server 实例. Each element contains following attributes.",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"charge_type": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "Pay 类型 的 SQL Server 实例. For now, 仅 `POSTPAID_BY_HOUR` 是 有效.",
						},
						"engine_version": {
							Type:        schema.TypeString,
							Computed:    true,
						...
						"ha_type": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "实例 类型. `DUAL` (dual-服务器 high availability), `CLUSTER` (集群).",
						},
						"vpc_id": {
							Type:        schema.TypeString,
							Computed:    true,
						...
						"tags": {
							Type:        schema.TypeMap,
							Computed:    true,
							Description: "Tags 的 SQL Server 实例.",
						},
					},
				},
			},
...
func dataSourceTencentCloudSqlserverInstanceRead(d *schema.ResourceData, meta interface{}) error {
	defer tccommon.LogElapsed("data_source.tencentcloud_sqlserver_instances.read")()
