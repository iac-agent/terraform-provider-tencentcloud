	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"strategy": {
				...
				Description:  "Backup 策略 (0: 实例 备份, 1: multi-数据库 备份).",
			},
			"db_names": {
				Type:        schema.TypeSet,
				Optional:    true,
				Elem:        &schema.Schema{Type: schema.TypeString},
				Description: "List 的 names 的 databases 到 是 backed up (必填 仅 对于 multi-数据库 备份).",
			},
			"instance_id": {
				Required:    true,
				Type:        schema.TypeString,
				Description: "实例 ID 在 格式 的 mssql-i1z41iwd.",
			},
			"backup_name": {
				Optional:    true,
				Computed:    true,
				Type:        schema.TypeString,
				Description: "Backup 名称. 如果 此 参数 是 left 空, 备份 名称 在 格式 的 [实例 ID]_[Backup start timestamp] 将 是 automatically generated.",
			},
			"flow_id": {
				Computed:    true,
				Type:        schema.TypeString,
