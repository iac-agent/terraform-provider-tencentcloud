	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"publish_subscribe_name": {
				...
				Description: "名称 的 Publish 和 Subscribe. Default 是 `default_name`.",
			},
			"delete_subscribe_db": {
				Type:        schema.TypeBool,
				Optional:    true,
				Default:     false,
				Description: "Whether 到 delete subscriber 数据库 当 deleting Publish 和 Subscribe. `true` 对于 deletes subscribe 数据库, `false` 对于 does 不 delete subscribe 数据库. 默认值 是 `false`.",
			},
			"database_tuples": {
				Type:        schema.TypeSet,
				Required:    true,
				MinItems:    1,
				MaxItems:    80,
				Description: "Database Publish 和 Publish relationship 列表. elements inside 可以 是 删除 和 added individually, 但 modification 是 不 allowed.",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"publish_database": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Publish 数据库.",
						},
						"subscribe_database": {
							Type:        schema.TypeString,
							Required:    true,
							Description: "Subscribe 数据库.",
						},
					},
				},
			},
