	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"rule_id": {
				...
				Description: "CLB监听规则ID。仅支持`HTTPS`和`HTTP`协议的监听器。",
			},
			"domain": {
				Type:          schema.TypeString,
				ForceNew:      true,
				...
				Description: "目标转发规则的域。提供参数“rule_id”时不生效。",
			},
			"url": {
				Type:          schema.TypeString,
				ForceNew:      true,
				...
				ConflictsWith: []string{"rule_id"},
				Description: "目标转发规则的URL。提供参数“rule_id”时不生效。",
			},
			"protocol_type": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "侦听器内的协议类型。",
			},
			"targets": {
				Type:        schema.TypeSet,
				Required:    true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"instance_id": {
							...
							Optional:    true,
							Default:     "",
							Description: "后端服务器的 CVM 实例 ID，与 eni_ip 冲突，但必须指定其中之一。",
						},
						"eni_ip": {
							Type:        schema.TypeString,
							Optional:    true,
							Default:     "",
							Description: "Eni后端服务器的IP地址，与instance_id冲突，但必须指定其中之一。",
						},
						"port": {
							Type:         schema.TypeInt,
							Required:     true,
							ValidateFunc: tccommon.ValidateIntegerInRange(0, 65535),
							Description: "后端服务器的端口。有效值范围：（0~65535）。",
						},
						"weight": {
							Type:         schema.TypeInt,
							Optional:     true,
							...
							Description: "后端服务的转发权重。有效值范围：（0~100）。默认为“10”。",
						},
					},
				},
			},
