	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"load_balancer_id": {
				...
				Optional:    true,
				ForceNew:    true,
				Description: "CLB实例ID（load_balancer_id和target_group_id至少需要一个）。",
			},
			"target_group_id": {
				Type:        schema.TypeString,
				Optional:    true,
				ForceNew:    true,
				Description: "目标组 ID（load_balancer_id 和 target_group_id 至少需要一个）。",
			},
			"associations": {
				Required:    true,
				ForceNew:    true,
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"load_balancer_id": {
							...
							Optional:    true,
							ForceNew:    true,
							Description: "CLB实例ID，当绑定目标为目标组时，需要关联中的load_balancer_id。",
						},
						"target_group_id": {
							Type:        schema.TypeString,
							Optional:    true,
							ForceNew:    true,
							Description: "目标组ID，当绑定目标为clb时，需要associations中的target_group_id。",
						},
						"listener_id": {
							Type:        schema.TypeString,
							ForceNew:    true,
