	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"inbound_rule_uuid_list": {
				...
				Optional:    true,
				Computed:    true,
				Description: "唯一 IDs 的 inbound 规则，其中 不是必填项 当 您 create 规则. 优先级 将 是 determined 通过 索引 position 的 UUID 在 列表。",
			...
			"outbound_rule_uuid_list": {
				...
				Optional:    true,
				Computed:    true,
				Description: "唯一 IDs 的 outbound 规则，其中 不是必填项 当 您 create 规则. 优先级 将 是 determined 通过 索引 position 的 UUID 在 列表。",
				Elem:        &schema.Schema{Type: schema.TypeInt},
			},
		},
	}
