	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"force_delete": {
				...
				Optional:    true,
				Default:     false,
				Description: "Indicate 是否force deletes the CAM 用户 If set false，the API secret 键 will be checked and failed when exists; otherwise the 用户 will be deleted directly. 默认为 false。",
			},
			"use_api": {
				Type:        schema.TypeBool,
				Optional:    true,
				Default:     true,
				Description: "Indicate 是否generate the API secret 键 or not。",
			},
			"console_login": {
				Type:        schema.TypeBool,
				Optional:    true,
				Default:     false,
				Description: "Indicate 是否CAM 用户 can login to the web console or not。",
			},
			"password": {
				Type:         schema.TypeString,
				Optional:     true,
				...
				Description:  "The 密码 of the CAM 用户 密码 should be at least 8 characters and no more than 32 characters，includes uppercase letters，lowercase letters，numbers and special characters. Only 必填 when `console_login` is true. If not set，a random 密码 will be automatically generated。",
			},
			"need_reset_password": {
				Type:        schema.TypeBool,
				Optional:    true,
				Default:     true,
				Description: "Indicate 是否CAM 用户 need to reset the 密码 when first logins。",
			},
			"phone_num": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "Phone 数量 the CAM 用户",
			},
			"country_code": {
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
				Description: "Country 代码 of the phone number，for example: '86'。",
			},
			"email": {
				Type:        schema.TypeString,
				Optional:    true,
			...
			"secret_key": {
				...
				Computed:    true,
				Sensitive:   true,
				Description: "Secret 键 of the CAM 用户",
			},
			"secret_id": {
				Type:        schema.TypeString,
				Computed:    true,
