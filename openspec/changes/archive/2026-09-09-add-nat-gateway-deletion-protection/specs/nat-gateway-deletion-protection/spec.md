...
- **THEN** the NAT gateway SHALL be created with deletion protection enabled by passing `DeletionProtectionEnabled` to the `CreateNatGateway` API
...
- **THEN** the NAT gateway SHALL have deletion protection enabled after the update via `ModifyNatGatewayAttribute`
...
- **THEN** the NAT gateway SHALL have deletion protection disabled after the update via `ModifyNatGatewayAttribute`

#### Scenario: Read deletion protection state
- **WHEN** user reads a `tencentcloud_nat_gateway` resource
- **THEN** the `deletion_protection_enabled` attribute SHALL reflect the current deletion protection state from `DescribeNatGateways` response
