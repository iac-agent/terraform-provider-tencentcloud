# VPC Replace Routes With Route Policy Config Parameters Specification

**Capability**: `vpc-replace-routes-route-policy-config-params`

## Purpose

Defines the canonical requirements for the `tencentcloud_vpc_replace_routes_with_route_policy_config` resource, covering write-path parameter wiring to `ReplaceRoutesWithRoutePolicy`, read-path query parameters for `DescribeRouteTables`, a dedicated non-breaking read helper, empty-response handling, and mock-based unit tests.
...
This specification is fully backward compatible:
- All new parameters are `Optional`
