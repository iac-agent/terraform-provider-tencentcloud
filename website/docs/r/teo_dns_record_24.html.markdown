...
* `dns_records` - DNS record list.
  ...
  * `status` - DNS record resolution status. valid values are: <li>enable: has taken effect;</li><li>disable: has been disabled.</li> note: Status is only used as an output parameter and cannot be used as an input parameter in ModifyDnsRecords. If this parameter is passed, it will be ignored.
...
<li>SRV: identifies a server using a service, commonly used in microsoft's directory management.</li>.
  * `weight` - DNS record weight. value range -1 to 100, -1 means no weight is set, 0 means no resolution. weight configuration is only applicable when type (dns record type) is A, AAAA, or CNAME.
