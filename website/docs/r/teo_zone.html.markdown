...
* `allow_duplicates` - (Optional, Bool, ForceNew) Whether to allow duplicate access. true: Allow duplicate access; false: Do not allow duplicate access. Default value is false.
* `jump_start` - (Optional, Bool, ForceNew) Whether to skip existing DNS record scanning during site creation. Default value is false.
...
* `ownership_verification` - Ownership verification information. Note: This field may return null, indicating that no valid value can be obtained.
  ...
  * `file_verification` - CNAME, no-domain access, using file verification information. For details, please refer to [Site/Domain Name Ownership Verification](https://cloud.tencent.com/document/product/1552/70789#7af6ecf8-afca-4e35-8811-b5797ed1bde5). Note: This field may return null, indicating that no valid value can be obtained.
    * `content` - The content of the verification file.
    * `path` - The URL path for file verification.
  * `ns_verification` - NS access, switching DNS server information. For details, please refer to [Modify DNS Server](https://cloud.tencent.com/document/product/1552/90452). Note: This field may return null, indicating that no valid value can be obtained.
    * `name_servers` - NS access, DNS server addresses assigned to the user.
