...
### Configure SSL certificate with client certificate (edge mutual TLS)
...
* `client_cert_info` - (Optional, List) In the edge mutual authentication scenario, this field represents the client's CA certificate, which is deployed in the EO node for the EO node to authenticate the client certificate. When used as an input parameter, it is left blank to indicate retaining the original configuration.
