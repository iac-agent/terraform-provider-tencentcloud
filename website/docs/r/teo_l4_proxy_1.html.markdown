...
layout: "tencentcloud"
page_title: "TencentCloud: tencentcloud_teo_l4_proxy_1"
sidebar_current: "docs-tencentcloud-resource-teo_l4_proxy_1"
description: |-
  Provides a resource to create a TEO L4 proxy instance
---

# tencentcloud_teo_l4_proxy_1
...
```hcl
resource "tencentcloud_teo_l4_proxy_1" "proxy" {
  accelerate_mainland = "off"
  area                = "overseas"
  ipv6                = "off"
  proxy_name          = "proxy-test"
  static_ip           = "off"
  zone_id             = "zone-2qtuhspy7cr7"
}
```
...
* `ddos_protection_config` - (Optional, List, ForceNew, **Deprecated**) It has been deprecated from version 1.82.90. Please do not use this field anymore. Layer 3/Layer 4 DDoS protection. The default protection option of the platform will be used if it is left empty. For details, see [Exclusive DDoS Protection Usage](https://intl.cloud.tencent.com/document/product/1552/95994?from_cn_redirect=1).
...
* `cname` - Access CNAME.
* `ips` - When fixed IP is enabled, this value will return the corresponding access IP; when not enabled, this value is empty.
* `l4proxy_rule_count` - Number of forwarding rules under the L4 proxy instance.
* `proxy_id` - L4 proxy instance ID.
* `status` - L4 proxy instance status. `online`: Enabled; `offline`: Disabled; `progress`: Deploying; `stopping`: Disabling; `banned`: Banned; `fail`: Deployment failed/Disablement failed.
...
```
terraform import tencentcloud_teo_l4_proxy_1.proxy zone_id#proxy_id
```
